...
def test_request_processor_waits_for_terminal_decoded_close() -> None:
    spec = TurnGenerationSpec(
        close_text="</think>",
        close_token_ids=(10, 11, 12),
        injected_token_ids=(3,),
    ...
    adapter._tokenizer = SimpleNamespace(
        decode=lambda token_ids, **_kwargs: {
            (20,): "mention </think> trailing",
            (20, 21): "mention </think> trailing final</think>",
        }.get(tuple(token_ids), "")
    )
    processor = adapter.new_req_logits_processor(
        SimpleNamespace(extra_args=spec.to_extra_args())
    ...
    logits = torch.arange(16, dtype=torch.float32)
    interior = processor([20], logits)
    terminal = processor([20, 21], logits)

    assert torch.isfinite(interior[0])
    assert not torch.isfinite(interior[3])
    assert torch.isfinite(terminal).nonzero().flatten().tolist() == [3]
