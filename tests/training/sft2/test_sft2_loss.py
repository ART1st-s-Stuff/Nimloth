...
def test_dino_alignment_loss_directly_matches_projected_query_state() -> None:
    ...
    loss, metrics = compute_dino_alignment_loss(
    ...
    assert metrics == {"dino_mse": 0.0}
    loss.backward()
    assert query_state.grad is not None
    assert state_proj.weight.grad is not None


def test_dino_alignment_loss_rejects_non_direct_dimension_mapping() -> None:
    class FakeDINOEncoder:
        hidden_size = 3
