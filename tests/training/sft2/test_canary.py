...
def make_loop(root, cap, *, resume=None, rank=0, distributed=False):
    ...
    if resume is not None:
        ...
        state = load_sft2_loop_state(resume=True, resume_state_path=resume / "training_state.pt",
                                    ...
                                    training_invariants={"test": "cursor", "training_unit": "complete_trajectory_v1"})
    ...
    def save(name, **metadata):
        ...
        torch.save({**metadata, "optimizer": optimizer.state_dict(),
                    "training_invariants": {"test": "cursor", "training_unit": "complete_trajectory_v1"}}, path / "training_state.pt")

    checkpoint = SFT2CheckpointRuntime(
        manager=SimpleNamespace(output_dir=root, save=save), rank=rank,
...
def test_gradient_diagnostic_preserves_rng_and_existing_grads():
    ...
    batch = SimpleNamespace(inputs=BackboneBatch({"input_ids": torch.tensor([[1, 2]])}))
