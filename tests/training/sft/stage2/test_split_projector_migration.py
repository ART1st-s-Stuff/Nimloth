...
def test_dense_fp32_rows_migrate_exactly_to_input_only_k65():
    ...
    install_input_query_rows(
        language, target_query, schema=INPUT_QUERY_PROJECTOR_SCHEMA
    )
    migrate_full_language_rows_to_input_query_rows(
        language,
        source,
    ...
    torch.testing.assert_close(
        language.lm_head.weight[list(source_query)], expected_output, rtol=0, atol=0
    )
    torch.testing.assert_close(
        language.lm_head.weight[target_query[-1]], expected_output.mean(0), rtol=0, atol=0
    )
...
def test_stage2_k64_projector_copies_both_k65_branches(tmp_path):
    ...
    torch.testing.assert_close(split(hidden)[:, :4], shared(hidden[:, :4]))
