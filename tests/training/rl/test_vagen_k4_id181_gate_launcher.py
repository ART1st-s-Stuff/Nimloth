...
RUNNER = ROOT / "experiments/training/rl/run_vagen_k4_id181_gate_phase.sh"
LAUNCHER = ROOT / "experiments/training/rl/launch_vagen_k4_id181_gate_on_hold.sh"
SLURM = ROOT / "experiments/training/rl/id181_k4_single_update_restore_gate.slurm"
VAGEN = ROOT / "external/VAGEN"
CONFIG = VAGEN / "vagen/configs/joint_id181_gate.yaml"
TRAIN = VAGEN / "examples/train/navigation/train_navigation_joint_id181.yaml"
VAL = VAGEN / "examples/train/navigation/val_navigation_joint_id181.yaml"


def test_id181_shells_parse_and_bind_exact_allocation() -> None:
...
def test_id181_embedded_python_compiles() -> None:
...
def test_id181_pins_code_assets_and_two_separate_checkpoint_roots() -> None:
    ...
    assert "ID181_ACTOR_MODEL=${ACTOR_MODEL}" in source
    assert "ID181_PLANNING_CHECKPOINT=${PLANNING_MODEL}" in source
    assert "status --porcelain --untracked-files=all" in source
    assert "PYTHONDONTWRITEBYTECODE=1" in source
    assert "rsync" not in source and "scp " not in source


def test_id181_contract_is_one_update_then_restore_only() -> None:
    source = RUNNER.read_text()
    launcher = LAUNCHER.read_text()
    ...
    assert "ID180 was cancelled before rollout or update" in source
    assert "caught optional JIT warning is not a fatal result" in source


def test_id181_formal_values_and_three_split_data_are_explicit() -> None:
    config = CONFIG.read_text()
...
def test_general_production_gate_remains_closed() -> None:
    ...
    assert "id181_k4_single_update_restore_gate_v1" in gate
