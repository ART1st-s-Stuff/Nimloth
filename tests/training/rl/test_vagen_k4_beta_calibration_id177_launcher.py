...
RUNNER = ROOT / "experiments/training/rl/run_vagen_k4_beta_calibration_id177.sh"
LAUNCHER = ROOT / "experiments/training/rl/launch_vagen_k4_beta_calibration_id177_on_hold.sh"
HOLD = ROOT / "experiments/training/rl/hold_eight_gpu_60m_k4_calibration_id177.slurm"
ENTRYPOINT = ROOT / "external/VAGEN/vagen/k4_beta_calibration.py"


def test_id177_k4_calibration_shells_parse_and_bind_target_allocation() -> None:
    ...
    assert "nimloth-id177-k4-calibration" in hold
    assert "sleep infinity" not in hold
    assert "launch_vagen_k4_beta_calibration_id177_on_hold.sh" in hold
...
def test_id177_k4_calibration_embedded_python_compiles() -> None:
...
def test_id177_k4_calibration_pins_code_data_checkpoint_and_clean_worktrees() -> None:
    ...
    assert "177_calibration_k4mcts_tp8_actionrepair176" in launcher
    assert "nimloth-id177-k4-calibration" in hold
    assert "experiment_id\":177" in source and "'experiment_id':177" in source
    ...
    assert "RUNTIME_ROOT=/tmp/i177-${SLURM_JOB_ID}" in source
    assert "RAY_TMPDIR=${RUNTIME_ROOT}" in source
    worst_case_socket = (
        "/tmp/i177-99999999/ray/session_2026-08-15_00-40-07_"
        "477970_1313372/sockets/plasma_store"
    )
    assert len(worst_case_socket.encode()) <= 107


def test_id177_k4_calibration_values_are_explicit_and_optimizer_free() -> None:
    source = RUNNER.read_text()
    exact = (
