...
- 按人类要求已建立精确old-scene复现对比`3_compare_k8_vs_vit_oldsamescenes_r8_t5_euler50_cfg2`：提交`afb3773`，手动固化旧combined原8 records/order/first5 actions，先用当前epoch2 Qwen+StateProjector对这些旧trajectory prefixes重新编码k8 states，再以同GT images/actions/noise、Euler50、CFG2比较`GT | old lowLR ViT GT/Pred | current k8 GT/Pred`。combined cache+compare job474920在normal/dgx-27运行，W&B `nimloth-recon` ID3。README写入时未quote heredoc导致反引号被shell展开，已修复输出并记录重复错误`E0028`。
