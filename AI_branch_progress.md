...
- world2 smoke`482289`/W&B ID31 `dz48wt5c`虽Slurm exit0，但10个finite train steps后所有val指标NaN，判定INVALID，全部checkpoint禁用。根因是`SafeBatchNorm1d`以`new_empty`创建scratch running stats，momentum更新后online encoder出现37个负running variances；train batch stats掩盖，eval暴露。已改为clone当前running stats、增加标准BN/双forward回归测试和non-finite fail-fast，登记`E0034`。
- 修复commit`1e5208b`的world2 smoke`482446`/W&B ID32 `3qhd3t97`有效：3m23s完成，20 train transitions/10 steps及12 val transitions；val total/latent/DINO/SIGReg/value=`2.228710/0.729252/1.110717/0.478685/0.896231`且全部finite。epoch/best/latest/final所有浮点module tensors finite，BN running variance均非负，checkpoint state epoch1/step10/best2.2287099063。
- 人类已明确批准启动正式SFT2。正式commit`c3801f6`，Slurm`482470`，W&B ID33 `n1knd7lt`：全部59,389/6,054 train/val transitions，强制Qwen+DINO caches，world8，10ep，batch2/GA4 effective64，SIGReg projections1024，48h，完成epoch级resume。初始pending priority，scheduler预计`2026-07-21 09:26`启动；测试2×4和4×2形状预计约12:33，故保留较早的单节点world8。详见`ai_tasks/ai_progress/2026-07-20_dinov2_grid_sft2.md`。
...
- 正式DINOv2 SFT2已健康启动：W&B ID30 `c2lkxd63`，作业`480807`，`dgx-21,dgx-28,dgx-29,dgx-54`各2卡组成world8，总64CPU/384G/48h。runtime24m41s时CSV到step277且DINO/总/各子loss finite；8卡99–100%、约61.6–64.3GiB；W&B heartbeat正常；periodic latest checkpoint可读（epoch1/step263/micro1052）。日志无traceback/OOM/NaN/NCCL error/timeout；mixed IB/RoCE与DDP grad-stride warning暂为非致命性能风险。最终训练代码commit `5e7fc3f`。实时记录：`ai_tasks/ai_progress/2026-07-19_dinov2_sft2.md`。
- 人类要求评估达到SFT1级速度并选择不打断当前作业。SFT1仅50步/27m12s，当前14,560步，保持目标不可能到半小时。DINO cache经`480899`验证只占step约1–2%。首轮profile`480933`发现trainer缺少pre-epoch1 `model.train()`，GC未激活；未来代码已修复。修正world8 profile`480952`：online+GC5.209s/peak26.42GiB，cached+GC5.116s/25.85GiB，cached+noGC4.500s/56.15GiB，均50步finite；cache约省1.8%，noGC再省12.0%。新增fail-closed cached-noGC config。剩余64%瓶颈为backward+跨节点每micro同步；人类选择停止dynamic-DDP/no_sync验证。
- **人类阶段纠正（最高优先级）**：DINO对齐应作用在SFT1，而不是SFT2。已立即取消Slurm`480807`（runtime2h42m39s，CSV epoch2/step1900，latest step1743，无final/done），释放8卡；W&B`c2lkxd63`关闭并标记cancelled/invalid_stage。旧SFT2 DINO checkpoint/曲线仅作诊断，禁止resume/正式引用；已登记`E0029_dino_alignment_belongs_in_sft1.md`。现有SFT2 DINO实现不再获批，后续须先确认SFT1接口再迁移/清理。
