...
- retry2 launcher已分离allocation worker eligibility与Navigation-head eligibility：仍允许23/37作模型worker，但head额外排除13/23/32/37/51，并将选中的合格head放在cluster node/IP identity首位；无合格head即fail closed。common与runner cleanup均改为只按继承的runtime-root environment认领进程，避免cmdline误杀协调`srun`。parent=`3a90e9db`、服务器launcher`11 passed`，独立Opus review`APPROVE`无P0/P1。尚未提交retry2。
