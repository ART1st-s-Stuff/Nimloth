...
- 人类批准新SFT2使用8×1024 hidden总容量、residual、T4。实现`1541c50`+YAML及`cbd879f`配置映射，server focused63 passed：每个external State8192 reshape8×1024，T4形成32 tokens；6-layer hidden1024/16×64 attention/MLP4096，总125,962,312 params(~0.469GiB FP32)，无8192→1024单向量瓶颈；fixed4 slots采用right-aligned real history+zero left PAD+显式valid mask，block-causal attention允许同frame8 tokens互看且完全屏蔽PAD；action PAD为mask后的zero embedding而非真实action；delta head zero-init，输出`s_t+delta`；teacher-forced T4 contexts仅从同record连续steps构造，recursive rollout使用完全相同warmup mask。SFT2 CLI/config/checkpoint invariants/DDP forward+dummy均支持并强制exact architecture/full-trajectory batching。`5bb5a4e`额外把identity MSE/cos及gain写入train/val/W&B/CSV，防止raw MSE再次掩盖低于baseline。latest commit server复测因superpod SSH `Connection closed UNKNOWN65535`暂未执行；昂贵smoke/formal尚未提交，等待按实验启动规则向人类确认资源/init/output/resume。
