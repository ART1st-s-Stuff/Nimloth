...
- 已修复 retry9 暴露的 Ray client 依赖问题：`rollouts_greedy_parallel.slurm` 移除未使用的 `--ray-client-server-port` 和 `RAY_CLIENT_SERVER_PORT`，保留 head/node/object/dashboard/metrics/worker port 隔离。已本地 `bash -n`，准备新输出目录继续重试。
