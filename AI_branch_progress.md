...
- 当前optimizer-free production guided rollout已形成完整但未统一验证的candidate：immutable snapshot transport、独立1CPU/0GPU frozen-Q actor、parameter-dtype/thread门禁、manager整batch pin/CAS lifecycle、restart-stable sample/repeat identity、coordinator预分配per-turn key，以及agent-loop capture→CPU score→draw→trace/behavior→`step_guided`→guided ledger/DataProto均已接通；失败路径unpin，disabled constructor兼容。standalone guided入口不设实验默认值，trainer仍fail closed。实现前RED为`3 failed,1 skipped`；实现后本地`py_compile`和diff-check通过，服务器统一门禁尚未运行。直接rsync源码被确认违反服务器Git同步规则并登记`E0097`，后续仅测试明确candidate SHA的干净worktree。
