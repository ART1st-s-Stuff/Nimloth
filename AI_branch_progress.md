...
- 新增 compressed vision-token offline eval 基础设施：`representation.type=compressed_vision_tokens` 可加载 compressor checkpoint + token predictor checkpoint，对 current/next image 直接提取 Qwen vision tokens，再压缩后计算 one-step 与 multistep token metrics；新增 compressed 1-step / rollout4 eval YAML。服务器 worktree 已同步到 `bb16479`；server smoke `py_compile`、eval YAML load、临时 compressor/predictor checkpoint validate/load 通过，输出 `compressed_vision_eval_smoke=ok`。尚未启动真实训练，尚未接入 RCDM direct-concat visualization。
