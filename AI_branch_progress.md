...
- 人类批准先跑source20的heldout Base60+Common Sense60共120条，并对每条完整保存图片/CoT/16-slot state和100次MCTS。新增ID189 phase/config/runner/1×8 Slurm及120-row/3-batch/browser严格finalizer；VAGEN=`14d862e8`、runtime Parent=`992554c2`，本地定向VAGEN`4 passed`、Parent`7 passed`，服务器同为`4 passed`和`7 passed`。单条20-turn browser实测408MiB，预计本次约49GiB；提交时NFS剩余325GiB。attempt0 Job`525905`在`preempt/dgx-42`分配8×H800后于`00:01:15`以`FAILED 1:0`结束：allocation、动态render、Ray `[8]`、runtime import/env和cleanup通过，但在formal output前被clean-tree gate拒绝。根因为AI在production worktree运行pytest时未禁用bytecode，生成`external/le-wm/__pycache__/module.cpython-312.pyc`。无RUN_OUT/W&B/model load/rollout/update/checkpoint；attempt0不可resume/reuse，已登记`E0136`。retry1使用从未运行test/import的fresh production worktree`id189-bc120-r1-37489767`、fresh `_retry1` output和W&B `-r1`身份；Job`527282`在`preempt/dgx-47`于`00:01:12`以`FAILED 1:0`结束。clean/allocation/render/Ray/source checkpoint/assets/120-row config/cleanup均过，失败是复制自300-row原评估的唯一ID断言仍要求300，而真实过滤结果为120；无W&B/model load/rollout/update/checkpoint，partial output已写`progress.md`，登记`E0137`。人类随后要求使用normal现有4×2资源；已修复120-ID门禁，VAGEN改为ID189 normal4×2且保持TP8/DP1、actor DP8。fresh retry2 Job`527287`于`2026-08-22T15:14:41+08:00`在`dgx-[26,28-29,31]`立即启动；动态render、Ray`[2,2,2,2]`、source20 checkpoint、120-row manifest、Base/Common prewarm、DP8 actor load及分布式TP8 vLLM startup已通过，当前RUNNING等待正式rollout。
