...
- Rollout/converter、SFT1全部gate通过。SFT2正式grid22/max_pixels100352+budget12最长19-prefix完成8步，峰值51.12/52.17GiB，cache非瓶颈；epoch resume通过。Partial step5同epochskip5/8正常；counter RNG后resume step6 pre-update metrics已与uninterrupted完全一致，但optimizer后有极小分叉（Qwen max3.58e-7），定位DDP新进程bucket rebuild。所有model/aux DDP改为构造时static_graph，待最后exact gate。
