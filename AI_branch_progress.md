...
- 人类要求同时做严格WM验证与去decoder瓶颈公平CFM。实现`2ccf47c`/server21 tests：全val consecutive transitions teacher-forced T1对每个actual state评全部8 actions，并与identity/train-mean/7 wrong actions比较MSE/cos、correct rank/best、action spread/per-action；CFM新增独立`condition_token_count`，将flat Projected8192视为8×1024→8×256，匹配Query CFM的8×256 cross-attention interface，同时source cache仍是one Projected State。WM validator479922与fair-CFM smoke479923同时提交（均PENDING Priority）；afterok formal479924用与Query ID19相同seed/data/body/budget/55,680steps，afterok diverse40 strict-T1 eval479925。
