...
Online迁移现推进至commit`420853e`、VAGEN`896aac1`、VERL`dbca62d9`：新增local/service staged manager（BPE-safe完整thought、框架注入k queries/action-start、8-action白名单采样、source XML env与Nimloth replay分离），完整episode row携带latent位置/action/WM transition mask。Actor worker已接入StateProjector+WM predictor next-latent MSE、独立DDP optimizer/LR及严格`nimloth_wm_aux.pt` sidecar resume；4.55 critic和zero-warmup patch由worker external-lib自动安装。相关server targeted suite为`21 passed, 2 warnings`。尚未GPU验证WM update/checkpoint，也尚未真实env service rollout；不得描述为在线RL成功。
