...
- 获批的正式world8 ID16（commit`801de41`、Slurm`481482`、W&B`4k53mvne`）在1m14s、step1前失败：PEFT按后缀匹配language LoRA targets时误装了96个visual LoRA trainables，违反vision freeze并触发DDP unused-parameter检查。单GPUcheckpoint审计确认252个language LoRA-B和query adapter已更新，而96个visual LoRA-B全为零。已改为PEFT后重新执行path freeze并在发现任何visual trainable时fail-fast，不使用`find_unused_parameters`掩盖；登记`E0033`，正式retry前需multi-GPU smoke。详见`ai_tasks/ai_progress/2026-07-19_dinov2_grid_sft1.md`。
