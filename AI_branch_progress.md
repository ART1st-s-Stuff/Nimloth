...
- Rollout/converter、SFT1全部gate通过。SFT2正式grid22/max_pixels100352+budget12最长19-prefix压力完成8步，峰值51.12/52.17GiB，cache非瓶颈；epoch resume通过。`2ded494` partial step5恢复能同epochskip5/8并完成step6-8，但与uninterrupted metrics不完全相同，定位为SIGReg等RNG未续接；已加入counter-based micro RNG与seed/world/gradaccum/loader/RNG-version checkpoint invariants，待重跑并逐tensor比较。
