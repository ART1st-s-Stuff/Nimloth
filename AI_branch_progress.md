...
- 人类要求用新full8192 SFT2 epoch2重走recon训练+可视化。锁定source `sft2/27_state8192.../epoch_002`（step3310/epoch complete、LLM+vision LoRA、vision EMA、val WM MSE.00305007），而非epoch4。commit`b0a386a`使query cache canonical加载PEFT base+adapter并materialize全部vision EMA shadow，query-only跳过新8192 projector/WM，fingerprint包含adapter/EMA；server23 tests。smoke479373 `COMPLETED00:01:09` exit0：EMA322 keys exact，train77/val12、[8,2048] finite，FP train255a824603e3a295/val2f3655ff53f56c06；相对旧state cosine约.392/.402，确认未误用旧/base表示。full2GPU cache479375 RUNNING dgx-54；afterok direct CFM479376（同30ep/55,680/drop.15/Qwen-init协议）和diverse40 Qwen compare479377已dependency提交。
