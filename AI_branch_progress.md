...
- attempt477513在dgx-14+21 trainer/dgx-24 env机械完成2条k8 trajectories和finite step1（WM0.00232395/value0.143456/actor-4.17e-7/entropy1.79462/total0.127833），W&B in-process step1。但post-gate正确判为无效：fresh LLM LoRA suffix同时命中visual MLP，96个visual LoRA-B全部变化，违反该次运行的`vision_tune=freeze`；frozen StateProjector从BF16加载成FP32。ID9约22GiB保留，禁止resume/reuse/质量解释。修复`465e8bd`按显式mode控制Vision LoRA并保持StateProjector checkpoint dtype，server26 tests passed。人类纠正：Vision tune是可配置参数，不能把本次freeze泛化成永久LoRA=0约束；登记E0030。launcher现显式支持`VISION_TUNE=freeze|lora|full`及mode-aware gate，本次retry获批选择freeze。新retry使用新W&B ID/output和重新snapshot当时latest。详见`ai_tasks/ai_progress/2026-07-16_fsdp_dynamic_rollout.md`。
