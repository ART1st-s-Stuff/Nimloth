...
- ID185 retry3仅在`pad_dataproto_to_divisor`已知尾部padding rows上确定性改写synthetic unique `uid/group_idx/rollout_sample_id`，原始real rows不变且真实duplicate gate保留；synthetic UID不属于original set，现有no-concat filter会丢弃dummy outputs。VAGEN=`b2e984a9`、Parent=`cdf5298a`；remote VAGEN定向`17 passed`、parent launcher`19 passed`，真实DataProto `20→24/pad4`门禁证明24个sample/repeat identities唯一且原20不变。fresh worktree clean，新output `_retry3`与W&B `nimloth-id185-k4-full-eval-test300-retry3`不存在。Job`524473`已提交normal exact4×2/5h，当前`PENDING(Priority)`、暂无start estimate。
