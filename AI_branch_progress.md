...
- 随后确认eval479795存在严重train/inference protocol mismatch，故上述WM-predicted视觉不能用于判断learned WM质量：SFT2 loss始终经`wm_predictor.forward→predict_next_emb`以`T=1`训练；checkpoint config却`history_size=4`，eval调用`rollout_states`以T=4推理，使用从未训练的position1–3，并把缺失历史填成重复state+有效action0(move_forward)，且无padding mask。h1即差且h1..5不单调与此吻合。登记E0047。有效重评必须分teacher-forced T1和递归`predict_next_emb` T1；只有真实history4训练后才能评T4 rollout。Projected CFM ratio1.0028仍是独立decoder confound。
