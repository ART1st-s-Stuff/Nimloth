...
- Compressed predictor rollout4 smoke job `466245` 已按人类确认提交，输出 `/project/peilab/atst/nimloth/outputs/experiments/representation_ablation/2026-07-06/compressed_vision_rollout4_smoke_852b7a5_466245`，在训练前失败：默认 `.venv` 的 transformers 4.49 无法加载 adapter-backed SFT2 checkpoint，报 `GenerationConfig.from_model_config` 中 dict 缺 `.to_dict()`。输出 README 与 `outputs/experiments/representation_ablation/progress.md` 已记录失败；无训练 step、无 checkpoint。已修复 launcher 默认使用 `/project/peilab/atst/nimloth/.venv-vagen-main`，服务器 worktree 同步到 `0c22cce`，`bash -n` 与 `.venv-vagen-main` import/config smoke 通过。下一步可 retry smoke。RCDM direct-concat visualization 尚未接入。
