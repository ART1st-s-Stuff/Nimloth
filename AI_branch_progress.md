...
- ID185 retry4新增每batch append-only atomic journal：每个完整batch独立写JSONL后fsync/rename，再写row/identity SHA256 complete marker，拒绝覆盖；八个连续batch、300个全局唯一identity、各source60及全hash复核后才发布journal `complete.json`和正式validation。partial journal可在失败后诊断success，但明确禁止作为完整结果或拼接resume。最终VAGEN=`67539996`、Parent=`7640079c`；remote VAGEN`47 passed, 32 subtests`、parent`19 passed`，fresh worktree clean且新output `_retry4`/W&B不存在。Job`524485`已提交normal exact4×2/5h，当前`PENDING(Priority)`、暂无start estimate。
