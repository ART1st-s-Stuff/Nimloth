...
- corrected parent`386ca4ed`显式传播`/cm/shared/apps/slurm/current/bin`和固定`SLURM_CONF`，server direct probe返回`slurm 23.02.6`，launcher`10 passed`；cleanup会重试双节点owned-process audit并保留既有primary failure status。fresh retry worktree五层clean。phase2仍只在phase1 step5、validator、W&B完成后提交。
