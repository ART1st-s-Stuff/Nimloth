...
- Rollout 与 converter gates 已通过。SFT1 vocab 修复 `1b31dcc` 后 epoch1 2-GPU forward/backward/optimizer/val/checkpoint 通过（step1/val_loss5.03155）。Resume attempt 虽跑到 step2，但 raw `model.load_state_dict(strict=False)` 对 PEFT checkpoint 出现700 unexpected keys，故 epoch2/final 已隔离为 invalid。已切换 `set_peft_model_state_dict`，并对保存的702个 LoRA+embedding tensors 做 key/shape/value 完全一致验证后才允许 resume。
