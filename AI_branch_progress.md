...
- ID14 W&B=`cfm6lpiz`/job478603验证free-port、world8、env health和multimodal image gate均通过，但错误地让AI2-THOR共享rank7 H100；`thor-CloudRendering`持续99% SM，rank7阻塞GPU runtime，其他7 ranks以0% SM等待。11:18取消，0 trajectory/step，终态FAILED/NON-RESUMABLE。ID15用8个dedicated trainer GPUs+独立dgx37 env仍复现首个真实multimodal policy forward全trainer GPU0%、process poll/futex sleep的collective stall，证明sharing无效但不是已证根因；7:36取消，0 trajectory/step，W&B=`wnu71oeo` terminal。下一diagnostic run增加SIGUSR1 all-thread stack dump及rank-local forward enter/model-exit/token-selected/input-shape日志；无stack/stage证据前禁止再猜根因。
