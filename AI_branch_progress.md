...
- 人类确认统一rollout browser必须覆盖K4 joint、SFT1/SFT2、greedy/reference及所有真实环境rollout evaluation，并要求底层永久保存完整action评分、完整MCTS candidates、raw response/CoT和全部真实图片；UI可折叠但不得截断底层。设计已固化于`ai_tasks/evaluation_rollout_browser_design.md`：采用versioned capability schema、VAGEN no-concat与现有`RolloutTrajectory`双adapter、逐batch原子提交、全局complete/hash门禁，以及静态manifest选择器+按需单轨迹页面；无真实rollout的teacher-forced SFT loss validation明确不冒充rollout browser。当前仅完成设计，未开始实现。
