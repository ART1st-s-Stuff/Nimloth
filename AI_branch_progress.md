...
- 人类要求立即使用normal碎片。nonuniform DDP排障：476443 one-task/GPU cgroup导致NCCL invalid device ordinal；476453/476464 node launcher但ProcessGroup首barrier device推断卡住；`5e2b454`为`init_process_group`显式传cuda device/server26 tests，登记E0033。476457在一次stale pending状态后已转RUNNING18s却被误取消，重复E0029并补充atomic state check。最终job476479在dgx32:2+dgx54:2、world4/GA8（有效batch严格32）健康运行，W&B`z3c0w63v`，4-rank init+至少88 finite steps；median约9.43s/micro、GPU约63GiB。live dgx32↔dgx54 benchmark证明NET/IB GDRDMA约33.5GiB/s；额外world2 IB handoff476486虽健康16 steps但median11.12s且每epochsteps翻倍，作为诊断停止。正式数据base batches46,569，world4每epoch11,643 micro-batches；CSV每行是一次optimizer step而非micro-step，GA8下约1,456 optimizer steps/epoch。latest@step128明确记录`micro_step_in_epoch=1024`，因此9.43s/optimizer step对应约1.18s/micro；正确估算约3.8h/epoch、7.6h两epoch，另加checkpoint/validation开销（早先误把optimizer interval乘micro count而写成61h，已纠正）。latest step128/micro1024完整，维度与world4/GA8/dynamics2048 invariants通过。已提交同节点homogeneous world4/IB resume476507，并以`afterany:476479`阻止两个writer重叠；若476479的8h allocation未完成2ep则从20min latest、同W&B继续。
