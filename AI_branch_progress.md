...
- k8 runtime提交`9b1d652`/`6fe766a`：k-query prompt/token map、canonical policy/PPO/latent prompt、k-block hidden extraction、checkpoint-width StateProjector/ValueHead load、严格独立heldout collector/fixed seed、protocol/resume metadata、explicit resume checkpoint、前5轮0成功early-stop。新增live checkpoint稳定snapshot+LoRA merge prep、2 trainer fragments+dgx-37 env原子launcher及smoke/pilot configs。server36 tests passed；真实source组件shape和SFT1 processor k8 query IDs151665..151672 gate通过。尚未提交GPU prep/smoke。详见`ai_tasks/ai_progress/2026-07-16_fsdp_dynamic_rollout.md`。
