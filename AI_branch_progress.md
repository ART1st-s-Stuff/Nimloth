...
- 人类要求评估达到SFT1级速度并选择不打断当前作业。核实SFT1仅613记录/5 epochs/50 optimizer steps、总27m12s；当前SFT2总14,560步，单步4.57s已快于SFT1约17–18s，保持目标时不可能达到半小时walltime。已为后续run实现fail-closed DINO CLS float32 sidecar cache（teacher/processor/父cache/图片/shape指纹、partial resume、checkpoint invariant），相关suite81 passed；尚未获批运行GPU cache build/A-B。packed-forward因已证实不等价继续禁用。
