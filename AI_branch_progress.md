...
- 已更新 SFT1 rollout 脚本以兼容当前 VAGEN：`rollouts_greedy_parallel.slurm` 会为每个 shard 生成 parquet dataset，检测 `vagen.trainer.main_ppo` 后走新入口，设置 `rollout_manager.use_service=True` 与单个 active env server URL；保留旧 `vagen.main_ppo` fallback。`submit_env_external_4gpu.sh`、`submit_rollouts_greedy.sh`、`env_external_4gpu.slurm`、`rollouts_greedy_parallel.slurm` 现在支持通过 `REPO=/project/.../.worktree/exp-latent-repr-ablation` 提交，避免强制使用主 worktree。已做本地 `bash -n`；远程用 `python -m vagen.trainer.main_ppo --cfg job` 验证新增 Hydra overrides 可解析。尚未提交新 rollout retry。
