...
- 按人类指示改用 `dgx-44` 3 张卡恢复：Nimloth `4614934` 让 SFT1 external env 支持 `ENV_SERVICE_COUNT` / `ENV_URL_HOST`；已提交 env job `468509` 到 `dgx-44`/`preempt`，`ENV_SERVICE_COUNT=3`、`ENV_URL_HOST=dgx-44`、ports `20530-20532`，env 已 ready；随后提交 rollout array `468513_[0-3%2]`。当前 `468513_0` on `dgx-46` 和 `468513_1` on `dgx-28` 运行中，已启动 Ray、进入 `vagen.trainer.main_ppo` 并通过 config validation/dataset load；`468513_2/3` 因 array throttle pending。暂未生成 JSONL。
