...
- 人类同意后已改用 normal `dgx-18` 4GPU 再次恢复 env：提交 env job `468759`，`ENV_SERVICE_COUNT=4`、`ENV_URL_HOST=dgx-18`、`PORT_BASE=20630`，输出目录不变。当前 `468759` 为 `PENDING (Priority)`，Slurm 预估 `StartTime=2026-07-09T23:21:38`；本轮 rollout 尚未提交，需等 `468759` 写出新的 `external_env_4gpu/ready` 后再提交，避免误用旧 dgx-44 URLs。
