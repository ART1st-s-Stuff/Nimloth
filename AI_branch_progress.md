...
- 人类要求可视化加入predicted State。WM predictor输出projected8192而非8×2048 Query，不能输入direct Query CFM；为避免把decoder failure误当dynamics failure，实现`3882d31`/server11 tests：从validated Query cache离线应用exact StateProjector16384→8192→8192生成aligned projected cache；用同一frozen Qwen ViT CFM训练Query与Projected→16×512 adapters；从actual projected step0对recorded actions自回归rollout horizon1..5。最终五列固定为`GT|Qwen ViT|Query actual|Projected actual|WM predicted`，同时报raw State/token/image及horizon metrics。full projected cache+real-shape smoke479779 RUNNING dgx-29；afterok adapter479780、diverse40 eval479781。
