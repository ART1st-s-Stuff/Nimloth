...
- tiny完整K4 actor/planning单update与fresh actor checkpoint restore测试已通过；restore字段名误写已修复并登记`E0116`，SIGReg按global micro-group count归一，gradient accumulation不再放大`0.1`权重。服务器最终`169 passed, 120 subtests`；parent=`e6dd1d28`、VAGEN=`6cd96ef`、VERL=`494f2644`，三层clean。通用production gate仍关闭；未启动GPU optimizer、canary或长训练。
