...
- retry前新增有界Ray内部日志持久化：cleanup杀进程前/后分别在四节点执行共享script，每节点独立manifest，单文件2MiB/节点总16MiB；任一srun/文件/manifest失败会写marker并保留local Ray root。首轮review发现stdin多task设计和缺manifest门禁后拒绝，已改为shared script file并逐个核验4 manifests；parent=`3c18d9fd`，服务器launcher`11 passed`，复审`APPROVE`无P0/P1。尚未重提。
