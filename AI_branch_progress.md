...
- corrected parent`17c92ce7`将bootstrap W&B local目录移到`${RAY_LOG_ROOT}/wandb`并静态拒绝旧路径；正式`RUN_OUT`仍只能由phase runner首次创建。fresh retry3 Job`520711`于16:52提交2×4；当前允许节点仅dgx-52有≥4空卡、dgx-18仅2卡，任务为`PENDING(Resources)`、scheduler估计19:28，未占GPU或创建正式output/W&B。phase2仍只在phase1 step5、validator、W&B完成后提交。
