...
- 人类批准先跑source20的heldout Base60+Common Sense60共120条，并对每条完整保存图片/CoT/16-slot state和100次MCTS。新增ID189 phase/config/runner/1×8 Slurm及120-row/3-batch/browser严格finalizer；VAGEN=`14d862e8`、runtime Parent=`992554c2`，本地定向VAGEN`4 passed`、Parent`7 passed`，服务器同为`4 passed`和`7 passed`。单条20-turn browser实测408MiB，预计本次约49GiB；提交时NFS剩余325GiB。attempt0 Job`525905`在`preempt/dgx-42`分配8×H800后于`00:01:15`以`FAILED 1:0`结束：allocation、动态render、Ray `[8]`、runtime import/env和cleanup通过，但在formal output前被clean-tree gate拒绝。根因为AI在production worktree运行pytest时未禁用bytecode，生成`external/le-wm/__pycache__/module.cpython-312.pyc`。无RUN_OUT/W&B/model load/rollout/update/checkpoint；attempt0不可resume/reuse，已登记`E0136`，retry必须fresh clean worktree及fresh output/W&B身份。
