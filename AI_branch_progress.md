...
- 人类随后要求必须支持跨节点凑卡。再次实时确认`520517`仍PENDING后已取消；终态`CANCELLED by 3738`、elapsed0、无node/allocation/Python/Ray/GPU/output/W&B，phase2未提交。原single-node launch合同失效；重提前必须真实接通multi-node Ray/FSDP与TP8-over-TCP并做目标拓扑门禁，不能用未验证的碎片组合替代。
