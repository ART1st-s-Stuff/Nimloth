...
- 按人类要求调整 predictor capacity diagnostic 方案以贴合 LeWM：新增 attention/Perceiver-style `AttentionTokenCompressor`，将 raw Qwen vision tokens `(K=81,D=2048)` 压缩到 `K=16,D=512`；训练 compressor+predictor，loss 为 autoregressive predictor MSE + `lambda_sigreg * SIGReg(compressed tokens)`，不使用 reconstruction loss，RCDM 仍作为后验诊断工具。新增 compressed 1-step / rollout4 training YAML；本地 `py_compile` 与 YAML load 通过。尚未启动真实训练，尚未接入 RCDM direct-concat visualization。
