...
- Rollout preflight 已修复 startup、module/config/parquet/service API、val-only batch/ref/train-loader invariants（`82770ca`..`3708b14`）。Attempt 7 已让两个 FSDP workers 完整加载 source 4 shards并进入 vLLM 0.11 engine，但 vLLM sleep CuMemAllocator 拒绝 `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`；退出后 GPU 清零，仍无 JSONL。已在 rollout env 局部 unset，并改用显式 local Ray address/独立 dashboard port。
