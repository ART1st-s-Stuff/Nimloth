...
- preprojection对照实现于branch`exp/k8-preprojection-recon` commit`5d92e7e`：cache可保存Qwen final-norm的`[8,3584]` query hidden；paired token-conditioned patch decoders共享shape-compatible body初始化、同sample/target，分别输入projected`[1,1024]`和preprojection`[8,3584]`，独立best，报告correct/wrong/PSNR/output delta和并排图；server18 tests passed。
- smoke job475045 `COMPLETED00:01:07`：cache train77/val16且alignment通过，paired step10/checkpoint/contact/W&B`90im9e9r`均finite。resume gate475048 `COMPLETED00:00:32`，cache hit、step10→12并复用同W&B ID。短smoke ratio≈1不作质量解释。原formal normal8 jobs475049/475050因预计2026-07-16才开始而执行前取消；normal2 cache475052曾在dgx-29运行00:05:10并写2×512-row partial shards，收到人类“dgx-22有4GPU”后取消，dependent475053执行前取消。有效replacement为preempt/dgx-22 4GPU cache475078（nproc4/64CPU/128G/4h，启动会原子清理partial shards）和afterok normal train475079（b16/18,560≈5ep，W&B ID10）。提交时Slurm确认dgx-22仅alloc4/8 GPU；调度前其余资源被占满，当前节点alloc8/8，475078 `PENDING(Priority)`。若人类的4GPU在已有hold allocation内，需job ID才能安全attach。
