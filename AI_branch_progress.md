...
- 2026-07-02 已启动 full-scale RCDM 一轮实验（按用户确认）：
  ...
  - 已创建分支/worktree `recon-compare-qwen` / `../nimloth-recon-compare-qwen`，实现 Qwen visual encoder 对照基线：`src/nimloth/rcdm/qwen_vision_cache.py`、`src/nimloth/training/reconstruction/rcdm_qwen_vision.py`、`src/nimloth/eval/rcdm_qwen_vision_reconstruction.py`。该基线冻结 Qwen visual encoder，缓存 mean-pooled image features，只训练 RCDM UNet，用于和当前 SFT2 latent-vector RCDM 对比。提交：`30cc804 feat(rcdm): add qwen vision reconstruction baseline` 与 `97781ca fix(rcdm): handle single qwen vision eval sample`，进度记录提交 `abc4c00 docs: record qwen vision baseline smoke`。验证：服务器 venv `pytest -q tests/test_rcdm_adapter.py` 通过；Slurm smoke train job `465218` 完成，tiny RCDM max_steps=1 可 build qwen vision cache/train/val/checkpoint；eval smoke job `465220` 完成，可生成 DDIM-250 与 DDPM-1000 strips。失败的 eval smoke `465219` 暴露 `num_per_split=1` 除零 bug，已在 `97781ca` 修复。
  - 人类确认后已启动 full Qwen visual 对照实验：cache build job `465244`（1 GPU, preempt, dgx-55）正在运行；训练/可视化 job `465245` 已以 `afterok:465244` dependency 提交（8 GPU）。输出：`/project/peilab/atst/nimloth/outputs/experiments/training/reconstruction/2026-07-04/rcdm_qwen_vision_full_current_128px_cache_8g`；cache：`/project/peilab/atst/nimloth/outputs/experiments/training/reconstruction/cache/rcdm_qwen_vision_current_trainval_f16_gzip_abc4c00`。full 计划：128px RCDM，5 epochs，trainable RCDM UNet only；condition 为 frozen Qwen visual encoder current-image mean-pooled feature；每个 epoch 后上传 8 train + 8 val 的 DDIM-250/DDPM-1000 strips 到 W&B。
