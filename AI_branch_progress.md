...
- K4 actor现已调用统一planning update：future images随turn compiler持久化，terminal row额外保存真实terminal observation image；各DP rank用固定DINO identity直接编码in-memory PIL target，不落临时图像。actor/planning optimizer完整后source step才递增。VAGEN=`6c742b7`；K4 full snapshot/checkpoint export/restore仍主动NotImplemented，production继续fail closed。parent DINO API待提交，superpod torch回归仍被SSH故障阻塞。
