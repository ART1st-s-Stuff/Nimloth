...
- 人类要求评估达到SFT1级速度并选择不打断当前作业。SFT1仅50步/27m12s，当前14,560步，保持目标不可能到半小时。DINO CLS cache经job`480899`验证bitwise正确且teacher-only快199–261x，但只占step约1–2%。world8 profile `480933`进一步测得epoch1模式backward+DDP64%、next Qwen20%、current Qwen14%、DINO1.9%；同时发现trainer首轮前未`model.train()`，导致Transformers默认eval、GC在epoch1/profile未激活，epoch1 val后才切train。已修复未来代码为首轮前显式train并记录GC状态（suite82 passed）；480933的no-GC对照无效，需重跑。正式job480807仍用旧commit，epoch2性能/显存切换必须监控。packed-forward因不等价继续禁用。
