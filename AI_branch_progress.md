...
- 2026-07-13 SFT1 cache完成；5-epoch train473384在dgx-32健康运行，epoch1 val0.178709、epoch2 val0.092776，检查时epoch3 step30。人类指出与旧SFT1不一致后修正结论（E0019）：问题是mask/推理/eval协议不一致，不应单称evaluator bug。Mask后query input embeddings/hidden仍从后续action CE间接学习，但不会学习自主生成query IDs；若保持旧VAGEN自由生成协议，SFT1应unmask query labels；只有框架确定性注入query block时才应mask并使用注入式eval。等待人类决定正式协议，当前masked run不应直接作为最终格式模型。
