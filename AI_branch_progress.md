...
- K4 production compiler首个CPU里程碑完成：ledger v3的Frozen-V使用`softmax(LLM+beta*MCTS)`对behavior-time direct Q求期望；batch持久化MCTS guidance与direct Q为两个tensor，且strict复核embedded K4 search/root/candidate/identity。legacy ID171 ledger v2路径保持；Ray ledger boundary新增K4 schema。VAGEN=`ce6de0c`，production gate仍关闭，服务器torch回归待跑。
