...
- fresh runtime确认retry1证据保留、retry2 output与W&B identity absent。Phase1 retry2提交客户端返回Slurm`Unexpected message received`，但controller实际原子接受Job`521163`；`scontrol`确认normal exact4×2/64CPU/256GiB/5h、只排除13/32/51、命令指向fresh runtime，当前`PENDING(Priority)`、预计00:33、候选18/21/23/27。phase2未提交。
