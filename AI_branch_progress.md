...
- 人类指出stock `algorithm.adv_estimator: grpo`错误描述实际Frozen-V GAE。所有joint gate config现改名`joint_frozen_v_gae`；driver和Ray trainer双层拒绝stock estimator，custom batch/actor仍唯一生成并消费`joint_advantages`。VAGEN=`3216d4e`，服务器定向回归`25 passed,32 subtests`；登记`E0124`。这只修正算法身份，不授权或自动提交retry。
