...
- ID14 W&B=`cfm6lpiz`错误共享rank7/env GPU，ID15 W&B=`wnu71oeo`改独立dgx37 env；两者都在首条`rl_ep`后全trainer GPU0%且无trajectory，分别11:18/7:36取消、0 steps。ID16 diagnostic W&B=`epzlgdud`/job478689用SIGUSR1 stack和forward stage证实：没有任何rank进入policy input/forward；rank0阻塞VAGEN `create_environments_batch` HTTP response，ranks1–7正确等待Gloo broadcast。600s后明确`ReadTimeout`并discard episode0；11:36取消。故先前“FSDP deadlock”归因已撤销。登记E0037：HTTP health/Unity99%不等于create+reset健康。下一smoke改用ID13已到达真实policy turn的dgx51专用env，trainer为normal4+2+2。
