...
- 新增strict `k4_world_model_training`合同：无joint默认值，显式固定1--4步窗口、ID74 DINO identity/grid4、SIGReg17/1024、三个loss weight、统一AdamW projector/predictor/ValueHead LR groups、betas/eps/WD/clip和snapshot transport root；K4 policy必须同步enable且checkpoint root一致。VAGEN=`996933a`，production gate仍关闭。superpod SSH持续`Connection closed by UNKNOWN port 65535`，torch/server回归尚未完成，禁止声称通过。
