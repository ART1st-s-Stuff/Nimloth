...
- K4 WM compiler/loss module继续完成：每个turn携带最长4步future hidden/action prefix，final successor来自真实terminal CoT K16；所有valid depth1--4 prefix windows等权。behavior-snapshot projector target detach，online source/projected next接收梯度；同模块计算state MSE、DINO-grid MSE、selected-action Huber和global SIGReg，并由一个AdamW三个named groups更新。VAGEN=`a6a0fcb`。DINO image target生成、actor调用、full planner snapshot/checkpoint仍未接；superpod SSH仍不可用，torch回归待验证。
