...
- 人类允许使用更多资源跑 train shards 后，retry15 train array `466453` 已提交：`--array=0-2%3`，每个 task 请求 2 GPU / 56 CPU / 180G，节点 `dgx-27`、`dgx-29`、`dgx-24`，复用 env job `466322`。结果为部分失败：`validation/train/shard_001_180/0.jsonl`（540 rows, 11,219,566 bytes）和 `validation/train/shard_361_540/0.jsonl`（540 rows, 10,326,434 bytes）已完成；`shard_181_360`、`shard_541_720`、`shard_721_900`、`shard_901_1080` 缺失。Slurm accounting：env job `466322` PREEMPTED；`466453_[0-2]` FAILED。失败原因为 env service 在 `dgx-05` 不可用/被 preempt，client 对 ports `19530-19532` 出现 `RemoteDisconnected`、connection refused 或 read timeout；不是 retry14 的 vLLM 多图问题。恢复方式：新开 env job，重跑缺失 train shards，保留已完成 JSONL。恢复已启动：先提交 normal env `466592` 后因 priority pending 取消，改用 preempt env job `466593` on `dgx-38`，ports `19630-19633` ready；因 `10.23.1.53` 从 login/其他节点不可达，已把 env URLs 改成 `http://dgx-38:19630-19633` 并验证 `/health` OK。缺失 shard rerun：normal array `466596_0` 曾在 `dgx-14` 跑 task0；pending 的 `466596_1/2` 已取消，改用 preempt array `466599_{1,2}` on `dgx-48`。随后发现脚本 skip 逻辑错误：validation-only 实际写 `0.jsonl`，但脚本检查 `${CHECKPOINT_STEP}.jsonl`（300），导致恢复 job 没有跳过已完成 shard。`466599_1` 已重新生成并覆盖 `shard_361_540/0.jsonl`（540 rows, 10,092,639 bytes，mtime 2026-07-07 00:36）；`shard_001_180/0.jsonl` 在发现时仍未变。已取消 `466596_0`，修复 skip 逻辑并提交 `3f0504a fix(sft1): skip existing validation-only rollout jsonl`，服务器 worktree已同步；重新提交 task0-only job `466650`，其日志确认 `[skip] train/shard_001_180 already done`，正在只跑缺失 `shard_181_360`。截至 2026-07-07 00:44，`466599_1`、`466599_2`、`466650_0` 均 RUNNING，暂无新 traceback；缺失 JSONL 尚未写出。
