...
- fair-CFM smoke479923训练100步后因multi-token被错误路由到Query-only sample而FAILED；修复`685ef62`按representation选sample并strict T1/server18 tests。retry479931再次完成100步，但project quota满导致atomic final duplicate checkpoint仅写`.tmp`后exit1，无sample；两次failed checkpoints已删除，logs/metrics保留。为释放约5GB，已清理completed ID19/ID25 intermediate checkpoints，仅保留best/final；dead deps479924/25、479932/33已取消。storage-safe retry smoke479937（无interval ckpt）当前PENDING Priority；afterok formal479938（save每10k，同ID19 recipe）→eval479939。
