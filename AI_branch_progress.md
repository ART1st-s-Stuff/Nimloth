...
- 2026-07-13 SFT1 cache完成；5-epoch train473384在dgx-32健康运行，epoch1 val0.178709、epoch2 val0.092776，检查时epoch3 step30。确认`format_correct_rate=0`是检测协议bug（E0019）：latent query labels全mask为-100，但旧evaluator从user prompt自由生成并要求模型输出8个未监督query tokens；不代表action CE失败。Best只按val_loss选择不受影响；训练后需用“think后框架注入8 query tokens，再评估action block”的正确协议复评。
