...
- 人类批准ID192冻结feature-location audit；真实hook smoke Job`529749`完成。Formal attempt0 Job`529767`在commit`69ce3dc0`、normal/dgx-37运行2:14后失败：Qwen BPE将某instruction结尾标点与后续换行合并，隔离tokenize的instruction IDs不是真实prompt子序列。无feature cache/readout/result/optimizer update，W&B正确finished failed，不可resume。登记E0150；retry1改为对完整archived instruction field tokenize并用offset mapping取exact character span，同时更换fresh output/W&B identity。
