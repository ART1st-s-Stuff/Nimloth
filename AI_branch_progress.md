...
- 人类要求评估达到SFT1级速度并选择不打断当前作业。核实SFT1仅613记录/5 epochs/50 optimizer steps、总27m12s；当前SFT2总14,560步，单步4.57s已快于SFT1约17–18s，保持目标时不可能达到半小时walltime。后续run的fail-closed DINO CLS float32 sidecar cache已实现（suite81 passed）并经GPU job`480899`验证：15m18s构建306MB、65,443 rows、16样本bitwise一致；teacher-only快199–261x，但按当前1.27 images/micro推算只占step约1.3%、全程约省15分钟。主瓶颈仍是Qwen current/next、backward/checkpoint recompute及跨节点每micro同步；需另行确认full-SFT2 timing profile。packed-forward因不等价继续禁用。
