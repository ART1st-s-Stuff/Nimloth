...
- K4 actor replay继续接通：current action logits与persisted MCTS root means按批准beta构成Scheme-B，执行动作的PPO ratio和guided entropy由此计算；MCTS tensor detach，不接actor梯度。`main_ppo`不再丢弃K4 search fields，training contract fingerprint也接受并绑定K4 policy。VAGEN=`c48b144`，production gate仍关闭。superpod SSH目前持续`Connection closed by UNKNOWN port 65535`，故torch/server回归尚未完成，禁止声称通过。
