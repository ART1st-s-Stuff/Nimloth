...
- 网络恢复后 rollout smoke attempt 1 在模型加载前 exit 143，未生成 JSONL/checkpoint。定位为 nested login shell 的 worktree HOME 缺少 `.ssh`，以及 `pkill -f 'ray::'` 可能自匹配父 shell；已修复 `.ssh` 初始化和安全 `[r]ay::` pattern，待提交后重试。
