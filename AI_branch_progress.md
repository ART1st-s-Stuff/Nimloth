...
- ID182首次fresh restore-only在Ray/model/checkpoint load前失败：update phase因env启动路径执行过`cd VAGEN`，restore phase跳过env分支后从Slurm cwd运行，relative Hydra searchpath无法解析`ppo_trainer`。已登记`E0120`；该launcher bug不否定phase1，ID182为可恢复的`needs_restore_only_retry`，严禁重跑update。后续须unconditional VAGEN cwd、新非覆盖phase2 attempt目录、fresh exact-SHA验证和人类对8GPU restore-only continuation的批准。
