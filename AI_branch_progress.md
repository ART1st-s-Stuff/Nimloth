...
- 按人类建议新增`fb74dec`/server10 tests：实际Projected8192直接训练独立TokenConditionedFlowUNet，actual/predicted projected严格共享该CFM；final columns=`GT|Qwen ViT CFM|Query CFM|Projected CFM|WM predicted CFM`。smoke479793完成；formal479794 `COMPLETED00:59:06`，55,680/30ep，W&B`gygcldff`，best subset.0336661，full correct.0405128/shuffled.0406263/ratio1.002801，故projected native CFM未过condition-sensitivity gate。eval479795 `COMPLETED00:01:46`，W&B`885911vb`，40runs/200frames/4 sheets：L1 Qwen.275183/Query.326296/Projected actual.333355/WM predicted.320217；predicted较低L1来自mean-color blur。State MSE.003393/cos.7053；视觉predicted几乎全坍缩beige/gray silhouette。
- 随后确认eval479795存在严重train/inference protocol mismatch，故上述WM-predicted视觉不能用于判断learned WM质量：SFT2 loss始终经`wm_predictor.forward→predict_next_emb`以`T=1`训练；checkpoint config却`history_size=4`，eval调用`rollout_states`以T=4推理，使用从未训练的position1–3，并把缺失历史填成重复state+有效action0(move_forward)，且无padding mask。登记E0047。
- 按人类要求commit`9978971`/server42 tests从结构上强制exact T：`_predict_from_context`要求input T==configured history_size；`predict_next_emb`仅允许config1；single-state rollout仅允许config1并递归T1，彻底删除fake history padding；SFT2新增`--wm-history-size`并在当前transition trainer拒绝非1，checkpoint/config mismatch也立即报错；legacy错误checkpoint只能通过显式override迁移为one-slot architecture并strict load（仅slice训练过的pos0）。
- corrected recursive-T1 diverse40 eval479918 `COMPLETED00:01:56` exit0/dgx-27，W&B`x8fqdses`已重传；metadata确认one-slot/T1。结果仍是beige/gray collapse：L1 predicted.320138，State MSE.0034163/cos.70357，horizon cos .711/.689/.670/.704/.744；与无效T4 run几乎相同。T4协议bug不是该checkpoint视觉失败的主因；T1 h1即有较大State angular/off-manifold gap。
- 人类要求同时做严格WM验证与去decoder瓶颈公平CFM。实现`2ccf47c`/server21 tests：全val teacher-forced T1全部8 actions vs identity/train-mean/wrong；CFM将Projected8192视为8×1024→8×256以匹配Query interface。
- WM validator479922 `COMPLETED00:00:25`，W&B`gs21abz9`，5,699 heldout pairs：predictor MSE.0030036/cos.74173，identity .0026633/.76966，train mean .0024029/.78071；即predictor比identity差12.8%、比mean差25.0%。但wrong-action MSE.003525比correct差17.36%，correct action在8项中best41.57%/mean rank3.04，说明有action signal但整体next-State prediction不及trivial baselines。dominant move_forward 2856 pairs上predictor.003424远差identity.002296；turn-right/left/backward则显著优于identity。结论：当前WM predictor整体未训好，虽学到部分转向/后退action dynamics。
- fair-CFM smoke479923训练100步后因multi-token被错误路由到Query-only sample而FAILED；修复`685ef62`。retry479931又因project quota满在atomic final save失败；failed ckpts删除、ID19/25 intermediate ckpts清理约5GB并保留best/final；storage-safe chain479937→479938→479939已提交。
- 人类批准新SFT2使用8×1024 hidden总容量、residual、T4。实现`1541c50`+YAML及`cbd879f`配置映射：external State8192 reshape8×1024，T4=32 tokens；6-layer hidden1024/16×64/MLP4096，共125,962,312 params；fixed4 slots为right-aligned real history+zero left PAD+valid mask，block-causal允许同frame tokens互看并屏蔽PAD；PAD action为masked zero embedding；zero-init delta输出`s_t+delta`；teacher-forced/recursive同协议。CLI/config/checkpoint/DDP/full-trajectory invariants严格。`5bb5a4e`记录identity-relative train/val metrics；`4728f0a`确保非packed per-prefix路径也构造真实trajectory T4 contexts并新增launchers；server latest49 tests（此前64/63）通过。
- 人类选择smoke通过后自动formal。real-data smoke479978 `COMPLETED00:02:27` exit0/dgx-14，W&B`ndcz52xp`：35steps+val，真实nonpacked path/LoRA/EMA/aux DDP通过；val WM.00463178优于identity.00469884/gain+.00006706，cos.996288>.996225；T4/token8/residual70 tensors finite，delta norm.718647。formal479979已释放但两hetero components均PENDING Resources；scheduler estimate group0约10:49 HKT、group1显示03:04但必须同获资源，故最早约10:49且不保证。输出ID29，strict3217/val355/world4 pair/GA8/5ep；group1 cap8h，20min exact resume。
- fair8-token Projected CFM storage-safe链完成：smoke479937/W&B`vmkfshpz`；formal479938 W&B`yi808j9e`，full ratio1.002517未过gate且不优于single-token1.002801；eval479939/W&B`7skxv7oc`，L1 Qwen.275183/Query.326296/actual fair Projected.367705/pred.344066；匹配8-token未恢复视觉敏感性，旧single256 bottleneck非主因。
- 新T4 residual SFT2 formal479979健康RUNNING，W&B`nvy3monl`。epoch1已于step1655完成并写epoch_001/best：full val WM MSE.00240206 vs identity.00369671，gain+.00129465；pred cos.900947 vs identity.851121，明确通过identity gate；fixed rollout success.309859。16:33约step1696/8275、epoch2开头，last100 WM.002501 vs identity.004077/gain+.001428，89% steps gain正；latest16:30。无traceback/OOM/nonfinite。当前约9–10s/step且epoch val/checkpoint有额外开销，dgx54 8h cap20:24时预计仅到epoch2后段，需从latest续提。
