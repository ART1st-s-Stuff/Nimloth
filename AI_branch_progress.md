...
- Plan B retry15 smoke 已启动：复用 retry14 env job `466322`（`dgx-05`, ports `19530-19533`），在新 run dir `/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry15` 用 manual `srun --overlap` 跑 array task 3，PID `1284150`，log `manual_srun_rollout_array3_466322_retry15.log`。截至最近检查：进程仍运行，config 中 `limit_mm_per_prompt: 20`，vLLM 日志显示 encoder cache profiled with `2 image items`，已越过 retry14 的多图报错并持续生成；尚未写出 `validation/**/300.jsonl`。
