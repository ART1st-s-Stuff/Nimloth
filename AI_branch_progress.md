...
- Rollout/converter、SFT1全部gate通过。SFT2正式grid22/max_pixels100352+budget12最长19-prefix完成8步，峰值51.12/52.17GiB，cache非瓶颈；epoch resume通过。`096c576` partial step5严格skip5/8并完成step6-8，counter RNG+invariants使首个resumed pre-update losses逐值一致；跨进程DDP最终仅有界数值复现（Qwen max3.58e-7、aux BF16量级），不宣称bit-exact。全部preflight gate通过，full-scale尚未启动，待人类确认。
