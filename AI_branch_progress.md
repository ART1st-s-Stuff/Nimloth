...
- ID185 retry1将`dgx-10`加入Navigation-head exclusions但保留worker eligibility，使用新output suffix `_retry1`和W&B `nimloth-id185-k4-full-eval-test300-retry1`；Parent=`317074d6`、VAGEN=`735226a1`，fresh server worktree五层clean，remote parent launcher回归`19 passed`且retry output/W&B不存在。Job`522968`已提交normal exact4×2/5h，当前`PENDING(Priority)`、elapsed0；attempt1 output/W&B身份不会复用。
