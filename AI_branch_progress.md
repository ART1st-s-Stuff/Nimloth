...
- W&B 上传默认只记录必要的 scalar / image；table 也会产生 W&B 内部 table artifact，只有确有必要才用。不要显式上传 artifact，除非人类明确要求，或确有不可再生文件需要保存且已先说明空间成本并得到确认。
...
- 2026-07-05 子 agent 已只读溯源 low-success-source SFT2 run `/project/peilab/atst/nimloth/outputs/experiments/training/sft2/2026-07-01/sft2_lejepa_align_fn1_dgx56/`：step≈1000 诊断 checkpoint 选用 `ckpt_step1000_preserved`，包含 LoRA adapter、`vision_full_state.pt`、`vision_ema.pt`、`state_proj.pt`、`wm_predictor/predictor.pt`、`value_head/value_head.pt`，`training_state.pt` step=1000。SFT2 数据为 vagen79 converted records：train `/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_sft_records_vagen79_nimloth_format/train_all.jsonl`，val `.../val_all.jsonl`；split README/manifest 证明 train/val/test 语义。
- 已新增诊断 eval config：`configs/eval/representation_ablation/diagnostic_low_success_sft2_step1000_value_predictor.yaml`，用于 step1000 single-latent value/predictor 离线诊断；另有 smoke 版 `diagnostic_low_success_sft2_step1000_value_predictor_smoke.yaml`（`max_records=2`, `max_batches=2`）。尚未启动真实 eval。
- 已修复 representation ablation eval 对 SFT2 LoRA adapter-only checkpoint 的加载：若 `qwen_checkpoint` 是 adapter dir，则从 `adapter_config.json` 读取 base model，配置 LoRA，加载 adapter 与 `vision_full_state.pt`；`StateProjector` hidden size 通过 helper 从模型/PEFT wrapper 解析。
- 已为 Plan B rollout 脚本新增 `ROLLOUT_MODEL_PATH` override，允许直接指定高成功率 VAGEN step300 actor HF path。
- 子 agent 已溯源该 SFT2 使用的 SFT1 source：`/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_train_vagen79_qwen25vl_alltrain_8gpu_lora/epoch_002/hf_merged`，SFT1 初始 VAGEN 是旧 step79 checkpoint。高成功率 VAGEN step300 rerun 有两条路线：Plan A 复用旧 vagen79 records、只换 SFT1 init；Plan B 先用 step300 重新采集/转换 train/val/test records 再跑 SFT1+SFT2。人类确认并行跑：A 线 low-success step1000 debug；B 线 Plan B。
- A 线诊断 eval smoke 已完成：Slurm job `465669` 在 `dgx-04` `COMPLETED`，elapsed `00:02:23`，输出 `/project/peilab/atst/nimloth/outputs/experiments/representation_ablation/2026-07-05/diagnostic_low_success_sft2_step1000_smoke_a0a1446`。真实加载 SFT2 adapter checkpoint 成功，日志含 `vision_full_state_loaded=true`；关键 summary：`num_encoded_transitions=2`，`predictor_1step_mse=0.0015321865`，`predictor_1step_cosine=0.6884475350`，`value_top1_action_acc=1.0`，`value_top2_action_acc=1.0`，`value_chosen_mse=0.0159209650`。AUC 与 depth4/8 NaN 是 smoke 样本太小。
- B 线 Plan B 未能健康启动，后续 dependent jobs 已取消，无 active jobs。Attempt 1：env `465666` 在 `dgx-24` 失败，AI2-THOR smoke 仅 1/4 GPU 通过；rollout `465667` 失败，conversion/SFT1/SFT2 `465670-465672` 取消。Attempt 2：env `465677` 在 `dgx-56` AI2-THOR smoke 4/4 通过，但 env server 报 `No module named vagen.envs.navigation.serve`；rollout `465678` 失败，conversion/SFT1/SFT2 `465682-465684` 取消。
- B 线 blocking：服务器当前 `/project/peilab/atst/nimloth/external/VAGEN` commit `93c1124...` 有 `vagen.server.server`、`vagen.env.navigation.*`，但没有旧 SFT1 rollout/conversion 脚本引用的 `vagen.envs.navigation.serve` 和 `vagen.envs.navigation.utils.nimloth_format`。继续 Plan B 需要修改脚本/imports 或选择兼容 VAGEN checkout；若修改 submodule，需创建 `nimloth/exp/...` 分支。
- 旧 step79 VAGEN checkpoint provenance：直接 artifact 未记录训练时 branch/commit/dirty status；子 agent 基于远程 Git reflog 重建，最可能为 Nimloth repo `main` commit `2f683f5de25c5eaf0b804f1fbadc507697c336a6`，`external/VAGEN` branch `main` commit `629c270cf069680c70282f615e7f1b83a45684ab`，VAGEN nested `verl` commit `6360bfe706a00b4ece9105285776b5727ee449b7`。confidence=medium，无法排除当时未提交改动；W&B retro metadata commit `3342f0cc...` 属事后上传脚本，不是训练 provenance。2026-07-05 复查服务器 VAGEN remote 分支：`origin/nimloth/vagen-legacy` HEAD 为 `acc0e7550f73da71b66c80f0762fbfcff3905213`（2026-06-21, `Add wm alias for legacy navigation format`, `verl=869ff12...`）；`origin/vagen-legacy` HEAD 为 `787c7e2d36822ad2348255c2e838159327eb320c`（2026-03-20）。`629c270...` 被 `main`/`nimloth/main` 包含且 tree 中有旧 `vagen/envs/navigation/serve.py`，而 `origin/nimloth/vagen-legacy` / `vagen-legacy-dev` 使用 `vagen/env/navigation.*` 与 `vagen.server.server`，因此“step79 基于 vagen_legacy”仍需进一步用 artifact 证明，不能直接采用前述 reflog 推断作为高置信结论。人类随后确认其判断 step79 大概率来自 `origin/nimloth/vagen-legacy-dev`；后续 Plan B 如需基于 step79/VAGEN legacy 路线，应直接使用该分支/commit `93c1124aeaa7850098f46f2b708ee224ba894861` 作为工作假设。
- 已修复 Plan B 脚本对 `nimloth/vagen-legacy-dev` API 的兼容性：`experiments/training/sft1/env_external_4gpu.slurm` 现在检测旧 `vagen.envs.navigation.serve`，若不存在则使用统一 Hydra server `python -m vagen.server.server server.port=... navigation.devices='[0]' navigation.max_workers=48`；`convert_rollouts.py` 兼容从 `vagen.env.navigation.nimloth_format` 导入 Nimloth action format。
- 已开始 Phase 2 multi-token 基础实现：`src/nimloth/representation_ablation/qwen_tokens.py` 支持多 latent marker 展开与最后一组连续 latent token hidden 提取；`token_set.py` 提供 `(B,K,D)` token-set WM predictor/value head、rollout 与 checkpoint save/load；`tests/representation_ablation/test_token_set.py` 已新增形状/提取/checkpoint 测试。本地 `py_compile` 与 config tests 通过。服务器轻量 smoke 已在 commit `09b1b09` 通过：`tests/representation_ablation` 15 passed，`py_compile` 通过，`qwen_tokens` / `token_set` import 不要求 LeWM 初始化。
- Phase 2 multi latent 已初步接入离线 eval 主路径（尚未训练 checkpoint，尚未接入 reconstruction/fastpath）：`validate_eval_config` 支持 `representation.type=qwen_multi_latent` + `predictor.type=token_transformer` + `value_head.type=pooled_mlp`；`eval.py` 会展开 K 个 latent markers、提取 `(B,K,D)` hidden token set，可选逐 token 复用 `StateProjector`，加载 `TokenSetWMPredictor` / `TokenSetValueHead` 计算 value 与 predictor 多步指标；新增 K=4/8/16 eval YAML 模板。未实现的 vision tokens / compressed vision tokens / token-set reconstruction 仍显式报错。服务器 worktree 已同步到 `b656d16`；因服务器 venv 没有 pytest，改跑等价 smoke：`py_compile` 覆盖 representation_ablation/eval/tests，通过；Python smoke 覆盖 marker 展开、token-set 提取、TokenSetWMPredictor/ValueHead forward、K=4/8/16 YAML load + validator，通过，输出 `phase2_multi_latent_smoke=ok`。
- Phase 2 multi latent 训练入口已新增（尚未跑真实训练）：`src/nimloth/representation_ablation/train.py` 与 wrapper `python -m nimloth.training.representation_ablation.train --config <yaml>`；当前支持 `qwen_multi_latent` + `token_transformer` + `pooled_mlp`，冻结 Qwen/可选 StateProjector，on-the-fly 编码 K 个 latent tokens，训练 predictor MSE 和 chosen-action value MSE，保存 `wm_predictor/` 与 `value_head/` checkpoint。新增 K=4/8/16 训练 YAML 模板。本地 `py_compile` 与 YAML load 通过。服务器 worktree 已同步到 `3b5b3a9`；server smoke 通过：`py_compile` 覆盖 train entry/wrapper，`python -m nimloth.training.representation_ablation.train --help` 通过，K=4/8/16 training YAML load 通过，输出 `phase2_multi_latent_train_smoke=ok`。尚未启动真实训练。
- 新增 predictor capacity diagnostic 的第一步代码（尚未训练/尚未 RCDM 可视化）：按人类纠正，`qwen_vision_tokens` 使用 Qwen vision encoder 的直接输出（不是 `<|image_pad|>` LLM hidden states），不做 pooling，predictor 输入/输出为 `(B,K,D)` vision token set；训练入口支持 `train.rollout_steps=1` 的一轮预测监督与 `rollout_steps=4` 的 autoregressive 4 步监督（每步用 GT vision tokens 监督）。新增 `vision_tokens.py` 和 1-step / rollout4 training YAML；当前根据服务器 processor smoke，255x255 导航图像得到 `image_grid_thw=(1,18,18)`，Qwen visual merge 后 K=81。服务器 worktree 已同步到 `9afe0d2`，server smoke `py_compile` 与 1-step/rollout4 YAML load 通过，输出 `vision_token_predictor_config_smoke=ok`。RCDM 直拼接 vision tokens 的训练/采样入口尚未接入。
...
- 已接入并完成 GT compressed-token RCDM reconstruction 首轮诊断：新增 `src/nimloth/representation_ablation/rcdm_gt_cache.py`、`src/nimloth/eval/rcdm_cache_reconstruction.py` 与 launcher `experiments/training/representation_ablation/train_rcdm_gt_compressed.slurm`，commit `cde5e94`。Slurm job `466447` 在 `dgx-27` `COMPLETED 0:0`，elapsed `02:37:41`；输出 `/project/peilab/atst/nimloth/outputs/experiments/representation_ablation/2026-07-06/rcdm_gt_compressed_rollout4_b128_gt_current_1k_466447`。条件为 GT current image 的 raw Qwen vision tokens 经 rollout4 compressor 压缩后 flatten：`(16,512) -> cond_dim=8192`，target 为 current image。cache：train `54702` transitions，val `5468` transitions。训练 `1000` steps，final train loss `0.0104351994`，val loss `0.0067838798`；checkpoint `model_000001000.pt` / `ema_0.9999_000001000.pt` / `training_state_000001000.pt`。已生成 8 个 held-out val reconstruction strips：`samples_gt_current/sample_0000_strip.png` ... `sample_0007_strip.png`。曾误传到错误的 W&B entity `844956689-hong-kong-university-of-science-and-technology` 的 run `3v2c6fmo`；已按人类要求用 W&B API 删除该 run 及 artifacts。随后使用 `/project/peilab/atst/flower/.env` 中的 W&B 凭据，重新上传到正确 entity `art2nd-hong-kong-university-of-science-and-technology`：`https://wandb.ai/art2nd-hong-kong-university-of-science-and-technology/nimloth/runs/y8bfpn0o`。上传内容包括 samples table、contact sheet、summary metrics、训练曲线（100 个 train loss 点 + 1 个 val loss 点，custom x-axis `rcdm_train/optimizer_step`）与 visualization/metrics artifacts。初看 sample_0000 左侧 GT 清晰，右侧重建仍显著噪声化/模糊，说明 1k-step GT RCDM 已跑通但视觉质量尚不能证明 compressed tokens 足以高质量重建；下一步建议训练更久或先评估更多 sample/contact sheet，再决定是否转向 predicted-token visualization。
- 4GPU GT compressed-token RCDM full train 已启动并修复数据加载瓶颈：launcher `experiments/training/representation_ablation/train_rcdm_gt_compressed_4gpu.slurm`，目标 5 epochs，per-rank batch 8 / effective batch 32，使用同一 GT compressed cache。job `466675` 在 `dgx-27` 跑到 step3400 附近时发现速度异常慢（约 `11.2 sec/step`，GPU util 低），原因判断为 gzip state-cache random access + `RCDMStateCacheDataset._load_shard` 只缓存 4 个 shard；已提交 `74da7ce fix: speed up rcdm cache resume` 将 shard cache 增至 64，并把 4GPU launcher 默认 `SAVE_INTERVAL` 改为 500、支持 `RESUME=1`。job `466675` 已取消，保留 durable checkpoint step3000；随后短暂 resume job `466805` 因 launcher 让 `/project/peilab/atst/flower/.env` 覆盖 `WANDB_PROJECT=flower` 被取消，已提交 `1db34a2 fix: preserve wandb project in rcdm launcher`，删除/尝试删除错误 W&B run `art2nd.../flower/ybkojaxs`，移走错误 `wandb_run_id.txt`，并将 `train_step_log.csv` 截回 step<=3000（原日志备份为 `train_step_log_before_resume_truncate_step3400.csv`）。resume job `466808` 在 `dgx-27` 已完成，W&B 正确指向 `art2nd.../nimloth/runs/2kfx4hvq`，从 `training_state_000003000.pt` 恢复后完成到 step8550；修复后速度约 `0.23 sec/step`。最终 checkpoint：`model_000008550.pt` / `ema_0.9999_000008550.pt` / `training_state_000008550.pt`；final train loss `0.0045956858`，val loss `0.0039379080`。已上传 32 个 held-out val diverse scene 的对比 reconstruction 到同一 W&B run：`GT current | RCDM 1k | RCDM 5ep`，上传项包括 `compare_1k_vs_5ep_table`、contact sheet 和 artifact `rcdm_gt_compressed_compare_1k_vs_5ep_diverse32`；输出目录 `reconstruction_compare_1k_vs_5ep_diverse32/`。中间 upload job `466847` 因脚本放在 compute node 不可见的 `/tmp` 失败，retry job `466849` 完成。肉眼初看：5ep 相比 1k 噪声明显减少、颜色更平滑并偶尔保留门/窗/墙面轮廓，但整体仍偏模糊/色块，不能视为高保真重建。按人类要求随后不再比较 1k，而是选 8 个不同 held-out val trajectory/run，每个取前 5 个 step，并从第 2 个 step（step1）开始上传 `GT image | GT-token recon | pred-token recon`；预测使用 rollout4 compressed-token predictor 从 step0 token 与动作序列 autoregressive rollout。upload job `466863` 完成，输出 `reconstruction_gt_vs_pred_first5steps_runs8/`，W&B 同 run `2kfx4hvq` 增加 `gt_vs_pred_first5_steps_table`、`gt_vs_pred_first5_runs_table`、contact sheet 和 artifact `rcdm_gt_compressed_gt_vs_pred_first5steps_runs8`。初看 GT-token 与 pred-token recon 在同一 trajectory 内有相似粗布局/色调，pred 有时接近 GT-token，但两者整体仍是模糊色块。随后按“区分降维网络问题 vs reconstruction 问题”做诊断：job `466877` 上传 raw/EMA、100-step/1000-step、EMA shuffle、token NN retrieval；job `466883` 修正为 raw checkpoint 的 condition shuffle/loss。输出目录 `diagnostics_reconstruction_vs_compression_ema_sampling_shuffle_retrieval/`，W&B 同 run `2kfx4hvq`，artifact 包括 `rcdm_gt_compressed_reconstruction_failure_diagnostics` 与 `rcdm_gt_compressed_raw_condition_sensitivity`。关键结果：EMA `0.9999` 在仅 8550 step 后采样基本是彩色噪声（不能作为本次采样依据）；raw 1000-step correct vs shuffled condition 视觉上会随 condition 改变，说明 RCDM 不是完全忽略 condition；但 denoising loss correct `0.0129142`、shuffled `0.0134757`，相对差仅 `4.35%`，condition signal 很弱。compressed-token nearest train retrieval top1 cosine mean `0.9076`（min `0.7690`, max `0.9757`），多数 NN 图像与 query 在布局/场景上明显相似，说明降维 compressed tokens 保留了不少场景信息。当前判断：主要瓶颈更像 RCDM reconstruction/conditioning 设计和训练目标没调好，而不是 compressor 完全丢信息；但不能排除 compressed tokens 丢失精细纹理。随后 tiny-overfit 进一步确认：job `466896` 从 scratch 用同一 RCDM config 在 train split 64 个样本上训练 10000 steps（batch8, lr1e-4），输出 `tiny_overfit_train64_scratch_b8_lr1e4_steps10000_466896/`，W&B artifact `rcdm_gt_compressed_tiny_overfit_train64`。final correct train denoising loss `0.0031235`，shuffled-condition `0.0087861`，relative delta `181%`，说明模型在小数据上能明显利用 condition；但最终 sample sheet 仍是模糊/偏色、只保留粗结构，不能高保真复原训练图。更新后的结论：compressed tokens 至少含有粗场景信息，RCDM condition path 也不是坏掉；主要问题更可能是当前 RCDM reconstruction setup（flatten token -> single FC to time embedding、训练步数/目标/采样、可能需更强 decoder）不足以把这些 tokens 解成清晰图像。随后 deterministic token decoder tiny-overfit 给出更强证据：preflight 第一次因脚本括号语法错误失败，未提交 Slurm；修复后 job `466969` 在 `dgx-46` 完成（elapsed `00:03:30`），输出 `deterministic_token_decoder_train64_b16_lr2e4_steps10000_466969/`，W&B artifact `deterministic_compressed_token_decoder_tiny64`。模型为 256 learned image patch queries cross-attend 到 16 个 compressed tokens，loss `L1 + 0.5*MSE`。final train loss `0.016413`、L1 `0.015969`、MSE `0.0008888`、PSNR `36.53dB`；wrong-condition loss `0.463675`，wrong/correct ratio `28.25x`。final sheet 中 GT train 图几乎被清晰复原，wrong condition 会复原对应 wrong GT。结论更新为：GT compressed tokens 至少对训练图保留了足够高保真重建信息；RCDM 的问题主要是 diffusion reconstruction/conditioning recipe，不是单纯参数量，也不是 compressor 已经把图像信息丢光。随后测试 token-aware RCDM v2 的最小改法：job `466978` 用 16 tokens cross-attention 解成 32ch 16x16 spatial condition map，上采样后与 noisy image concat，再送入 ADM UNet（in_channels=35），train64 tiny-overfit 10000 steps，输出 `token_cond_rcdm_v2_train64_b8_lr1e4_steps10000_466978/`，W&B artifact `token_conditioned_rcdm_v2_tiny64`。结果失败：final correct train denoising loss `0.0097264`，shuffled `0.0097215`，相对差 `-0.05%`；final sheet 中 correct/wrong condition 采样几乎相同，都是彩色模糊块。结论：简单 input-concat spatial map 仍会被 diffusion/UNet 忽略，不能解决 reconstruction；后续必须用更强约束/结构，例如在 UNet attention/resblock 内做真正 token cross-attention、用 deterministic decoder 输出作为强 spatial scaffold 进行 diffusion refinement，或直接用 deterministic decoder 做当前 predicted-token visualization。按人类要求已把 6 张关键比较图汇总上传到 W&B run `2kfx4hvq`：`gt_vs_pred_first5`、`raw_condition_shuffle`、`raw_token_nn_retrieval`、`rcdm_tiny_overfit`、`deterministic_decoder_tiny64`、`token_rcdm_v2_tiny64`；新增 table `reconstruction_method_comparison/table`、contact sheet `reconstruction_method_comparison/contact_sheet`、artifact `reconstruction_method_comparison_sheets`，输出 `reconstruction_method_comparison_upload/`。随后 full deterministic decoder + 5-action rollout 完成：job `467001` 在 `dgx-15` 完成（elapsed `00:12:01`），输出 `deterministic_token_decoder_full_b64_e5_rollout5_467001/`，训练 full train cache `54702`、held-out val `5468`，5 epochs / 4275 steps / batch64 / lr2e-4；final val loss `0.2139186`、L1 `0.1840999`、MSE `0.0596374`、PSNR `18.27dB`，train subset PSNR `20.45dB`。已上传 8 条 held-out val trajectory 的 5-action rollout（step1..5）：`GT image | GT-token deterministic recon | pred-token deterministic recon`，W&B keys `det_token_decoder_full/rollout5_contact_sheet`、`rollout5_steps_table`、`rollout5_runs_table`，artifact `deterministic_compressed_token_decoder_full_rollout5`。初看 full decoder 泛化仍偏模糊块状，GT-token 与 pred-token deterministic recon 在色调/粗布局上接近，但未达到 tiny64 overfit 的清晰度；说明当前 deterministic decoder 结构能证明信息存在，但 full 泛化 decoder 还需更强训练/结构/损失。按人类同意的下一路线，新增 token-conditioned conditional flow matching tiny64 overfit：job `467029` 在 `dgx-15` 完成（elapsed `00:04:11`），输出 `token_cfm_decoder_tiny64_b8_lr1e-4_steps10000_467029/`，训练 first64 train-cache samples，loss 为 flow velocity MSE；final correct-condition flow MSE `0.00848754`、shuffled-condition `0.08632887`、shuffled/correct `10.17x`。W&B keys `token_cfm_tiny64/*`，artifact `token_conditioned_cfm_tiny64`。final sheet 中 correct condition 能清晰 overfit，shuffled condition 变成错误布局/场景；结论：CFM tiny64 明确吃 compressed-token condition，已通过进入 full CFM train + held-out 5-action rollout 的 gate。full CFM + true 5-action rollout 随后完成：job `467041` 在 `dgx-15` 完成（elapsed `00:23:30`），输出 `token_cfm_decoder_full_b32_e10_lr1em4_rollout5_467041/`，full train `54702`、held-out val `5468`，batch32、10 epochs、17100 steps、lr1e-4。final held-out val full correct flow MSE `0.0380000`、shuffled `0.0398812`、shuffled/correct `1.0495`；best val-subset correct MSE `0.0353706`，checkpoint `token_cfm_full_best_val.pt`，final checkpoint `token_cfm_full_final_step_017100.pt`。已上传 8 条 held-out val trajectory 的 true 5-action rollout（step1..5）：`GT image | GT-token CFM recon | pred-token CFM recon`，W&B keys `token_cfm_full/rollout5_contact_sheet`、`rollout5_steps_table`、`rollout5_runs_table`，artifact `token_conditioned_cfm_full_rollout5`。视觉上 full CFM 明显比旧 RCDM 和 full deterministic decoder 清楚，常能生成门、房间、浴室、窗、柜台等可识别结构；GT-token 与 pred-token CFM 在粗布局/色调上经常接近。但 val flow-loss condition sensitivity 只有约 `1.05x`，说明 full 模型仍可能较依赖图像先验，CFM 结果应作为强 qualitative visualization，而非已证明高保真条件解码。
- 按人类要求补跑 full CFM 的 5 ODE/Euler sampling-step reconstruction 并上传：首个 upload job `467113` 在采样/上传前失败（dynamic import 未注册 `sys.modules` 导致 dataclass 报错），输出 `reconstruction_5ode_rollout5_467113/` 已记录失败；修复后 retry job `467116` 在 `dgx-51` 完成（elapsed `00:01:07`），输出 `token_cfm_decoder_full_b32_e10_lr1em4_rollout5_467041/reconstruction_5ode_rollout5_467116/`。使用 full CFM final checkpoint `token_cfm_full_final_step_017100.pt`，held-out val cache，只评估不训练，8 条 held-out val trajectory × 5 actions，列为 `GT image | GT-token CFM 5ode recon | pred-token CFM 5ode recon`。已上传到 W&B run `2kfx4hvq`：`token_cfm_full_5ode/rollout5_contact_sheet`、`rollout5_steps_table`、`rollout5_runs_table`，artifact `token_conditioned_cfm_full_5ode_rollout5`。视觉上 5-ODE 比 50-ODE 更噪、更软，但仍保留门/房间/浴室/窗等粗结构，GT-token 与 pred-token reconstruction 在许多 row 上仍接近。随后按“先检查 best checkpoint”补跑 best-val checkpoint sampling：job `467126` 在 `dgx-09` 完成（elapsed `00:02:19`），输出 `reconstruction_best_50ode_rollout5_467126/` 与 `reconstruction_best_5ode_rollout5_467126/`，W&B keys `token_cfm_full_best_50ode/*`、`token_cfm_full_best_5ode/*`，artifacts `token_conditioned_cfm_full_best_50ode_rollout5`、`token_conditioned_cfm_full_best_5ode_rollout5`。初看 best checkpoint 比 final checkpoint 稍干净/稳定，尤其 50-ODE，但提升不大；best-5ODE 仍明显比 best-50ODE 更噪更软。结论：qualitative visualization 可优先用 best checkpoint；但普通继续训练未必能解决 full condition sensitivity。
- condition dropout + CFG CFM continuation 已完成：job `467188` 在 `dgx-29` 完成（elapsed `00:40:50`），输出 `token_cfm_decoder_full_b32_e10_lr1em4_rollout5_467041/token_cfm_dropout_cfg_b32_e20_lr3em5_drop0p15_467188/`。从 full CFM best checkpoint `token_cfm_full_best_val.pt` 初始化，只训练 CFM decoder，Qwen/compressor/cache/predictor 冻结；full train cache `54702`、held-out val cache `5468`，batch32、20 epochs、34200 steps、lr `3e-5`、condition dropout `0.15`。final full-val correct flow MSE `0.0378681`、shuffled `0.0394756`、shuffled/correct `1.04245`，uncond `0.0373392`、uncond/correct `0.98603`；best val-subset correct MSE `0.0346057`，best checkpoint `token_cfm_dropout_cfg_best_val.pt`，final checkpoint `token_cfm_dropout_cfg_final_step_034200.pt`。best-checkpoint full-val sampled metrics：correct `0.0386756`、shuffled `0.0403907`、shuffled/correct `1.04435`、uncond/correct `0.99349`。已上传 CFG scale `1.0` 和 `2.0` 的 8 条 held-out val trajectory × 5 actions contact sheets 到 W&B run `2kfx4hvq`，artifacts `token_conditioned_cfm_dropout_cfg1p0_rollout5`、`token_conditioned_cfm_dropout_cfg2p0_rollout5` 和 `token_conditioned_cfm_dropout_cfg_model`。初步结论：condition dropout + CFG 没有显著改善 full-val flow-loss condition sensitivity（仍约 `1.04x`，且 uncond MSE 不差于 correct）；CFG=2 视觉上比 CFG=1 更锐利/对比更强，但也更容易幻觉结构，因此只能作为定性可视化，不证明 full decoder 已强依赖 tokens。
- SFT2 single-latent → frozen CFM reconstruction 试验已完成：job `467246` 在 `dgx-09` 完成（elapsed `00:01:25`），输出 `token_cfm_decoder_full_b32_e10_lr1em4_rollout5_467041/sft2_latent_adapter_to_cfm_b256_e10_lr1em3_467246/`。这不是把 1024-dim SFT2 latent 直接喂进 CFM；脚本训练一个小 adapter 将 `SFT2 latent(1024)` 映射到 CFM 的 compressed-token 条件空间 `(16,512)`，冻结 condition-dropout CFM best checkpoint 和 SFT2 WM predictor。paired train cache `54702`、held-out val cache `5468`，adapter batch256、10 epochs、2140 steps、lr `1e-3`。token metrics：best val-subset MSE `0.4648114`；full-val token MSE `0.4677339`、cosine `0.7210491`、shuffled/correct MSE ratio `1.10858`。5-action rollout contact sheet 列为 `GT image | GT-compressed-token CFM | SFT2 GT-latent→adapter→CFM | SFT2 pred-latent→adapter→CFM`，W&B final keys `sft2_latent_to_cfm_final/*`（最终重传只含 scalar/image，无显式 artifact）。初看 GT SFT2 latent 可恢复一些房间/门/墙结构，但明显弱于 GT compressed tokens；predicted SFT2 latent rollout 经常漂到错误的暗门/走廊场景，支持旧 single latent/predictor 在多步视觉保真上弱于 compressed-token state。
- 按人类要求补做两项 CFM 诊断并完成：① 重新训练 SFT2 1024-d latent direct-conditioned CFM，② ViT compressed-token CFM 低学习率继续训练。代码为输出目录中的一次性脚本 `train_sft2_latent_direct_cfm_and_compare.py`、`continue_vit_token_cfm_low_lr.py`、`make_combined_lowlr_vit_sft2.py`，服务器 worktree commit `dad1292`，数据为同一 paired train cache `54702` / held-out val cache `5468`，W&B 只上传 scalar/image、无显式 artifact/table。SFT2 direct CFM job `467260` 在 `dgx-29` 完成（elapsed `00:25:49`），输出 `token_cfm_decoder_full_b32_e10_lr1em4_rollout5_467041/sft2_latent_direct_cfm_b32_e10_lr1em4_compare_467260/`；batch32、10 epochs、17100 steps、lr `1e-4`；best val-subset correct flow MSE `0.03654445`，final full-val correct `0.03918332`、shuffled `0.03926515`、shuffled/correct `1.00209`，说明 direct CFM 几乎不依赖 SFT2 latent condition。ViT CFM low-lr continuation job `467261` 在 `dgx-29` 完成（elapsed `00:24:49`），从 `token_cfm_dropout_cfg_best_val.pt` 继续训 batch32、10 epochs、17100 steps、lr `1e-5`、condition dropout `0.15`，输出 `vit_token_cfm_low_lr_cont_b32_e10_lr1em5_from_dropcfg_467261/`；best val-subset correct flow MSE `0.03534796`，final full-val correct `0.03848815`、shuffled `0.04046172`、shuffled/correct `1.05128`、uncond/correct `0.95383`。相对 dropout-CFG 旧 best（best subset `0.0346057`，full correct `0.0386756`）没有实质改进，uncond 仍不差，condition sensitivity 仍弱。综合对比图先由 `467260` 生成旧 ViT 版，随后 job `467283` 在 `dgx-21` 完成（elapsed `00:02:15`）用 low-lr ViT best checkpoint 重生成最终图：`sft2_latent_direct_cfm_b32_e10_lr1em4_compare_467260/combined_with_vit_lowlr/combined_rollout5_cfg2p0/contact_sheet_combined_rollout5_runs8.png`，列为 `GT | ViT GT | ViT Pred | SFT2 GT direct | SFT2 Pred direct | SFT2 GT adapter | SFT2 Pred adapter`，已上传 W&B key `combined_lowlr_vit_sft2_cfm/rollout5_contact_sheet` 到 run `2kfx4hvq`，确认 `0 artifact file(s)`。视觉结论：low-lr ViT GT/Pred 通常仍是最有结构的；SFT2 direct 有时生成房间/门/浴室结构但条件敏感性极弱；SFT2 pred direct/adapter 多步预测更常漂移到错误暗门/走廊或不相关场景。
- Plan B retry（commit `1c5163e`）env job 已可在 `dgx-56` ready，使用 `vagen.server.server` 启动 ports `18900-18903`；rollout array 仍在 Ray startup 前 SIGTERM，未生成 records/training，downstream jobs cancelled。已修复可疑点：`rollouts_greedy_parallel.slurm` 移除 user-wide `ray stop` / `pkill -f ray::`，改为 per-array-job `RAY_PORT` 与 `RAY_TMPDIR`，避免同节点并发 array tasks 互相杀 Ray。
- Plan B retry7（Nimloth `1c81d97`，VAGEN `93c1124`）已由子 agent 完成失败诊断：env job `465783` 在 `dgx-56` ready，rollout array `465785` 各 shard 已启动 Ray 并打印 resources，但无 shard JSONL 写出；downstream conversion/SFT1/SFT2 未提交。根因是 rollout 命令仍使用旧 VAGEN 入口/配置（`vagen.main_ppo`、`vagen/configs/vagen_multiturn`），当前 legacy-dev VAGEN 需要 `vagen.trainer.main_ppo` 与内置 `vagen/trainer/config/ppo_trainer.yaml`，数据也应为 parquet rows with `extra_info.env_name/env_config/seed`。
- 已更新 SFT1 rollout 脚本以兼容当前 VAGEN：`rollouts_greedy_parallel.slurm` 会为每个 shard 生成 parquet dataset，检测 `vagen.trainer.main_ppo` 后走新入口，设置 `rollout_manager.use_service=True` 与单个 active env server URL；保留旧 `vagen.main_ppo` fallback。`submit_env_external_4gpu.sh`、`submit_rollouts_greedy.sh`、`env_external_4gpu.slurm`、`rollouts_greedy_parallel.slurm` 现在支持通过 `REPO=/project/.../.worktree/exp-latent-repr-ablation` 提交，避免强制使用主 worktree。已做本地 `bash -n`；远程用 `python -m vagen.trainer.main_ppo --cfg job` 验证新增 Hydra overrides 可解析。
- Plan B retry8（Nimloth `aa09f1a`，VAGEN `93c1124`）已完成失败诊断：远程 worktree 初始 clean、HEAD 正确、脚本 `bash -n` 通过；env job `465811` 在 `dgx-56`/`preempt` ports `19130-19133` ready，后已取消。rollout job `465813` 首先因长 retry8 输出路径导致 Ray `AF_UNIX path length cannot exceed 107 bytes`；用 `/tmp/hs8` symlink wrapper 重交 `465818` 后确认真正进入 `vagen.trainer.main_ppo`、使用 parquet 与 `rollout_manager.use_service=True`，但在 `RayPPOTrainer._validate_config` 失败：缺少 `actor_rollout_ref.rollout.micro_batch_size` 或 `actor_rollout_ref.rollout.micro_batch_size_per_gpu`（配置中 rollout log-prob micro batch 字段为 `None`）。另有一个 array task 命中 Ray worker grpc port `10003` 冲突。没有 env reset 证据，没有 `validation/*/shard_*/300.jsonl`，未做 conversion/SFT1/SFT2。远程 README：`/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry8/README_retry8.md`。
- 已修复 retry8 暴露的 Nimloth rollout 脚本问题（未改 submodule）：新 VAGEN trainer 路径加入 `actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=1`；Ray temp dir 默认改为短 `/tmp/ray_sft1_${USER}_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}`；为每个 array task 显式隔离 Ray node/object/dashboard/client/metrics/worker port，并把 `RAY_ADDRESS` 固定到本任务 head，降低同节点并发串线风险。已本地 `bash -n`，远程 `python -m vagen.trainer.main_ppo --cfg job ... actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=1` 验证 Hydra override 可解析。
- Plan B retry9（Nimloth `8041689`，VAGEN `93c1124`）已完成失败诊断：远程 worktree clean、HEAD 正确、VAGEN submodule 正确、脚本 `bash -n` 通过；env job `465938` 在 `dgx-56`/`preempt` ports `19130-19133` ready，后已取消。rollout array `465940_[0-3]` 均在 Ray startup 立刻失败：`ray start` 收到 `--ray-client-server-port` 后要求安装 `ray[client]`，而服务器 venv 是 minimal Ray。此次失败发生在 Ray head 真正启动前，未进入 `vagen.trainer.main_ppo` / `_validate_config`，无 env reset 证据，无 `validation/*/shard_*/300.jsonl`，未做 conversion/SFT1/SFT2。远程 README/metadata：`/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry9/README_retry9.md`、`metadata_retry9.json`。
- 已修复 retry9 暴露的 Ray client 依赖问题：`rollouts_greedy_parallel.slurm` 移除未使用的 `--ray-client-server-port` 和 `RAY_CLIENT_SERVER_PORT`，保留 head/node/object/dashboard/metrics/worker port 隔离。已本地 `bash -n`。
- Plan B retry10（Nimloth `f048539`，VAGEN `93c1124`）已完成失败诊断：远程 worktree clean、HEAD 正确、VAGEN submodule 正确、SFT1 Slurm 脚本 `bash -n` 通过；env job `465949` 在 `dgx-56`/`preempt` ports `19130-19133` ready，后已取消。rollout array `465950_[0-3]` 已越过 Ray startup / retry9 的 `ray[client]` 问题，进入 `vagen.trainer.main_ppo`，并通过 config validation；随后在 `trainer.init_workers()` 加载 step300 HF model 时失败。根因是指定 checkpoint shard `model-00001-of-00002.safetensors` 实际存在但权限为 `0600`、owner=`hligb`，当前提交用户无法读取；诊断 job `465966` 在 `dgx-56` 确认 `PermissionError [Errno 13] Permission denied`。无 env reset 证据，无 `validation/*/shard_*/300.jsonl`，未做 conversion/SFT1/SFT2。远程 README/metadata：`/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry10/README_retry10.md`、`metadata_retry10.json`。下一步需要让 step300 HF model shards 对当前提交用户可读（或提供可读副本），再用新输出目录重试；本问题不需要修改 submodule。
- Plan B retry11 preflight（未提交 Slurm job）验证人类提供的新路径 `/project/peilab/atst/vagen_ckpt/actor/huggingface`：目录结构是 HF/Qwen2.5-VL，`config.json`、tokenizer、processor、`model.safetensors.index.json` 可读，`AutoConfig`/`AutoTokenizer`/`AutoProcessor` 成功；但两个 actor HF safetensors shard 仍为 `0600 hligb:peilab`，当前提交用户直接 `open()` 报 `PermissionError`，`safetensors.safe_open` 也无法打开。因此没有启动 env/rollout，未产生 records/conversion/SFT1/SFT2。远程记录：`/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry11_preflight/README_retry11_preflight.md`、`metadata_retry11_preflight.json`。可读的 `/project/peilab/atst/vagen_ckpt/actor/model_world_size_8_rank_*.pt` FSDP shards 可能可用 `external/VAGEN/verl/scripts/model_merger.py` 另行合并成 HF，但这是额外计算/存储任务，未获明确确认前不启动。
- 人类更改权限后，Plan B retry11 smoke（env `466262`，rollout `466264_3`）确认 `/project/peilab/atst/vagen_ckpt/actor/huggingface` 的 safetensors 已可读，rollout 进入 `vagen.trainer.main_ppo` 并通过 config validation；随后 vLLM 初始化失败，原因是 `common_env.sh` 继承的 `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` 与 vLLM memory pool 不兼容。已提交 `24c9dc3 fix(sft1): unset expandable cuda alloc for vllm rollout`，只在 rollout `COMMON_ENV` 中 unset 该变量。
- Plan B retry12 smoke（Nimloth `24c9dc3`，env `466268`，rollout `466274_3`）确认 checkpoint 权限和 vLLM allocator blocker 均已解除：Transformers/vLLM 成功加载 `/project/peilab/atst/vagen_ckpt/actor/huggingface`，vLLM 初始化 KV cache 并开始 validation。新 blocker：当前 VAGEN `vagen.env.navigation.env.NavigationEnv.ValidEvalSets` 只允许 `base`、`common_sense`、`complex_instruction`、`visual_appearance`、`long_horizon`，不接受脚本用于训练数据收集的 `base_train` / `common_sense_train` / `long_horizon_train`，env server 在 `/environments` 因 `assert config.eval_set in self.ValidEvalSets` 返回 HTTP 500。无 env reset 成功、无 `validation/*/shard_*/300.jsonl`、未做 conversion/SFT1/SFT2。下一步不能用 heldout eval sets 充当训练数据；需要人类确认真实 train dataset source 或兼容 VAGEN checkout 后，才能修改 submodule/split handling。
- 子 agent 只读溯源 `_train` split：当前 `93c1124` 是 `vagen/env/navigation` legacy-dev 布局，只有 `datasets/{base,common_sense,complex_instruction,visual_appearance,long_horizon}.json`（各 60 tasks），没有 train json；`*_train` 定义存在于 VAGEN `origin/nimloth/main`（HEAD `f7aefd3057bc462338065b75259f6bd946b84325`）/ `origin/main` 的旧 `vagen/envs/navigation` 布局，`VALID_EVAL_SETS` 包含 `base_train`、`common_sense_train`、`long_horizon_train`，assets 中三个 train json 各 1200 tasks；关键历史 commit 包括 `bc349d7`（base/common_sense train）和 `7f6a1bd`（long_horizon train）。旧评估 5 个 JSON 与 `f7aefd3` 的 5 个 eval assets hash 相同，说明 schema 匹配。建议方向：若继续 Plan B，优先让人类确认是否切 VAGEN 到 `origin/nimloth/main` `f7aefd3`（有旧 `vagen.envs.navigation.serve.py` / `vagen.main_ppo.py`，脚本有 fallback），或批准在 `93c1124` 上补 train datasets + ValidEvalSets；不能把 heldout eval set 当训练数据。
- 按人类指示没有切换 VAGEN 到 `nimloth/main`，而是在 legacy-dev 工作线上补 train split 兼容：VAGEN `6be3088ce87a8e40c700c6f5411c1c659a968ef2` 添加 `base_train/common_sense_train/long_horizon_train` dataset JSON 与 `ValidEvalSets`；VAGEN `375e6ef498999ebf8f145cc3a27efb3849323bd8` 增加 legacy VAGEN 的 `eval_mode` prompt/parser 支持；Nimloth 父提交分别更新到 `600d776`、`a4fc1ee`。VAGEN 分支已推到 `nimloth/exp/latent-repr-ablation-train-splits`。
- Plan B retry14 smoke（Nimloth `a4fc1ee`，VAGEN `375e6ef`）用 env job `466322` 在 `dgx-05` 启动 ports `19530-19533`，并用 `srun --jobid=466322 --overlap` 手动启动 array task 3（PID `1101711`，log `.../sft1_rollouts_highsuccess_step300_greedy_parallel_retry14/manual_srun_rollout_array3_466322.log`），生成了 `val_shard_1081_1200.parquet`（360 rows）。本轮已越过 train split 与 `eval_mode` blocker：env server 对 `base_train` 有 reset/initialize 日志，Transformers/vLLM checkpoint 加载和 config validation 均通过。
- Plan B retry14 smoke 随后失败，无 `validation/**/300.jsonl` 输出：vLLM 在第一次生成时抛出 `ValueError: You set image=1 (or defaulted to 1) in --limit-mm-per-prompt, but passed 2 image items in the same prompt.`，`srun` exit code 1，manual PID 已退出。失败原因是当前 rollout 的多图 prompt 与 vLLM 默认每 prompt 1 张图限制不匹配；当前 smoke 没有可估算 ETA。
- 已修复 retry14 暴露的多图限制问题：nested `external/VAGEN/verl` commit `cf4f50ad7ec451e17c272309924f2997abc47240` 不再靠 checkpoint path 是否包含 `Qwen2.5-VL` 来决定是否传 `limit_mm_per_prompt`，而是只要 rollout config 有该字段就传给 vLLM；VAGEN pointer commit `a9bec3f6bc8300c9dd45918d928cb35c2bef0724`；Nimloth rollout 脚本显式 override `actor_rollout_ref.rollout.limit_mm_per_prompt=20`，避免默认值不可见。
...
- 已按人类要求立即修复 rollout 收集代码并启动补救：VAGEN `81646ba fix(trainer): persist validation rollout images` 让 legacy-dev `_dump_validation_records` 保存 `image_0/images_<idx>/*.png` 并在 JSONL 写入 `image_paths`；Nimloth `ce6ed48 fix(sft1): support current rollout dump conversion` 让 converter 支持当前 `output_str` schema、plain `think/action` 格式和 action alias。服务器 worktree已同步。临时补救脚本放在 retry15 run dir `replay_tmp/`（不进 repo，补救完删除），通过同 `eval_set/env_seed` 和 `output_str` 中的 action 序列 replay AI2-THOR 来重建图片。2-record smoke job `466848_0` COMPLETED，转换 smoke 通过：2 records, 42 image_paths = 42 placeholders, 40 assistant_turns/actions, 0 validation issues。全量补救 array `466852_[0-7]` 已提交运行，输出到 `validation_replayed/`；完成后再用 converter 生成正式 SFT records。
- 按人类要求，已删除服务器上错误的 retry15 run 数据和临时补救代码：删除 `/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry15`、`sft1_sft_records_highsuccess_step300_retry15*` 无效转换目录、相关 `sft1-r15-*` / `r15-replay-img_*` Slurm logs，以及 `/tmp/validate_replay.py` 等临时检查文件；已 `scancel 466852`，确认无残留匹配目录、无残留匹配 slurm log、无 active `466852`。保留的是 repo 中已提交的修复代码和经验教训文档。
- 按人类要求把 `origin/nimloth/main` 中用于 SFT1/SFT2 的 navigation 修复补进 VAGEN `nimloth/vagen-legacy-dev`：新 VAGEN commit `1a3e99d Port Nimloth SFT navigation fixes to legacy dev` 已 push。内容包括从 main 复制 `base_train/common_sense_train/long_horizon_train`，加入 legacy `ValidEvalSets`；补 `eval_mode` prompt/parser；把 legacy prompt 改为按 `max_actions_per_step<=1` 使用 single-action hints、移除旧 multi-action system examples，并让后续 observation 不再重复 format instruction；把 validation dump 图片持久化为 `image_<step>/images_<idx>/*.png` 并写 `image_paths`。本地验证：`python3 -m py_compile vagen/env/navigation/prompt.py vagen/env/navigation/env.py vagen/env/navigation/env_config.py vagen/env/utils/parse_utils.py vagen/trainer/ppo/ray_trainer.py` 通过；用 importlib mock 验证 eval_mode parser、nimloth 单动作 guard、single-action prompt 不含旧 multi-action hint。当前 Nimloth worktree 的 `external/VAGEN` 已切回 `nimloth/exp/latent-repr-ablation-train-splits`，未更新父 repo submodule pointer。
- retry16 collection 已按人类要求启动：先把 VAGEN `nimloth/vagen-legacy-dev` 的 prompt port 同步到实验分支，VAGEN `nimloth/exp/latent-repr-ablation-train-splits` 现为 `5eb62d58`（含 legacy prompt single-action port、去掉 `Decide your next action(s)` 字样、validation image dump、train split/eval_mode/vLLM 多图修复），Nimloth 父提交 `38d5f51`。服务器 worktree 同步到 Nimloth `38d5f51` / VAGEN `5eb62d58` / verl `cf4f50ad`。新 run dir `/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry16`；env job `466907` 在 `dgx-48` preempt 运行，端口 `19730-19733`，需用 hostname URL `http://dgx-48:19730-19733`（IP URL 会 hang）；smoke job `466921_3` 已运行并进入 `vagen.trainer.main_ppo`/validation，当前无 Traceback/ValueError/RuntimeError/ERROR，正在等待 smoke 完成后做结构验证。已启动 `retry16-monitor` 子 agent 定时检查；只有 smoke 验证通过才允许提交 full array。
...
- 本轮已修改并提交 VAGEN submodule 到专用分支 `nimloth/exp/latent-repr-ablation-train-splits`；后续若继续改 submodule，仍必须在对应 `nimloth/exp/...` 分支上提交和记录。
...
- 服务器 multi-action collection smoke 已完成：Nimloth `22ceb59` / VAGEN `fb7691c` / verl `cf4f50a`，输出 `/project/peilab/atst/nimloth/outputs/experiments/training/sft1/2026-07-08/multi_action_rollout_smoke_22ceb59_retry2`，Slurm job `467931` on `dgx-44` `COMPLETED 0:0`，elapsed `00:02:45`。配置为 train-only `navigation_base_train/base_train` seeds `1..4`，`prompt_format=grounding_worldmodeling`，`max_actions_per_step=5`，checkpoint `/project/peilab/atst/vagen_ckpt/actor/huggingface`，无训练，只 rollout/convert。产物：`validation/train/shard_smoke/0.jsonl`、`converted/train_all.jsonl`、`smoke_summary.json`。summary：records=4，image_paths=16，actions=13，action_groups=6，multi_action_groups=6，max_actions_per_group=3，validation_issues=2。结论：collection/image dump/conversion 机械路径健康，multi-action groups 被保留；但 VAGEN metrics 为 `action_is_valid=0.0` / `action_is_effective=0.0`，因为 strict `grounding_worldmodeling` parser 没执行 malformed-but-action-containing responses（如 raw `<answer>moveahead, moveright</answer>` 但 think tags 不合 strict pattern）。这些 smoke records 可证明 plumbing，不应当作高质量 rollout 数据；若要用该 checkpoint 正式采集，需要先决定是否放宽 parser/execution 或换 prompt。
- 服务器 smoke 前两次失败均已记录到各自输出 README/progress：attempt `467927` 因 batch size 3 不能被 2 GPU 整除；retry1 `467929` 因临时脚本未 `export REPO`，误用 main repo nested verl，导致 `limit_mm_per_prompt` 未传给 vLLM。retry2 已修正。
- 根据人类澄清，step300 这类非 SFT2/VAGEN legacy 模型应使用原本 VAGEN prompt/action 提取，而不是 Nimloth prompt。已新增 rollout/env 配置 `ROLLOUT_ACTION_EXTRACTION_MODE`（默认 `strict`，可设 `answer_fallback`），并在 VAGEN navigation env 增加 `action_extraction_mode=answer_fallback`：当 strict WM/grounding tags 不合格但 `<answer>`/`<action>` 里有动作时仍按 legacy VAGEN answer/action block 提取并执行动作，但不发放 format reward。`rollouts_greedy_parallel.slurm` 的 YAML/parquet env_config 都会透传该字段；legacy multi-action 采集示例改为 `ROLLOUT_PROMPT_FORMAT=grounding_worldmodeling ROLLOUT_MAX_ACTIONS_PER_STEP=5 ROLLOUT_ACTION_EXTRACTION_MODE=answer_fallback`。
- 服务器 `answer_fallback` one-seed smoke 已完成并证明 legacy multi-action answer 能真实驱动环境动作：Nimloth `8af7f2f` / VAGEN `7908040` / verl `cf4f50a`，job `468179` on `dgx-15` `COMPLETED 0:0`，elapsed `00:20:20`，输出 `/project/peilab/atst/nimloth/outputs/experiments/training/sft1/2026-07-08/multi_action_rollout_smoke_answer_fallback_8af7f2f_retry6_swap_gpu_seed1`。配置为单条 `navigation_base_train/base_train` seed 1，`prompt_format=grounding_worldmodeling`、`max_actions_per_step=5`、`action_extraction_mode=answer_fallback`、`max_turns=1`、`max_response_per_turn=64`，checkpoint `/project/peilab/atst/vagen_ckpt/actor/huggingface`，无训练，只 rollout/convert。raw `output_str` 含 legacy `<answer>moveahead,moveright</answer>`，环境反馈记录 `extracted valid action is ['moveahead', 'moveright']`，metrics `action_is_valid=1.0` / `action_is_effective=1.0`，raw image paths=2；converted summary：records=1，actions=2，action_groups=1，multi_action_groups=1，validation_issues=0。结论：fallback 不是只被 converter 恢复，而是能执行 multi-action group；该 smoke 不评估成功率（success=0）。前几个 retry 卡在 validation/env reset；最终通过把 rollout 放 GPU0、env 放第二张 GPU 避免 THOR/vLLM 同卡竞争后完成，后续正式采集应沿用分离 GPU 的设置。
- 已按人类指令启动 step300 legacy multi-action `answer_fallback` 正式采集：输出 `/project/peilab/atst/nimloth/outputs/experiments/training/sft1/2026-07-08/step300_legacy_multi_action_answer_fallback_full_26df7f7`，Nimloth `26df7f7` / VAGEN `7908040` / verl `cf4f50a`，checkpoint `/project/peilab/atst/vagen_ckpt/actor/huggingface`。Slurm env job `468284`，rollout array `468285_[0-3%2]` 依赖 `after:468284`；启动时 env job 因 scheduler priority/resource pending，rollout 因 dependency pending。配置：`prompt_format=grounding_worldmodeling`、`max_actions_per_step=5`、`action_extraction_mode=answer_fallback`、greedy temperature 0、`max_turns=20`、`max_response_per_turn=512`。split 语义已从 VAGEN dataset 文件核实：train 用 `base_train/common_sense_train/long_horizon_train` seeds 1..1080；val-from-train 用 train split seeds 1081..1200；test 用 heldout eval sets seeds 1..60。README/metadata 与 `outputs/experiments/training/sft1/progress.md` 已记录。
- 上述正式采集首轮已失败但可恢复：env job `468284` 在 `dgx-21` 运行 `00:05:26` 后 `FAILED 2:0`，AI2-THOR 4GPU preflight 只有 2/4 GPU 通过，日志为 `ERROR need 4 good GPUs, got 2`；rollout array `468285_0..3` 随后因 `external_env_4gpu/failed` 立即 `FAILED 4:0`，无 JSONL、无 images、无 conversion/SFT。输出 README/metadata/progress 已更新。恢复方式：在 4 张 AI2-THOR-good GPU 的节点/健康 allocation 上重启 env，再对同一输出目录重跑 rollout；最好用 `afterok` dependency 或等 env `ready` 后再提交 rollout，避免 env preflight 失败后消耗 array attempts。
- 按人类指示改用 `dgx-44` 3 张卡恢复：Nimloth `4614934` 让 SFT1 external env 支持 `ENV_SERVICE_COUNT` / `ENV_URL_HOST`；env job `468509` 到 `dgx-44`/`preempt`，`ENV_SERVICE_COUNT=3`、`ENV_URL_HOST=dgx-44`、ports `20530-20532`，env 已 ready；rollout array `468513_[0-3%2]` 启动后取得部分进展，但 env 随后 PREEMPTED。完成 `validation/train/shard_001_180/0.jsonl`（540 rows，8734 image_paths，success mean 0.1259，`action_is_valid` mean 0.5656，`action_is_effective` mean 0.2968）和 `validation/train/shard_361_540/0.jsonl`（540 rows，8576 image_paths，success mean 0.1185，`action_is_valid` mean 0.5703，`action_is_effective` mean 0.2720）。`468513_0/1` 在尝试下一 shard 时遇到 `dgx-44:20530/20531` connection refused，`468513_2/3` 因 stale env ready/URLs 立即失败。当前无 active jobs；缺 train `181_360/541_720/721_900/901_1080`、val、test。远程 README/metadata/progress 已更新；恢复方式是在新 env 上重跑同一输出目录，脚本会跳过已完成 shards。
- 人类同意后曾改用 normal `dgx-18` 4GPU 再次恢复 env：提交 env job `468759`，`ENV_SERVICE_COUNT=4`、`ENV_URL_HOST=dgx-18`、`PORT_BASE=20630`，但该 job 长时间 `PENDING (Priority)`，后来人类同意使用 `dgx-12` preempt 后已取消 `468759`，避免重复 env。
- 当前恢复：env job `468785` 在 `dgx-12`/`preempt` 4GPU 已 ready 后提交 rollout array `468792_[0-3%2]`，但 env 后续再次 PREEMPTED。当前无 active jobs。已完成：train `shard_001_180`、`shard_181_360`、`shard_361_540`、`shard_541_720`、`shard_721_900`，每个 540 rows；val `shard_1081_1200`，360 rows；总 PNG `48421`。缺：train `shard_901_1080`（expected 540 rows）和 test `shard_001_060`（expected 300 rows）。失败原因：`468792_2` 在转向 `shard_901_1080`、`468792_3` 在转向 test 时遇到 dgx-12 env ports connection refused。远程 README/metadata/progress 已更新；下一次恢复同一输出目录即可自动跳过已完成 shards。
