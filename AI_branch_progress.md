...
- 人类确认统一rollout browser必须覆盖K4 joint、SFT1/SFT2、greedy/reference及所有真实环境rollout evaluation，并要求底层永久保存完整action评分、完整MCTS candidates、raw response/CoT和全部真实图片；UI可折叠但不得截断底层。设计见`ai_tasks/evaluation_rollout_browser_design.md`。核心实现已完成：Parent新增capability-aware `nimloth_rollout_audit_v2`、`RolloutTrajectory` adapter、完整证据/PNG archive、batch/global原子hash门禁、静态selector及SFT eval-set/shard合并；`rollout_env.py`对所有非train环境rollout自动产出browser。VAGEN no-concat新增task/raw-response传播、per-turn K4/standard adapter、official reward join，ID183–186配置与main gate均强制每global-step browser。当前VAGEN=`695d6065`、Parent实现commits=`879f342c,4545bdfc,86981970`（最新文档索引`554cffe4`）；本地CPU定向Parent`78 passed`、VAGEN`112 passed,33 subtests`，HTML JavaScript通过`node --check`。superpod SSH持续在banner阶段timeout，尚无固定服务器Python回归或真实1/40-rollout canary；因此代码核心完成但production验收仍待服务器恢复。
