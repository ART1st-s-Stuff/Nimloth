...
- Rollout preflight attempt 1 在模型加载前 exit 143，定位并修复 worktree HOME `.ssh` 初始化与 `pkill -f 'ray::'` 自匹配（`82770ca`）。Attempt 2 修复旧 module entry（`52739b4`）；attempt 3 确认并适配 pinned VAGEN 的内置 `ppo_trainer.yaml` + deterministic parquet + `rollout_manager` API（`3c4de98`）。Attempt 4 已通过 parquet/Hydra/tokenizer/Ray main，在静态 batch divisibility 检查停止（train batch1 不能用于2 GPU）；已改为 train batch2/val batch1。均未生成 JSONL/checkpoint。
