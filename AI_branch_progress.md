...
- 为当前normal资源新增scheduler-selected `3+2+2+1` world8 trainer + 独立第9张env GPU launcher，并在同一allocation内把bounded create+prompt+reset+schema+close设为trainer启动前硬gate；dgx-[18,32,52,54]继续硬排除。commit=`e84933fb42081b940ea35b04999061704b8a7f01`，server launcher tests=`31 passed`、bash syntax和`sbatch --test-only`配置检查通过。
- 新identity ID19=`19_smoke_fsdpdynamic_k8full8192_sft2epoch2step3310_schemav3_fullppo_kl001_fa2_frag3221env1_iter1_b2_visfreeze`，W&B=`rvz46qrl`，job=`478923`。只读复用ID12 complete Epoch2 init，不resume ID18；`base_train` seeds30002/30003、vision/StateProjector freeze、fresh LLM LoRA+WM+Value联合训练、FA2、schema-v3 full-response PPO-old/reference、actor low-var KL0.001。23:15提交后五个het groups均`PENDING/elapsed0/Resources`，尚无allocation、preflight、trajectory、optimizer或真实端到端结论；继续监控且不得因pending替换。
