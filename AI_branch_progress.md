...
- 人类选择修复并重跑。stable identity已在255 lineage VAGEN `192c35a`和旧504 lineage `ef851af`实现：reset保存`env_id->input index`，trainer按identity fail-fast合并，不再位置zip；两lineage测试分别7/8 passed。新增train120自动gate，要求120精确keys、UID/runtime config一致、metrics.success及全部RGB尺寸，root tooling 10 passed。下一步在完整实验配置再次确认后提交两个normal 6GPU任务。详见`ai_tasks/ai_progress/2026-07-19_rollout_resolution255.md`。
