...
- Rollout preflight 已修复 startup、module/config/parquet/service API、val-only invariants（`82770ca`..`3708b14`）。Attempt 7 的两个 FSDP workers 完整加载 source 4 shards并进入 vLLM 0.11，但 sleep CuMemAllocator 拒绝 expandable segments，已局部 unset（`6aaf684`）。Attempt 8 发现 dashboard 18265 落入 Ray worker port range，已改为 8266+task。仍无 JSONL，GPU 已清零。
