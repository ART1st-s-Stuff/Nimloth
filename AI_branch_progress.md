...
- 人类要求评估达到SFT1级速度并选择不打断当前作业。SFT1仅50步/27m12s，当前14,560步，保持目标不可能到半小时。DINO cache经`480899`验证只占step约1–2%。首轮profile`480933`发现trainer缺少pre-epoch1 `model.train()`，GC未激活；未来代码已修复。修正world8 profile`480952`：online+GC5.209s/peak26.42GiB，cached+GC5.116s/25.85GiB，cached+noGC4.500s/56.15GiB，均50步finite；cache约省1.8%，noGC再省12.0%。新增fail-closed cached-noGC config，结合正式job完整epoch1 noGC占用61–64GiB，可供同图片预算H800未来run，约省2–3h。剩余64%瓶颈为backward+跨节点每micro同步；dynamic DDP/no_sync仍需验证。正式job480807旧commit已进入epoch2/GC阶段并继续健康；packed-forward因不等价继续禁用。
