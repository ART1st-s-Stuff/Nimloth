"""使用小数据集 + 预加载 latents 快速验证 semantic align 评估流程。"""
import sys
import time
import json
...
print("=== Small dataset semantic align eval (with pre-loaded latents) ===")
...
with initialize_config_dir(config_dir=str(config_dir), version_base=None):
    ...
    # 创建小 manifest（从原始 manifest 提取前 100 条）
    ...
    # 创建本地小 manifest（使用绝对路径）
    small_manifest_dir = Path("/tmp/semantic_align_test_manifest2")
    ...
    for row in all_rows:
        if "image_path" in row:
            ...
            if not img.startswith("/"):
                row["image_path"] = str(train_manifest / img)
    ...
    # 创建单文件小 latent cache（避免分块目录的 encoder 检查）
    ...
    # 手动 warmup cache（避免 DataLoader 中的重复加载）
    print("Pre-loading latents into dataset...")
    dataset._wm_dataset._latent_cache = small_latents
