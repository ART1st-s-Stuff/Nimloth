...
CoT。只有版本为`vagen_step60_dual_view_conversion_v3`且通过source/audit/converted hash
...
`source_audit.contract_version=vagen_step60_reconstruction_audit_v3`，分开记录不可读source
commit与实际reconstruction base/HEAD/tree/diff/evidence，并只接受逐步reward；v3同时分开保存source package evidence（vLLM 0.8.5.post1）与实际 executable packages（vLLM 0.8.2），禁止用单一triplet覆盖二者；旧v1/v2 audit不得静默解释为v3。
