...
`verl_adapter.py`负责把一个完整多轮episode转换成一个VERL `DataProto` row：完整system/user/assistant/image transcript作为response，sampled thought/action mask1，latent queries、action delimiters、chat/environment scaffold mask0，每轮reward放在对应采样action token，terminal reward加到最后action，并保留mRoPE、multimodal inputs和审计文本。禁止按turn拆row，否则masked-GAE无法把后续turn reward归因给前面turn。ID39/40虽完成actor/critic update和resume mechanics，但后续E0064审计确认其critic因缺Transformers4.55官方checkpoint key转换而全backbone随机初始化，不再作为loaded-critic证据。

`vagen_online_rollout.py`现提供local/service两种严格staged manager：先让vLLM生成完整`<think>...</think>`，再注入k个latent queries和action-start，用八个action token白名单采样一个action；环境只接收source XML，record/replay保存Nimloth response。完整episode由`verl_adapter.py`序列化成一个row，并携带逐turn latent位置、action及WM transition mask。`verl_wm_aux.py`在actor PPO minibatch中从连续turn latent计算StateProjector+WM predictor next-state MSE；VERL worker使用独立DDP WM模块/optimizer。ID44/45已在world8 exact replay直接验证loaded critic、full actor语言+视觉、WM update，以及schema1/inject/k8 sidecar的module/optimizer/scheduler step1→2严格resume；reference保持immutable。Online direct gate已通过真实base_train environment-service preflight、Ray/world8 actor+critic和vLLM TP4 tied-head load，到ID57推进至KV/sampling profile；FlashInfer ninja PATH修复后尚未重跑，因此仍不能称online generation→env→update完成。thought-collapsed SFT/随机WM继续禁止quality结论。
