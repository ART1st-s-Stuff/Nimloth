...
真实 smoke/baseline 还必须通过 `dynamic_fsdp_k8_fragmented_8plus1.slurm` 的 artifact gate；该入口使用8个FSDP trainer GPUs和1个env GPU。仅 CPU mock 不能证明 AI2-THOR 的真实 `obs_str`、dataset task 或 reward 路径。
