...
def finalize_merged_vocab(model, vocab_size: int) -> None:
    ...
    if input_weight.data_ptr() == output_weight.data_ptr():
        raise RuntimeError(
            "merged lm_head unexpectedly shares storage with input embeddings"
        )
...
def restore_saved_untied_embeddings(model, adapter_dir: Path) -> tuple[str, str] | None:
    ...
    with torch.no_grad():
        input_weight.copy_(
            saved_input.to(device=input_weight.device, dtype=input_weight.dtype)
        )
...
def verify_adapter_loaded(model, adapter_dir: Path) -> int:
    ...
    loaded = get_peft_model_state_dict(
    ...
    mismatches = [
        ...
        or value.shape != loaded[key].shape
        or not torch.equal(value, loaded[key].cpu())
    ]
    if mismatches:
...
def copy_query_artifacts(source: Path, destination: Path, *, stage: str) -> None:
    """An exported query backbone keeps the exact trained projector for SFT3."""
    if stage != "query":
        return
    files = ("slot_projector.pt", "grid_state_config.json")
    for name in files:
        if not (source / name).is_file():
            raise FileNotFoundError(f"query checkpoint is missing {name}: {source}")
...
def merge_checkpoint(
    ...
    if training_state_path.is_file():
        ...
        merged.config.nimloth_training_stage = training_state.get(
    ...
    print(
        ...
        f"restored_embeddings={restored_embeddings} -> {out_dir}"
    )
    return 0
