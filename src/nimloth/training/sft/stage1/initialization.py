"""Prepare Stage 1 semantic action initialization without overwriting inputs.
...
def initialize_rows(input_weight, output_weight, row_sources, *, tied: bool):
    ...
    if input_weight.dtype != torch.bfloat16 or output_weight.dtype != torch.bfloat16:
        raise ValueError(
            "Stage 1 semantic initialization requires BF16 input and output matrices"
        )
...
def initialize_model(source: Path, output: Path):
    ...
    manifest = {
        "kind": "sft1_semantic_action_initialization_v1",
        "source": str(source),
...
def main():
    ...
    print(json.dumps(initialize_model(args.source, args.output), indent=2))
