"""SFT2 WM predictor facade (ARPredictor only; no LeWM pixel encoder)."""
...
from nimloth.wm.model import ARPredictor, Embedder, MLP
...
class LatentWMPredictor(nn.Module):
    ...
    def __init__(self, config: LeWMConfig) -> None:
        ...
        self.predictor = ARPredictor(
            ...
            cond_dim=config.action_emb_dim,
    ...
    def load_checkpoint(cls, path: Path, map_location: str | torch.device = "cpu") -> "LatentWMPredictor":
        ...
        jepa_state = torch.load(path / "model.pt", map_location=map_location, weights_only=True)
