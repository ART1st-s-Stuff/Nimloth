...
from nimloth.backbone.dino import DEFAULT_DINO_MODEL
...
def build_sft2_arg_parser(config_path: Path | None = None) -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        default=False,
        help="Directly align projected current query state to frozen DINO CLS features.",
    )
    ap.add_argument(
        "--dino-model",
        default=DEFAULT_DINO_MODEL,
        help="DINOv2/DINOv3 Hugging Face model ID or local checkpoint path (no fallback).",
    )
    ap.add_argument(
        "--dino-feature",
        choices=("cls",),
        default="cls",
        help="Frozen DINO feature used as current-RGB alignment target.",
    )
