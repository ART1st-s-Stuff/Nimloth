...
function actions(t){{const names=audit.action_space?.names||[];const logs=t.action_distribution?.log_probabilities||[];const qs=t.direct_q?.values||[];const roots=t.planner?.root_scores||[];const visits=t.planner?.root_visits||[];const n=Math.max(names.length,logs.length,qs.length,roots.length,t.executed_action.id+1);let rows='';for(let i=0;i<n;i++){{const name=names[i]||('action_'+i);rows+=`<tr class="${{i===t.executed_action.id?'executed':''}}"><td>${{esc(name)}}${{i===t.executed_action.id?' · EXECUTED':''}}</td><td>${{pct(logs[i])}}</td>{'<td>${f(qs[i])}</td>' if capabilities['direct_q'] else ''}{'<td>${f(roots[i])}</td><td>${visits[i]??"—"}</td>' if capabilities['planner'] else ''}</tr>`}}return `<table><thead><tr><th>Action</th><th>Behavior probability</th>{direct_label}{planner_label}</tr></thead><tbody>${{rows}}</tbody></table>`}}
...
audit.turns.forEach((t,i)=>{{const b=document.createElement('button');b.id='nav-'+i;b.textContent=`Step ${{i+1}} · ${{t.executed_action.name}}`;b.onclick=()=>show(i);nav.appendChild(b);const stateValue=t.direct_q?.state_value;const c=document.createElement('section');c.id='card-'+i;c.className='card';c.innerHTML=`<h2>Step ${{i+1}}</h2><div class="top"><div><img class="obs" src="${{esc(t.observation.image)}}"><div class="note">True environment observation before the executed action.</div></div><div><div class="metrics"><div class="metric"><span class="label">Executed action</span><strong>${{esc(t.executed_action.name)}}</strong></div><div class="metric"><span class="label">Environment reward</span><strong>${{f(t.environment.reward)}}</strong></div>${{stateValue==null?'':`<div class="metric"><span class="label">Current state value</span><strong>${{f(stateValue)}}</strong></div>`}}</div><details open><summary>Actual generated CoT</summary><pre>${{esc(t.cot)}}</pre></details></div></div><details open><summary>All available action evidence</summary>${{actions(t)}}</details>${{candidates(t)}}${{t.terminal?`<details open><summary>Terminal observation and CoT · no action executed</summary><img class="obs" style="max-width:520px;margin-top:10px" src="${{esc(t.terminal.observation.image)}}"><pre>${{esc(t.terminal.cot||'Terminal CoT was not captured by this policy.')}}</pre></details>`:''}}`;cards.appendChild(c)}});show(0);
</script></body></html>'''
