...
`nimloth_rollout_audit_v3` adds explicit `model_state` and `mcts_process` capabilities. A complete K4 MCTS turn stores the same-generation `16×2048` latent hidden, the behavior-time projector output `8×1024`, every unique predicted MCTS node state, and all 100 chronological UCT simulations (selection/expansion inputs, leaf action values, scalar leaf value, and backup before/after values). State tensors are stored once per unique node in a hash-bound `.npz`; JSON process records reference those node indices. Legacy v2 audits remain readable and declare both new capabilities unavailable.
