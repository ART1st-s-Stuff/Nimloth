...
| `dino_grid.py` | frozen DINO spatial-grid target 的在线计算、身份校验与只读 cache；`FrozenDINOMultigridTargets`绑定精确processor的224px/native16输出，从同一次teacher pass直接池化4×4/8×8并原样保留16×16，禁止串行跨grid池化或用`model.config.image_size`冒充runtime geometry |
