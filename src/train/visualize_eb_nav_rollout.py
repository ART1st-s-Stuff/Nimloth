...
def _nearest_action_id(gt_vec: torch.Tensor, *, action_dim: int = 3) -> int:
    """Match continuous GT action to closest discrete ACTION_MAP vector."""
    gt = gt_vec.detach().cpu().float().view(-1)[:action_dim]
    best_id = 0
    best_d = float("inf")
...
def _action_vec_tensor(action_id: int, *, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    vec = ACTION_MAP[int(action_id)]
    return torch.tensor(vec[:3], device=device, dtype=dtype).view(1, 1, 3)
    ...
    with torch.no_grad():
        ...
        for step_idx in range(int(z_future.size(1))):
            ...
            for aid in branch_action_ids:
                ta = base_hist.clone()
                av = _action_vec_tensor(aid, device=device, dtype=dtype)
                ta[:, -1, :] = av.squeeze(1)
    ...
    for step_idx in range(len(real_traj)):
        if future_action_ids is not None and step_idx < int(future_action_ids.numel()):
            ...
            gt_action_ids_per_step.append(
                _nearest_action_id(future_actions[0, step_idx], action_dim=3)
            )
...
def _color_for_action(action_id: int) -> tuple[float, float, float, float]:
    """Stable color per action id (tab10)."""
    try:
        cmap = plt.colormaps["tab10"]
    except (AttributeError, KeyError):
        cmap = plt.cm.get_cmap("tab10")
    return cmap(action_id % 10)  # type: ignore[return-value]
...
def _save_action_branch_pca_figure(
    *,
    ...
    n = n_steps
    real_coords = coords_np[:n]
    branch_coords = coords_np[n:].reshape(n_steps, k, 3)
    ...
    for s in range(n_steps):
        ...
        for j, aid in enumerate(branch_action_ids):
            c = _color_for_action(aid)
            is_gt_action = aid == gt_aid
            m = "*" if is_gt_action else "o"
            ms = 110 if is_gt_action else 38
            lab = None
            if aid not in shown_ids:
                short = ACTION_NAMES.get(aid, str(aid))
                if len(short) > 42:
                    short = short[:39] + "..."
                lab = f"pred id={aid} ({short})"
                shown_ids.add(aid)
            ax.scatter(
                branch_coords[s, j, 0],
                branch_coords[s, j, 1],
                ...
                depthshade=True,
                label=lab,
                edgecolors="k" if is_gt_action else "none",
                linewidths=0.6 if is_gt_action else 0.0,
            )
...
def _save_action_branch_umap_figure(
    *,
    ...
    branch_flat = emb[n_steps:].reshape(n_steps, k, 3)
