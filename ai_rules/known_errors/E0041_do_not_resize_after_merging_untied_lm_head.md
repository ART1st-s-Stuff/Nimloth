# E0041：独立 lm_head 合并后禁止再次 resize
...
- 旧 SFT1 epoch5 `hf_merged` 缺少 `lm_head.weight`，adapter 则保存了彼此不同的
  embedding 与 head；SFT2 epoch1 从该损坏 export 初始化后也缺少 head。
- 修复后的真实 epoch5 重合并导出包含 head，精确等于 adapter head，并由
  Transformers 与 vLLM 完整加载。
