...
- max turns `20`, max response tokens `256`; source W&B requirements pin vLLM `0.8.5.post1`, Transformers `4.49.0` and PyTorch `2.6.0`; tokenizer EOS is the only stop boundary, with `ignore_eos=false`, empty custom stop strings and empty custom stop-token IDs.
...
Resume consumes only shards with a valid complete marker whose manifest/hash/counts still match. Reconstruction bumps and validates the persisted contracts explicitly: runtime contract `vagen_step60_reconstruction_runtime_contract_v2`, raw row `vagen_step60_source_trajectory_v2`, shard manifest and COMPLETE marker `vagen_step60_complete_shard_v2`, dual-view conversion manifest `vagen_step60_dual_view_conversion_v2`, and rejection-sidecar envelope `vagen_step60_rejections_v2`. SFT2 remains structurally `nimloth_trajectory_v1` but its `source_audit.contract_version` is `vagen_step60_reconstruction_audit_v2`; SFT1 source-audit payload uses the same version. Each carries unavailable source commit, reconstruction base/HEAD/tree/diff/manifest hashes and `step_rewards`. Existing v1 raw rows, runtime/shard/COMPLETE manifests, conversion manifests, rejection sidecars or overloaded `source_runtime_commit` fields are rejected by the reconstruction path and never silently migrated. The deterministic partition manifest remains v1 because its source/split semantics do not change.
