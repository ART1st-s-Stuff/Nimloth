...
- [ ] [W-009] **Complete dual-repository review, commit gates and remote preflight**
  - Preserve the already approved/pushed VAGEN commit `170a673...`; W-012 does not modify VAGEN. Treat pushed Nimloth `a54ae97...` as pre-W-012 and not launchable.
  - After W-012, run `trellis-check` across the complete Nimloth v3/package-provenance diff and all selected known errors; obtain a separate approval for a new Nimloth commit, then a separate exact push approval for `HEAD:refs/heads/task/rollout-vagen-step60-sft1-sft2`. Do not change `origin/dev`, live server checkouts or the Nimloth gitlink.
  - Refresh the existing clean remote Nimloth worktree to the newly approved detached commit only after its push, while retaining VAGEN worktree `170a673...`. Reassert top-level/HEAD/common-dir/controller registration and recursive approved submodules; then run remote CPU affected suites with canonical `.venv`, verify actual 0.8.2 package/import/EOS provenance, source hashes, reconstruction manifest, checkpoint merge/load plan and output nonexistence. Cleanup, if later requested, uses reviewed non-force exact-path removal only.
