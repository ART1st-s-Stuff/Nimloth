...
- Source W&B requirements 记录 vLLM `0.8.5.post1`、Transformers `4.49.0`、PyTorch `2.6.0`，但 source conda 无权限；可访问 canonical `.venv` 为 Transformers `4.49.0` / PyTorch `2.6.0` / vLLM `0.8.2`。用户明确选择 reconstruction rollout 直接使用该 vLLM `0.8.2`，不安装 overlay。报告必须把 backend patch-version drift 作为限制；正式 smoke 必须验证实际 SamplingParams、EOS/stop evidence、prompt tokenization和一条真实response，不能声称复现 source vLLM package identity。
...
## Latest pilot override (2026-09-06)
For the user-authorized200-row trial, [wm200 launch contract](research/wm200-launch-contract-20260906.md) supersedes older prompt,dynamics,sampling,topology,and fullbatch launch directions. Only selectedtrain base100/common100 are run. Existing2000-row partition remains unchanged.
