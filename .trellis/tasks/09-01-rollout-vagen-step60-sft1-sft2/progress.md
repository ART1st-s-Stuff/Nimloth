...
- The first approved launch attempt stopped at the local pre-submit resource parser before any remote mutation or Slurm submission: the fixed-width table separates GPU total into its own token, so the reviewed AWK column indexes rejected a genuinely eligible snapshot. No run root, merge, model load, GPU, job, service or rollout existed; no experiment end record was required. The contract now uses an explicit Python parser over the repository query format and requires fresh review/commit/push/launch approval.
