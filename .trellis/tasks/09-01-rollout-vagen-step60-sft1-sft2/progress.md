...
- R4 is invalid and non-reusable. Its partition/runtime/merged actor are evidence only; task constraints prohibit silently reusing this output. Before another launch, the contract must persist the smoke-step wrapper and srun stderr/accounting so any scheduler refusal is diagnosable, then use a fresh run identity and approval.
