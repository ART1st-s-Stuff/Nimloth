...
- Added an exact candidate contract for a first `normal` one-node/four-GPU, three-hour actor merge + source-index-0 smoke stage. It explicitly excludes the 100-row and formal batch1 stages. No package installation, model load, GPU, Slurm, service, smoke or rollout has occurred.
