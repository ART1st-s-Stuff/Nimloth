...
- [x] 建立独立 child-task worktree/branch，核验基点包含已提交的 Stage 1 统一评估入口，保留并发 `external/le-wm` 状态。
- [x] 修改 Stage 1 加权 CE：八个动作编号权重 8，边界/EOS/其余为 1；新增明确 objective/checkpoint 身份并拒绝旧目标恢复。
- [x] 将训练期 Stage 1 格式评估改为 token 终止检查加共享严格正文解析；保留 Stage 2 query 专用行为。
...
- 成功子集为空、轨迹缺少合法动作标签或数据/划分证据不完整时，不启动 GPU 训练；动作类别零计数只记录，不作为停止条件。
