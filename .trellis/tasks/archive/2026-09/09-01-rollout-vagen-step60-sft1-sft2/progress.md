...
- Retained launcher script/log/PID SHA256 values are `afaa97add79c8827f7388db5cf4e052c850ea553ad31a7fdfdbe9286e56ab99e`, `3364c0f2492252dc5331ac3a1bb0b4109c08d52d47b08f3caf22f1e62b0e87e4`, and `9840ee78fe85ce8bff7c6c99ae945c5b1ee3b22cbfc41c9f5f1a892dd3a9484e`. R13 was superseded without remote execution when the human directed reuse of the existing rollout script.

### 2026-09-05 direct existing-script smoke submission

- Per human direction, stopped custom launcher iteration and directly submitted existing `experiments/training/sft1/rollout_full_6gpu_preempt.slurm` (SHA256 `63f6fce59134a1d0a8edca2be5abca3817edf488190d0f68e4121a9397c983ed`) with its existing child scripts hash-bound. R8's merged actor was fully revalidated immediately beforehand: merge manifest `c1f4d00605c430b02a0212bdb2457fa64e87143bdc2fde6f3f8de1f8ab59699e`, artifact manifest `f5a4a68a70de0f05a682264cc6b1cce5fb6f7b75fc67b08c67061b11fe23983b`.
- Direct Slurm job `546962` is `PENDING (Priority)` on `normal`, excluding `dgx-51`, requesting the existing script's one-node 6 GPU / 224 CPU / 180G / 8h resources. It is configured as one-row smoke with `base` seed `27670`, matching partition source index 0, four environment GPUs and two policy GPUs.
- This is explicitly an existing-pipeline compatibility smoke, not the final v3 terminal-CoT SFT1/SFT2 dataset. No custom hold job or SSH-bound launcher is involved; status is monitored directly by job ID.
- At the human's explicit request, job `546962` was cancelled while still `PENDING`; final `sacct` state is `CANCELLED by 3738`, elapsed `00:00:00`, no node assigned, and the run root remained absent. Therefore it produced no trajectory, terminal CoT, image or dataset artifact and requires no output cleanup.
- The human then required frequent durable saving and interruption resume. W-015 now owns per-trajectory atomic/fsynced checkpoints and explicit fail-closed interrupted-shard resume; no new GPU launch is authorized by that implementation scope.

### 2026-09-05 W-015 per-trajectory resume implementation

- Implemented in the clean task worktree: stable `<shard>.inprogress` identity, explicit fresh/`--resume` separation, crash-released exclusive collection lock, attempt-unique environment/image namespaces, stable record IDs, one atomic no-overwrite + file/directory-fsynced checkpoint per fully finalized trajectory, strict record/runtime/spec/image validation before environment or GPU activation, unfinished-row restart, deterministic final raw order, and no-overwrite/idempotent finalization artifacts.
- Resume accepts only complete trajectories that include the real terminal generation with `executed=false` and `environment_step_after_generation=false`; half trajectories are not reconstructed. Interrupted attempts and unreferenced image evidence remain preserved. Final raw/shard/conversion format identities remain v3.
- Review-found blockers were fixed: runtime activation ordering, concurrent resume ownership, checkpoint race, symlink ancestors, stale service-ID collision, staging-parent durability and non-obscuring failure diagnostics. Regression coverage observes checkpoint file fsync → exclusive publication → records-directory fsync before later rollout progress.
- Verification: `tests/training/sft1` **124 passed**; Ruff, Python compileall, shell syntax, Trellis task validation and `git diff --check` all passed. Real NFSv3 `flock`/hard-link/fsync and vLLM GPU resume behavior remain launch-preflight gates; no remote/GPU operation was performed.

### 2026-09-05 W-016 eight-GPU preempt orchestration

- The human selected maximum utilization of one eight-GPU preempt node: four isolated environment GPUs plus four independent TP1 policy collectors. Implemented a thin Slurm gate orchestrator that performs source-index-0 smoke with `fail_shard`/concurrency 1, validates it, then runs exactly four 100-row shards in parallel with `exclude_trajectory` and production concurrency.
- Fresh/resume run-root ownership is durable and hash-bound; ordinary failures publish `NON_RESUMABLE.json`, while only an actually observed TERM/INT interruption may resume. Output classification and one expensive actor/runtime/partition inspection occur before service/GPU activation and are transferred through an atomic SHA-bound handoff. Complete shards are independently validated/skipped; matching in-progress shards use explicit `--resume`.
- The orchestrator binds a clean exact Nimloth commit, reconstruction identity, actor/runtime/partition paths, separate smoke/gate semantics, disjoint GPU roles, attempt-unique logs/control and process groups. The existing environment entrypoint now honors exact `VAGEN_DIR` and retains the caller-selected `PYTHON_ENV`.
- Review iterations fixed executable-mode mismatch, failed-root reuse, descendant and smoke process-group cleanup, repeated 19GB actor inspection, signal provenance for rc143 and classification error propagation. Final affected verification: `tests/training/sft1` **148 passed**; Ruff, shell syntax, compileall, task validation and `git diff --check` passed. No remote/GPU launch occurred; real preempt/NFS/process-group/TP1 resource behavior remains launch-preflight evidence.

### 2026-09-10 a100 original-validation batch1 SFT1 conversion

- Under the later human-selected original-validation contract, a100-1 completed the exact batch1 identity set: 1,280 previously synchronized records plus 720 newly collected unique records. The prepared 1,800-row remainder is fully covered and its 200-row complement is the balanced training pilot, proving the intended 1,800 train + 200 internal-heldout source split. Two duplicate files were ignored by a retained deterministic selection sidecar.
- The source format is `original_validation_row_v1`. It contains real ordinary CoT, executed actions and aligned 512x512 RGB images, but its final assistant entry is an empty generation prompt rather than a terminal response. The resulting artifact is therefore explicitly SFT1-only and is not accepted as SFT2 terminal-state evidence.
- Published `/mnt/nimloth/outputs/datasets/sft1-vagen-step60/20260910T093222Z_batch1_original_validation_k16` on a100-1 without copying or modifying source images. Strict K16 conversion retained 1,902 trajectories and excluded 98 whole trajectories whose ordinary source response was malformed: train-all 1,709, train-success 1,149, heldout-all 193. Valid plus excluded closes exactly over 2,000 source identities.
- Independent validation rehashed the manifest, every output, every referenced image and every converted record; it checked K16/action mapping, message/action/image alignment, train/heldout disjointness and 2,000-row closure. Status is `VALID`; conversion-manifest file SHA256 `f45d4207f28d6ecee623f3f919d50e18a32987b9f962344e1553ef91337a71f9`, payload SHA256 `602f0e28bf6af7aa513b02d0334c276b6bb1b8e3e9effb094ab8280bc736f678`, and sibling validation evidence SHA256 `7c74f707c66c8bd699b49095b8f194f9b1b6c4c0ce702be98e86df4edf72a097`.

### 2026-09-10 a100 SFT1 launch

- Human explicitly requested starting SFT stage 1. On non-Slurm host `a100-1` (`n30191`), all eight A100-SXM4-40GB GPUs were idle and 495 GiB host memory was available immediately before launch.
- A real one-train/one-heldout CPU preprocessing gate passed with images, K16, `latent_query_mode=generate`, unmasked latent-query labels and BF16 pixel cache; the temporary gate cache was removed.
- Detached controller PID `518704` started at `2026-09-10T09:48:44Z`. It is building the complete CPU preprocessing cache before activating GPUs, then will run one-node 8-GPU DDP LoRA training for 20 epochs from `/mnt/nimloth/checkpoint/hf_actor`.
- Exact inputs are train-all 1,709 records SHA256 `af1f8d11a52279051d0deea96db81f3522de7bf556b089f921943d832e1224e6` and internal-heldout 193 records SHA256 `1632d9aebbe499076fe1d9477fa83c2dbfbadea593189ece640603bdad0115dd`. `train_success` remains an ablation and is not consumed by this run.
- Training contract: Nimloth commit `1e3b81a39df9e6cd4ecfb0757e56e2d7993b952f`, Python `/mnt/nimloth/venv/bin/python3`, world size 8, per-rank batch 1, gradient accumulation 8, effective batch 64, LoRA r64/alpha128, LR `2e-4`, embedding/LM-head LR `5e-4`, max length 20,000, max pixels 602,112, K16 generate mode, seed 42, and epoch-checkpoint resume within this unique run only.
- Run output: `/mnt/nimloth/outputs/experiments/training/sft1/2026-09-10/20260910T094500Z_step60_batch1_sft1_k16_all_lora`. External controller log/PID/script are under `/mnt/nimloth/logs/` with the same run ID; `launch_contract.json` and the exact launch script are preserved in the run output. At `09:49:12Z`, the controller and eight cache workers were healthy with 32/1,902 cache tensors present; GPUs remained intentionally idle during caching.

### 2026-09-10 a100 SFT1 first launch terminal failure

- The full BF16 preprocessing cache completed at `2026-09-10T10:11:29Z`: 1,709 train and 193 heldout tensors, about 64 GiB total. The detached controller then entered 8-GPU DDP training and terminated at `10:12:18Z` with exit code 1; `FAILED.json` confirms the terminal state and no epoch checkpoint was produced.
- The confirmed failure is CUDA OOM in Qwen2.5-VL self-attention before the first training step. Rank 3 attempted a 28.93 GiB allocation and rank 4 attempted 40.05 GiB while each GPU already held about 13--14 GiB. Cached token lengths span 3,224--18,065, so SDPA's quadratic attention allocation at the longest samples exceeds a 40GB A100 even with gradient checkpointing.
- This is not cache corruption or general GPU contention: all 1,902 cache records completed, all GPUs were otherwise idle, and memory was released after the distributed process stopped. The installed runtime has `flash_attn 2.7.4.post1`; a subsequent launch may reuse the complete hash-bound cache and test `--attn-implementation flash_attention_2`, but this failed run is terminal and must not be presented as having trained or resumed from a checkpoint.
