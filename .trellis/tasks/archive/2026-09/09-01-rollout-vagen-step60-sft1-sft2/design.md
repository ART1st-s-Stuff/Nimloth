...
Batch1 is split into bounded complete shards (final shard size selected in the exact launch contract; target 100 rows). Collection uses a stable direct-sibling `<shard>.inprogress` staging directory whose immutable `IN_PROGRESS.json` binds run/shard identity, ordered episode specs, max steps, format policy, source runtime, policy artifact/runtime, and their canonical payload hash. A fresh launch refuses an existing staging path; `--resume` requires it and refuses to create a new attempt silently.

Each trajectory becomes resumable only after its ordinary rollout, final observation and real terminal generation are complete. The collector saves its images under an attempt-unique namespace, atomically writes and fsyncs one hash-bound record checkpoint, then fsyncs the checkpoint directory before considering that source row complete. Resume verifies every checkpoint's schema/hash, exact spec identity, runtime/policy bindings and all referenced image sizes/hashes before policy or environment calls. Verified completed source rows are skipped regardless of completion order; an unfinished row restarts from its original eval-set/seed. Half-trajectory state is never reconstructed. Interrupted attempt namespaces and unreferenced images remain audit evidence and are not interpreted as completed rows.

After all expected record checkpoints validate, finalization deterministically emits `raw.jsonl` in the original requested spec order, creates the existing manifest and `COMPLETE` marker, and uses marker-last publication. A completed final shard remains non-resumable and immutable. Each shard owns a unique directory containing:
...
Batch-level resume consumes complete shards whose manifest/hash/counts still match; within one interrupted shard, explicit collector resume consumes only independently validated per-trajectory checkpoints from its stable in-progress staging directory. The human-selected backend drift introduces explicit v3 contracts before any real rollout was collected: runtime contract `vagen_step60_reconstruction_runtime_contract_v3`, raw row `vagen_step60_source_trajectory_v3`, shard manifest and COMPLETE marker `vagen_step60_complete_shard_v3`, dual-view conversion manifest `vagen_step60_dual_view_conversion_v3`, and rejection-sidecar envelope `vagen_step60_rejections_v3`. SFT2 remains structurally `nimloth_trajectory_v1` but its `source_audit.contract_version` is `vagen_step60_reconstruction_audit_v3`; SFT1 source-audit payload uses the same version.
...
A non-empty JSONL alone is never resumable. A partial shard is resumable only through its identity-bound staging contract and fully validated per-trajectory checkpoints; corrupt or half-written state fails closed. Each restarted unfinished trajectory writes to a new attempt namespace, preserving prior partial evidence. A changed contract or failed staging validation requires a new shard attempt directory rather than mutation of the old evidence.
...
The human subsequently selected maximum use of one available eight-GPU `preempt` node. The reviewed target topology is four isolated single-GPU environment services plus four independent single-GPU (`tensor_parallel_size=1`) policy collectors. This matches the source run's four-replica TP1 structure more closely than the abandoned TP2 smoke topology and avoids sharing an environment service across collectors. A one-row source-index-0 TP1 smoke briefly uses one environment/policy pair; only after strict smoke validation may the same allocation start four independent 100-row shard collectors in parallel, one pair per shard. Each output has its own stable staging/lock/checkpoints, and a preempted rerun explicitly validates/skips complete shards and passes `--resume` only for matching in-progress shards. CPU count, memory, walltime, exact device binding and command remain launch-gated and are revalidated against live Slurm availability immediately before submission.
