...
SBATCH_ARGS=(
  --parsable
  --export=ALL,SFT1_TUNE_MODE="${SFT1_TUNE_MODE}",WANDB_RUN_NAME="${WANDB_RUN_NAME}",LATENT_TOKEN_COUNT="${LATENT_TOKEN_COUNT}",MASK_LATENT_QUERY_LABELS="${MASK_LATENT_QUERY_LABELS}",REQUIRE_PREBUILT_CACHE="${REQUIRE_PREBUILT_CACHE:-0}"
)
if [ -n "${SFT1_DEPENDENCY:-}" ]; then
  SBATCH_ARGS+=(--dependency="${SFT1_DEPENDENCY}")
fi
if [ -n "${NODELIST}" ]; then
