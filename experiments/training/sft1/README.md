...
`original_validation200.py` verifies/prepares the balanced 200-row pilot and summarizes original validation dumps. `run_original_validation200.slurm` uses array 0–2%3: each member has its own Ray head, local environment and TP2 policy sharing two GPUs, with one node, 28 CPUs, 128G, six hours and no requeue. Original sampling, context, validation batch size 1 and unused training fields 32/16 remain; world size and padding divisor are 2. No training runs.

The controller prepares disjoint `SHARDED_DIR/shard_0.parquet` through `shard_2.parquet` (67/67/66 original-ID rows), retains the full `PREPARED_DIR` manifest and creates `RUN_OUT` first. Members exclusively create `RUN_OUT/node_<id>` for private runtime/logs and dump globally unique rows directly into shared `RUN_OUT/rollouts`. Each checks its expected row identities before writing private `node_done.flag`; only the controller summarizes all 200 after all members complete. Preflight checks actual HF attention/KV head divisibility by TP2. Changed topology does not guarantee bitwise reproduction of TP4 sampling. See the task research launch contract for provenance and limits.

`prepare_source200.py --selection batch1_remainder --prompt-format grounding_worldmodeling` prepares exactly 90 ordered shards of 20 rows: the 1,600 batch1 train rows outside the pilot plus all 200 batch1 heldout rows. This profile fixes `step_length=0.5`, `success_threshold=1.5`, `format_reward=0.02`, `invalid_action_penalty=-0.2`, `max_actions_per_step=1`, and `use_state_reward=False`, and removes `success_reward`.

`run_original_validation_batch1_remainder.slurm` runs those shards as array `0-89%8`. Every member has one node, two GPUs, 28 CPUs, 128G, a private Ray head/environment/cache, and a default 90-minute limit. The controller must use a new empty run root distinct from the current 200 run, create `RUN_OUT` before submission, and export `PREPARED_REMAINDER_DIR`; it must wait for all 90 `shard_XX/shard_done.flag` files before any aggregate summary. Members write globally unique source-index directories into shared `RUN_OUT/rollouts`; `record.json` is the atomic readiness marker. Array completion alone is not a valid summary or success-rate result.

`finalize_original_validation_batch1.py` takes the authoritative `--partition-manifest`, the current `--pilot-manifest` and `--pilot-run-out`, the remainder `--remainder-manifest`, and one or more `--run-out` remainder roots (including retry roots). Each completed pilot node and remainder shard must also contain `original_validation_sampling_audit_v1` as `sampling_audit.json`; the finalizer requires TP2 ranks `[0,1]`, vLLM `0.8.5.post1`, and sampling `do_sample=true`, temperature `0.7`, top-p `0.95`, top-k `-1`, n `1`, max tokens `256`, seed `null`. It rejects duplicate records or completed shard identities across roots, reuses the strict original-validation record/image validator, proves exact authoritative batch1 coverage, and writes overall, split, category, split-by-category rates plus artifact hashes using exclusive `--output` creation.
