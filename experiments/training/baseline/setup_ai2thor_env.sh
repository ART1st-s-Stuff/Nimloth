...
if [[ -f "${VULKAN_LIB_DIR}/libvulkan.so.1" ]]; then
  ...
  SYSTEM_VULKAN_LIB=${SYSTEM_VULKAN_LIB:-/lib/x86_64-linux-gnu/libvulkan.so.1}
  SYSTEM_VULKAN_ICD=${SYSTEM_VULKAN_ICD:-/usr/share/vulkan/icd.d/nvidia_icd.json}
  [[ -f "${SYSTEM_VULKAN_LIB}" && -f "${SYSTEM_VULKAN_ICD}" ]] || {
    echo "[setup_ai2thor_env] Missing both the local and system NVIDIA Vulkan runtime" >&2
    exit 1
  }
  export VK_ICD_FILENAMES="${SYSTEM_VULKAN_ICD}"
  VULKAN_SOURCE=system
fi
