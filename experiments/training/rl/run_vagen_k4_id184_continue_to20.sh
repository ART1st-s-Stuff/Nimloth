...
: "${ID184_HEAD_IP:?ID184_HEAD_IP is required}"
: "${ID184_EXPECTED_NNODES:?ID184_EXPECTED_NNODES is required}"
: "${ID184_EXPECTED_GPUS_PER_NODE:?ID184_EXPECTED_GPUS_PER_NODE is required}"
: "${ID184_CLUSTER_NODES:?ID184_CLUSTER_NODES is required}"
...
ENV_URL=http://${ID184_HEAD_IP}:${ENV_PORT}
RUNTIME_ROOT=/tmp/i184-${SLURM_JOB_ID}-${PHASE_TAG}
...
[[ "${ID184_EXPECTED_GPUS_PER_NODE}" == 2 ]]
...
IFS=, read -r -a CLUSTER_NODES <<<"${ID184_CLUSTER_NODES}"
(( ${#CLUSTER_NODES[@]} == 4 ))
...
(( ${#EXPECTED_NODE_IPS[@]} == 4 ))
[[ "${ID184_HEAD_IP}" == "${EXPECTED_NODE_IPS[0]}" ]]
[[ "${RAY_ADDRESS}" == "${ID184_HEAD_IP}:"* ]]
...
print(json.dumps({'status':'ID184_RAY_4X2_OK','nodes':rows}))
PY
...
try:
 ...
 raise RuntimeError(
  f'ID184 W&B identity already exists: {path} '
  f'name={run.name!r} state={run.state!r}'
 )
PY
)

[[ ! -e "${RUN_OUT}" ]] || { echo "ID184 output already exists" >&2; exit 2; }
[[ -f "${SOURCE_CHECKPOINT}/joint_checkpoint_complete.json" ]]
[[ -f "${SOURCE_CHECKPOINT}/data.pt" ]]
[[ -f "${ID183_SOURCE_RUN_OUT}/final_status.json" ]]
[[ -f "${ID183_SOURCE_RUN_OUT}/phase2_fresh_resume_to_10/validator.json" ]]
[[ -f "${ID183_SOURCE_RUN_OUT}/phase2_fresh_resume_to_10/wandb_final.json" ]]
"${PY}" - "${SOURCE_CHECKPOINT}" "${ID183_SOURCE_RUN_OUT}" <<'PY'
...
validator=json.loads((run/'phase2_fresh_resume_to_10/validator.json').read_text())
wandb=json.loads((run/'phase2_fresh_resume_to_10/wandb_final.json').read_text())
assert marker['global_step']==10 and marker['source_step']==786
assert final['status']=='passed' and final['global_step']==10
assert validator['status']=='ALL_OK' and validator['checkpoint_steps']==[5,10]
assert wandb['state']=='finished' and wandb['steps']==list(range(11))
print(json.dumps({'status':'ID184_SOURCE_STEP10_OK','marker':marker}))
PY
mkdir -p "${RUN_OUT}" "${CHECKPOINT_DIR}" "${PHASE_OUT}" "${RUNTIME_ROOT}" "${RAY_TMPDIR}" "${TMPDIR}" "${AI2THOR_HOME_ROOT}/.ai2thor"
...
payload={"phase":"resume_10_to_20","exit_code":status,"status":"passed" if status==0 else "failed","finished_at":datetime.now(timezone.utc).isoformat()}
...
PY
  [[ "${RUNTIME_ROOT}" == /tmp/i184-* ]] && rm -rf -- "${RUNTIME_ROOT}"
  exit "${status}"
}
trap cleanup EXIT
...
for name in ('train','val'):
 src=vagen/'examples/train/navigation'/f'{name}_navigation_joint_id184.yaml'
 text=src.read_text()
 ...
 dst=out/f'{name}_navigation_joint_id184.yaml'
...
(out/'source_hashes.json').write_text(json.dumps({
 'train_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/train_navigation_joint_id184.yaml').read_bytes()).hexdigest(),
 'val_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/val_navigation_joint_id184.yaml').read_bytes()).hexdigest(),
 'heldout_train_scene_overlap':0,
 'assets':assets,
},indent=2)+'\n')
PY
export ID184_TRAIN_CONFIG=${PHASE_OUT}/train_navigation_joint_id184.yaml
export ID184_VAL_CONFIG=${PHASE_OUT}/val_navigation_joint_id184.yaml
export ID184_ACTOR_MODEL=${ACTOR_MODEL}
export ID184_PLANNING_CHECKPOINT=${PLANNING_MODEL}
export ID184_AGENT_CONFIG=${VAGEN}/vagen/configs/agent_no_concat.yaml
export ID184_RUN_NAME=${RUN_NAME}
export ID184_RUN_OUT=${RUN_OUT}
export ID184_SOURCE_CHECKPOINT=${SOURCE_CHECKPOINT}

"${PY}" - "${ID184_TRAIN_CONFIG}" "${ID184_VAL_CONFIG}" "${PHASE_OUT}" <<'PY'
import json, sys
...
expected_val_sources={
 'navigation_base_val_id184',
 'navigation_common_sense_val_id184',
 'navigation_long_horizon_val_id184',
 'navigation_complex_instruction_val_id184',
 'navigation_visual_appearance_val_id184',
}
...
# ID184 K4 Scheme-B continuation from step10 to step20

- project/run: vagen / ${RUN_NAME}
- approval: resume the complete ID183 step10/source786 state, reset only the dataloader/sampler for an expanded deterministic train pool, execute updates11--20, and validate/save every five updates.
- parent/VAGEN/VERL: ${EXPECTED_PARENT_COMMIT} / ${EXPECTED_VAGEN_COMMIT} / ${EXPECTED_VERL_COMMIT}
- source checkpoint: ${SOURCE_CHECKPOINT}; actor, optimizer, scheduler, rank RNG, joint projector/predictor/ValueHead optimizer, active frozen snapshot, and global step restore exactly. Only the old dataloader cursor resets because the dataset identity changed.
- training data: base_train, common_sense_train and long_horizon_train; 60 unique tasks per split from inclusive seeds [0,1199], 180-task pool total. Deterministic sampler seed42184; each update remains 24 trajectories capped at20 real actions.
...
"${PY}" -m vagen.envs.navigation.serve --host="${ID184_HEAD_IP}" --port="${ENV_PORT}" --devices='[0]' --max_envs=40 --max_inflight=40 --thread_pool_size=40 --session_timeout=14400 >"${PHASE_OUT}/env_server.log" 2>&1 &
ENV_PID=$!
...
for split in base_train common_sense_train long_horizon_train base common_sense long_horizon complex_instruction visual_appearance; do
  timeout --signal=TERM --kill-after=10s 300s "${PY}" -m nimloth.environment.navigation.prewarm --env-url "${ENV_URL}" --eval-set "${split}" --seed 0 --timeout-seconds 300 --env-id "id184-prewarm-${split}-${SLURM_JOB_ID}" | tee "${PHASE_OUT}/prewarm_${split}.json"
done
...
COMMAND=(
  ...
  --config-name=joint_id184_continue
  ...
  trainer.test_freq=5
  trainer.save_freq=5
  trainer.joint_dataloader_resume_policy=reset
)
...
"${PY}" - "${WANDB_ENTITY}" "${WANDB_PROJECT}" "${WANDB_RUN_ID}" "${RUN_NAME}" "${PHASE_OUT}" <<'PY'
import json, os, sys, tempfile, time, wandb
from pathlib import Path
entity,project,run_id,run_name,out_raw=sys.argv[1:]
out=Path(out_raw); path=f'{entity}/{project}/{run_id}'; expected=list(range(10,21))
last=None
for _ in range(13):
 run=wandb.Api().run(path)
 ...
 if run.name==run_name and run.state=='finished' and steps==list(range(10,21)):
  break
 time.sleep(10)
else:
 raise RuntimeError(f'ID184 W&B completion mismatch: {last!r}')
...
EXPECTED_SOURCE=796
"${PY}" - "${SOURCE_CHECKPOINT}" "${CHECKPOINT_DIR}" "${PHASE_OUT}" "${RUN_OUT}/planning_snapshots" "${RUN_OUT}" <<'PY'
import json, os, sys, tempfile
from pathlib import Path
import torch
...
snapshots=Path(sys.argv[4]); run_out=Path(sys.argv[5])
...
source_payload,source_actor,source_owner=checkpoint_payload(source_path,10)
step15_payload,step15_actor,step15_owner=checkpoint_payload(root/'global_step_15',15)
payload,actor,owner=checkpoint_payload(root/'global_step_20',20)
assert source_actor['source_step']==786 and source_owner['activation_version']==10
assert step15_actor['source_step']==791 and step15_owner['activation_version']==15
assert actor['source_step']==796 and owner['activation_version']==20
assert payload['training_contract_id']!=source_payload['training_contract_id']
actual_checkpoint_steps={
 int(path.name.removeprefix('global_step_'))
 for path in root.glob('global_step_*') if path.is_dir()
}
assert actual_checkpoint_steps=={15,20}
log=(out/'train.log').read_text()
assert 'Setting global step to 10' in log
assert 'ID184_K4_CONTINUE_RESUME_OK global_step=10' in log
assert 'ID184_DATALOADER_RESET_OK global_step=10' in log
assert 'ID184_TRAINING_CONTRACT_PATH_MIGRATION_OK' in log
initial_path=Path(source_owner['active_snapshot_state']['transport_path'])
assert initial_path.parent.name=='source_step_786'
updated_path=snapshots/'source_step_796/frozen_k4_planner.pt'
...
assert initial.source_step==786 and updated.source_step==796
assert initial.snapshot_id==source_actor['snapshot_id']
...
for validation_step in sorted(expected_validation_steps):
 rows=[
  json.loads(line)
  for line in (run_out/'validation'/f'{validation_step}.jsonl').read_text().splitlines()
  if line
 ]
...
summary={
 'status':'ALL_OK','phase':'resume_10_to_20','global_step':20,
