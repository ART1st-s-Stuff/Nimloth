...
RUNTIME_ROOT=/tmp/id167-${SLURM_JOB_ID}-${PHASE_NAME}
...
if [[ "${PHASE}" == update_1 ]]; then
  [[ ! -e "${RUN_OUT}" ]] || { echo "ID167 output already exists" >&2; exit 2; }
...
export WANDB_RUN_ID=nimloth-id167-joint-update-gate
...
PY
  [[ "${RUNTIME_ROOT}" == /tmp/id167-* ]] && rm -rf -- "${RUNTIME_ROOT}"
  exit "${status}"
}
trap cleanup EXIT
...
for name in ('train','val'):
 src=vagen/'examples/train/navigation'/f'{name}_navigation_joint_id167.yaml'
 text=src.read_text()
 assert 'http://127.0.0.1:8000' in text
 dst=out/f'{name}_navigation_joint_id167.yaml'
...
(out/'source_hashes.json').write_text(json.dumps({
 'train_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/train_navigation_joint_id167.yaml').read_bytes()).hexdigest(),
 'val_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/val_navigation_joint_id167.yaml').read_bytes()).hexdigest(),
...
PY
export ID167_TRAIN_CONFIG=${PHASE_OUT}/train_navigation_joint_id167.yaml
export ID167_VAL_CONFIG=${PHASE_OUT}/val_navigation_joint_id167.yaml
export ID167_MODEL=${MODEL}
export ID167_AGENT_CONFIG=${VAGEN}/vagen/configs/agent_no_concat.yaml
export ID167_RUN_NAME=${RUN_NAME}
export ID167_RUN_OUT=${RUN_OUT}

if [[ "${PHASE}" == update_1 ]]; then
  cat >"${RUN_OUT}/README.md" <<EOF
# ID167 DP8 joint-update and exact-resume smoke
...
timeout --signal=TERM --kill-after=10s 300s "${PY}" -m nimloth.environment.navigation.prewarm --env-url "${ENV_URL}" --eval-set base_train --seed 0 --timeout-seconds 300 --env-id "id167-prewarm-${PHASE}-${SLURM_JOB_ID}" | tee "${PHASE_OUT}/prewarm.json"
...
COMMAND=("${PY}" -m vagen.main_ppo --config-path="${VAGEN}/vagen/configs" --config-name=joint_id167_gate "hydra.run.dir=${PHASE_OUT}/hydra" hydra.job.chdir=false "${PHASE_OVERRIDES[@]}")
