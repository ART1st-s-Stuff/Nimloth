...
RUNTIME_ROOT=/tmp/id168-${SLURM_JOB_ID}-${PHASE_NAME}
...
[[ "${RUN_NAME}" == 168_smoke_vagenlite_jointupdate_dp8_tp8_base_train8_t2_a1b1_g099_l095_clip02_akl001_ent001 ]]
...
if [[ "${PHASE}" == update_1 ]]; then
  [[ ! -e "${RUN_OUT}" ]] || { echo "ID168 output already exists" >&2; exit 2; }
...
export WANDB_RUN_ID=nimloth-id168-joint-update-gate
...
PY
  [[ "${RUNTIME_ROOT}" == /tmp/id168-* ]] && rm -rf -- "${RUNTIME_ROOT}"
  exit "${status}"
}
trap cleanup EXIT
...
for name in ('train','val'):
 src=vagen/'examples/train/navigation'/f'{name}_navigation_joint_id168.yaml'
 text=src.read_text()
 assert 'http://127.0.0.1:8000' in text
 dst=out/f'{name}_navigation_joint_id168.yaml'
...
(out/'source_hashes.json').write_text(json.dumps({
 'train_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/train_navigation_joint_id168.yaml').read_bytes()).hexdigest(),
 'val_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/val_navigation_joint_id168.yaml').read_bytes()).hexdigest(),
...
PY
export ID168_TRAIN_CONFIG=${PHASE_OUT}/train_navigation_joint_id168.yaml
export ID168_VAL_CONFIG=${PHASE_OUT}/val_navigation_joint_id168.yaml
export ID168_MODEL=${MODEL}
export ID168_AGENT_CONFIG=${VAGEN}/vagen/configs/agent_no_concat.yaml
export ID168_RUN_NAME=${RUN_NAME}
export ID168_RUN_OUT=${RUN_OUT}

if [[ "${PHASE}" == update_1 ]]; then
  cat >"${RUN_OUT}/README.md" <<EOF
# ID168 DP8 joint-update and exact-resume smoke
...
timeout --signal=TERM --kill-after=10s 300s "${PY}" -m nimloth.environment.navigation.prewarm --env-url "${ENV_URL}" --eval-set base_train --seed 0 --timeout-seconds 300 --env-id "id168-prewarm-${PHASE}-${SLURM_JOB_ID}" | tee "${PHASE_OUT}/prewarm.json"
...
COMMAND=("${PY}" -m vagen.main_ppo --config-path="${VAGEN}/vagen/configs" --config-name=joint_id168_gate "hydra.run.dir=${PHASE_OUT}/hydra" hydra.job.chdir=false "${PHASE_OVERRIDES[@]}")
