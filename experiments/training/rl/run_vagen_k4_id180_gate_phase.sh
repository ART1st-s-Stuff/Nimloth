...
[[ "${RUN_NAME}" == 180_gate_k4schemeb_jointupdate_dp8_tp8_train3x8_t20_s100_c1_a1_b85p78297006578457_t1_cot07p095 ]]
...
if [[ "${PHASE}" == update_1 ]]; then
  [[ ! -e "${RUN_OUT}" ]] || { echo "ID180 output already exists" >&2; exit 2; }
...
export WANDB_RUN_ID=nimloth-id180-k4-single-update-restore-gate
...
for name in ('train','val'):
 src=vagen/'examples/train/navigation'/f'{name}_navigation_joint_id180.yaml'
 text=src.read_text()
 ...
 dst=out/f'{name}_navigation_joint_id180.yaml'
...
(out/'source_hashes.json').write_text(json.dumps({
 'train_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/train_navigation_joint_id180.yaml').read_bytes()).hexdigest(),
 'val_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/val_navigation_joint_id180.yaml').read_bytes()).hexdigest(),
 'assets':assets,
},indent=2)+'\n')
PY
export ID180_TRAIN_CONFIG=${PHASE_OUT}/train_navigation_joint_id180.yaml
export ID180_VAL_CONFIG=${PHASE_OUT}/val_navigation_joint_id180.yaml
export ID180_ACTOR_MODEL=${ACTOR_MODEL}
export ID180_PLANNING_CHECKPOINT=${PLANNING_MODEL}
export ID180_AGENT_CONFIG=${VAGEN}/vagen/configs/agent_no_concat.yaml
...
- retry provenance: ID179 reached the first planning forward but failed before any optimizer step because newly constructed SIGReg buffers remained on CPU; ID180 moves the complete update module to the rank device before DDP and checks all parameters/buffers.
...
if [[ "${PHASE}" == update_1 ]]; then
  ...
  for split in base_train common_sense_train long_horizon_train; do
    timeout --signal=TERM --kill-after=10s 300s "${PY}" -m nimloth.environment.navigation.prewarm --env-url "${ENV_URL}" --eval-set "${split}" --seed 0 --timeout-seconds 300 --env-id "id180-prewarm-${split}-${SLURM_JOB_ID}" | tee "${PHASE_OUT}/prewarm_${split}.json"
  done
fi
...
COMMAND=("${PY}" -m vagen.main_ppo --config-path="${VAGEN}/vagen/configs" --config-name=joint_id180_gate "hydra.run.dir=${PHASE_OUT}/hydra" hydra.job.chdir=false "${PHASE_OVERRIDES[@]}")
...
if phase=='restore_only':
 ...
 assert 'ID180_K4_FRESH_RESTORE_ONLY_ALL_OK global_step=1' in log
summary={
