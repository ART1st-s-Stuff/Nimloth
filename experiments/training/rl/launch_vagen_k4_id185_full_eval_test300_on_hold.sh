#!/usr/bin/env bash
# Launch ID185 full historical test300 from complete ID184 step20 on exact 4x2 H800.
set -euo pipefail

HOLD_JOB=${1:?usage: launch_vagen_k4_id185_full_eval_test300_on_hold.sh HOLD_JOB}
...
COMMON_ENV=(
  ...
  WANDB_PROJECT=vagen
  WANDB_NAME="${RUN_NAME}"
  WANDB_RUN_ID=nimloth-id185-k4-full-eval-test300-retry4
...
assert all(row['wandb_run_id']=='nimloth-id185-k4-full-eval-test300-retry4' for row in probes)
