...
payload={"phase":"full_eval_test300","exit_code":status,"status":"passed" if status==0 else "failed","finished_at":datetime.now(timezone.utc).isoformat()}
...
PY
export ID185_TRAIN_CONFIG=${PHASE_OUT}/train_navigation_joint_id185.yaml
export ID185_VAL_CONFIG=${PHASE_OUT}/val_navigation_joint_id185.yaml
export ID185_ACTOR_MODEL=${ACTOR_MODEL}
export ID185_PLANNING_CHECKPOINT=${PLANNING_MODEL}
export ID185_AGENT_CONFIG=${VAGEN}/vagen/configs/agent_no_concat.yaml
export ID185_RUN_NAME=${RUN_NAME}
export ID185_RUN_OUT=${RUN_OUT}
export ID185_SOURCE_CHECKPOINT=${SOURCE_CHECKPOINT}
...
COMMAND=(
  ...
  --config-name=joint_id185_full_eval
...
out=Path(out_raw); path=f'{entity}/{project}/{run_id}'; expected=[20]
last=None
for _ in range(13):
 run=wandb.Api().run(path)
 ...
 if run.name==run_name and run.state=='finished' and steps==[20]:
  break
 time.sleep(10)
else:
 raise RuntimeError(f'ID185 W&B completion mismatch: {last!r}')
...
assert actual_validation_steps=={20}
expected_sources=(
 'navigation_base_test_id185',
 'navigation_common_sense_test_id185',
 'navigation_complex_instruction_test_id185',
...
rows=[
 json.loads(line)
 for line in (run_out/'validation'/'20.jsonl').read_text().splitlines()
 if line
]
assert len(rows)==300
