#!/usr/bin/env bash
# Launch one strict ID186 continuation phase on exact 4x2 H800.
set -euo pipefail

HOLD_JOB=${1:?usage: launch_vagen_k4_id186_continue_on_hold.sh HOLD_JOB}
...
if [[ "${PHASE}" == resume_20_to_30 ]]; then
  ...
  WANDB_RUN_ID=nimloth-id186-k4-continue-20-to30
else
  [[ "${PHASE}" == resume_30_to_40 ]]
  PHASE_TAG=p2
  PHASE_NAME=phase2_fresh_resume_30_to_40
  EXPECTED_SOURCE_STEP=30
  WANDB_RUN_ID=nimloth-id186-k4-continue-30-to40
fi
PHASE_OUT=${RUN_OUT}/${PHASE_NAME}
RUNTIME_ROOT=/tmp/i186-${HOLD_JOB}-${PHASE_TAG}
RAY_PORT=$((22000 + HOLD_JOB % 10000))
RAY_CLUSTER_ROOT=/tmp/i186-ray-${HOLD_JOB}-${PHASE_TAG}
RAY_LOG_ROOT=${ROOT}/outputs/experiments/training/rl/slurm/id186-ray-${HOLD_JOB}-${PHASE_TAG}
...
exec 9>"${ROOT}/outputs/experiments/training/rl/slurm/.id186-${HOLD_JOB}-${PHASE_TAG}.launch.lock"
flock -n 9 || { echo "duplicate ID186 launcher for job ${HOLD_JOB}" >&2; exit 94; }
...
if [[ "${PHASE}" == resume_20_to_30 ]]; then
  ID186_SOURCE_CHECKPOINT=${ROOT}/outputs/experiments/training/rl/2026-08-17/184_continue_k4schemeb_jointupdate_dp8_tp8_u20_from10_train3x60_b24_t20_s100_c1_a1_b85p78297006578457_t1_cot07p095_val5x8_retry1/checkpoints/global_step_20
else
  ID186_SOURCE_CHECKPOINT=${RUN_OUT}/checkpoints/global_step_30
fi
...
for candidate in "${HEAD_CANDIDATES[@]}"; do
  CANDIDATE_ROOT=/tmp/i186-head-probe-${HOLD_JOB}-${candidate}
  ...
  if [[ ${candidate_status} -eq 0 ]]; then
    ...
    HEAD_NODE=${candidate}
    echo "ID186_DYNAMIC_HEAD_RENDER_OK node=${HEAD_NODE}"
    break
  fi
...
[[ -n "${HEAD_NODE}" ]] || {
  echo "ID186_DYNAMIC_HEAD_RENDER_FAILED candidates=${HEAD_CANDIDATES[*]}" >&2
  exit 124
}
...
ID186_CLUSTER_NODES=$(IFS=,; echo "${CLUSTER_NODES[*]}")
...
cleanup_cluster() {
  ...
  timeout 30s srun --jobid="${HOLD_JOB}" --overlap --nodes=4 --ntasks=4 \
    ...
    RAY_LOG_CAPTURE_COMPLETE="${RAY_LOG_CAPTURE_COMPLETE}" bash -lc '
      [[ "${RAY_CLUSTER_ROOT}" == /tmp/i186-ray-* ]]
      [[ "${RUNTIME_ROOT}" == /tmp/i186-* ]]
      rm -rf -- "${RUNTIME_ROOT}"
...
COMMON_ENV=(
  ...
  ID186_TRAIN_CONFIG="${PHASE_OUT}/train_navigation_joint_id186.yaml"
  ID186_VAL_CONFIG="${PHASE_OUT}/val_navigation_joint_id186.yaml"
  ID186_ACTOR_MODEL="${ACTOR_MODEL}"
  ID186_PLANNING_CHECKPOINT="${PLANNING_MODEL}"
  ID186_SOURCE_CHECKPOINT="${ID186_SOURCE_CHECKPOINT}"
  ID186_AGENT_CONFIG="${REPO}/external/VAGEN/vagen/configs/agent_no_concat.yaml"
  ID186_RUN_NAME="${RUN_NAME}"
  ID186_RUN_OUT="${RUN_OUT}"
  VAGEN_REMOTE_ENV_BASE_URL_OVERRIDE="${ENV_URL}"
  VAGEN_REMOTE_ENV_BASE_URL_OVERRIDE_SCOPE=id186_exact_continuation_v1
...
timeout 120s srun --jobid="${HOLD_JOB}" --overlap --nodes=1 --ntasks=1 \
  ...
  EXPECTED_WANDB_RESUME="${WANDB_RESUME}" EXPECTED_SOURCE_STEP="${EXPECTED_SOURCE_STEP}" EXPECTED_WANDB_RUN_ID="${WANDB_RUN_ID}" "${PY}" - <<'PY' | tee "${RAY_LOG_ROOT}/cluster_probe.json"
import json, os, ray
ray.init(address=os.environ['RAY_ADDRESS'])
alive=[node for node in ray.nodes() if node['Alive']]
rows=sorted(
...
def probe_node():
    ...
    return {
        ...
        'id186_train_config':os.environ.get('ID186_TRAIN_CONFIG'),
        'id186_source_checkpoint':os.environ.get('ID186_SOURCE_CHECKPOINT'),
...
assert all(row['id186_train_config'] for row in probes)
assert all(row['id186_source_checkpoint'].endswith('/global_step_'+os.environ['EXPECTED_SOURCE_STEP']) for row in probes)
...
srun --jobid="${HOLD_JOB}" --overlap --nodes=1 --ntasks=1 \
  ...
  env "${COMMON_ENV[@]}" RAY_ADDRESS="${RAY_ADDRESS}" \
    ...
    ID186_HEAD_IP="${HEAD_IP}" ID186_EXPECTED_NNODES=4 \
    ID186_EXPECTED_GPUS_PER_NODE=2 ID186_CLUSTER_NODES="${ID186_CLUSTER_NODES}" \
