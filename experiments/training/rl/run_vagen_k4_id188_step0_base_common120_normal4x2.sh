...
TRAIN_CONFIG_SOURCE=${ROOT}/outputs/experiments/training/rl/2026-08-17/184_continue_k4schemeb_jointupdate_dp8_tp8_u20_from10_train3x60_b24_t20_s100_c1_a1_b85p78297006578457_t1_cot07p095_val5x8_retry1/continue_step10_to20/train_navigation_joint_id184.yaml
...
print(json.dumps({'status':'ID188_RAY_4X2_OK','nodes':rows}))
PY
...
export VAGEN_ROLLOUT_BROWSER_PACK_WORKERS
...
- audit: every real action step persists the true observation image, actual generated CoT, prior and executed action, direct all-action Q, Frozen-V current state value, MCTS root predictions/visits, all candidate action sequences, float32 K16 state, and all 100 chronological simulations.
- expected outcome: ${ID185_VIS_EXPECTED_OUTCOME}. This controls only final validation and never changes policy execution.
- results: 'evaluation_browser/global_step_0/index.html' is the unified interactive browser.
- W&B: project 'vagen', run '${RUN_NAME}', enabled=${ID185_VIS_ENABLE_WANDB}, identity '${WANDB_RUN_ID}', resume never.
EOF
...
/cm/shared/apps/slurm/current/bin/srun --overlap \
  --nodes=4 --ntasks=4 --ntasks-per-node=1 --gres=gpu:2 --label \
...
COMMAND=(
  env
  "ID188_TRAIN_CONFIG=${ID188_TRAIN_CONFIG}"
  "ID188_VAL_CONFIG=${ID188_VAL_CONFIG}"
  "ID188_ACTOR_MODEL=${ID188_ACTOR_MODEL}"
  "ID188_PLANNING_CHECKPOINT=${ID188_PLANNING_CHECKPOINT}"
  "ID188_AGENT_CONFIG=${ID188_AGENT_CONFIG}"
  "ID188_RUN_NAME=${ID188_RUN_NAME}"
  "ID188_RUN_OUT=${ID188_RUN_OUT}"
  "${PY}" -m vagen.main_ppo
  --config-path="${VAGEN}/vagen/configs"
  --config-name=joint_id188_step0_base_common120
...
assert 'ID188_K4_STEP0_BASE_COMMON120_BOOTSTRAP_OK global_step=0' in log
assert 'VALIDATION_BATCH_JOURNAL_COMPLETE batches=3 rows=120' in log
pack_lines=[line for line in log.splitlines() if 'ROLLOUT_BROWSER_STATE_PACK_COMPLETE' in line]
assert len(pack_lines)==3 and all('workers=8' in line for line in pack_lines), pack_lines
rows=[json.loads(line) for line in (run/'validation/0.jsonl').read_text().splitlines() if line.strip()]
assert len(rows)==120
counts=Counter(row['data_source'] for row in rows)
assert counts=={'navigation_base_test_id188':60,'navigation_common_sense_test_id188':60}
...
for path in rollout_files:
 ...
 assert record['provenance']['source_step']==776
 assert str(record['provenance']['snapshot_id']).startswith('sha256:')
...
payload={'status':'passed','phase':'base_common120','global_step':0,'source_step':776,'rollout_count':120,'archive_count':archives,'checkpoint_steps':[],'success_by_source':dict(success),'reward_sum_by_source':dict(reward),'turns_by_source':dict(turns),'browser':str(browser/'index.html'),'manifest_sha256':complete['manifest_sha256'],'pack_workers':8}
...
print('ID188_STEP0_BASE_COMMON120_BROWSER_ALL_OK '+json.dumps(payload,sort_keys=True))
PY
...
- Status: passed.
- Frozen source: corrected ID176 actor + ID74 planner at source step776; no backward, optimizer update, resume, or checkpoint.
