...
HOLD_JOB=${1:?usage: launch_vagen_k4_id182_restore_retry1_on_hold.sh HOLD_JOB}
...
exec srun --jobid="${HOLD_JOB}" --overlap --nodes=1 --ntasks=1 \
  ...
  bash -lc '
    ...
    RESTORE_ATTEMPT=retry1 PHASE=restore_only "${REPO}/experiments/training/rl/run_vagen_k4_id182_gate_phase.sh"
  '
