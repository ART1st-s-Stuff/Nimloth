...
正式RL后端已改为VAGEN/VERL full actor + full token critic + immutable ref；下方自写dynamic trainer入口仅保留diagnostic证据，不再用于quality pilot。Exact入口为`hold_verl_exact_replay_normal8.slurm`、`launch_verl_exact_replay_in_hold.sh`及`run_verl_exact_replay_worker_gate.py`；online入口新增`hold_verl_online_normal8plus1.slurm`、`launch_verl_online_in_hold.sh`和`run_verl_online_world8_smoke.sh`。ID39/40虽通过actor/critic update与resume mechanics，但其4.55 critic后来确认全backbone随机初始化，不能作为loaded-critic证据。ID44/45已在world8 fixed exact replay验证loaded critic、full actor语言+视觉、WM update、immutable ref，以及schema1/inject/k8 WM module+optimizer+scheduler从step1严格恢复到step2。真实service gate现已直接通过跨节点base_train preflight、Ray/world8 actor+critic、vLLM TP4 tied-head load并推进到KV/sampling profile；ID57最后因FlashInfer找不到venv ninja终止，commit`b58d877`已修复但尚未重跑。当前仍没有一次完成online generation→env→update；collapsed SFT1和随机WM禁止quality pilot。
...
当前资源入口`dynamic_fsdp_k8_fragmented_4plus2plus1plus1_env48.slurm`接受`RUN_MODE=smoke|baseline|pilot`；它按1+1+2+4组织trainer fragments；rank0绑定提交时核实高内存的dgx09，中间1+2由Slurm动态选择，4-GPU大组绑定提交时重新核实有4张空卡的dgx27，不保留过期protected-node exclude，并使用独立preempt dgx48 env GPU。在同一allocation内重跑bounded create/prompt/reset/schema/close preflight。dgx48已由job478976独立preflight通过。
