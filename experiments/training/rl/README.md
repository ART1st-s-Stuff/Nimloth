...
当前资源入口`dynamic_fsdp_k8_fragmented_4plus2plus1plus1_env48.slurm`只接受`RUN_MODE=smoke`；它按1+1+2+4组织trainer fragments；1+1+2由Slurm在全部normal节点动态选择，4-GPU大组只绑定提交时重新核实有4张空卡的dgx27，且不保留过期protected-node exclude，并使用独立preempt dgx48 env GPU。在同一allocation内重跑bounded create/prompt/reset/schema/close preflight，并明确拒绝baseline/pilot。dgx48已由job478976独立preflight通过。
...
| `dynamic_fsdp_k8_fragmented_4plus2plus1plus1_env48.slurm` | 当前normal 1+1+2动态fragments + submit-time verified dgx27 4-GPU fragment + preflight-proven dgx48 env；无旧exclude |
