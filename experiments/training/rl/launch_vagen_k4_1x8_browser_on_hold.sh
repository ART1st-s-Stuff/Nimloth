...
HOLD_JOB=${1:?usage: launch_vagen_k4_1x8_browser_on_hold.sh HOLD_JOB}
...
PHASE_NAME=${ID185_VIS_PHASE_NAME_OVERRIDE:-visualization}
...
timeout 30s srun --jobid="${HOLD_JOB}" --overlap --nodes=1 --ntasks=1 \
  --ntasks-per-node=1 --gres=gpu:8 bash -lc '
    set -euo pipefail
...
# Ray's own resource accounting keeps the eight worker actors within the single 8-GPU node.
COMMON_ENV=(
  ...
  ID187_TRAIN_CONFIG="${PHASE_OUT}/train_navigation_joint_id187.yaml"
  ID187_VAL_CONFIG="${PHASE_OUT}/val_navigation_joint_id187.yaml"
  ID187_ACTOR_MODEL="${ACTOR_MODEL}"
  ID187_PLANNING_CHECKPOINT="${PLANNING_MODEL}"
  ID187_SOURCE_CHECKPOINT="${ID185_SOURCE_CHECKPOINT}"
  ID187_AGENT_CONFIG="${REPO}/external/VAGEN/vagen/configs/agent_no_concat.yaml"
  ID187_RUN_NAME="${RUN_NAME}"
  ID187_RUN_OUT="${RUN_OUT}"
  ID187_SEED="${VIS_SEED}"
...
for node in "${WORKER_NODES[@]}"; do
  ...
  srun --jobid="${HOLD_JOB}" --overlap --nodes=1 --ntasks=1 \
    -w "${node}" --gres=gpu:8 --cpus-per-task=16 \
...
ray.shutdown()
if counts != [8.0] or addresses != sorted(os.environ['RAY_EXPECTED_NODE_IPS'].split(',')):
...
assert [row['gpus'] for row in rows] == [8.0]
...
srun --jobid="${HOLD_JOB}" --overlap --nodes=1 --ntasks=1 \
  -w "${HEAD_NODE}" --gres=gpu:8 --cpus-per-task=16 \
  env "${COMMON_ENV[@]}" RAY_ADDRESS="${RAY_ADDRESS}" \
    RAY_EXPECTED_NODE_IPS="${RAY_EXPECTED_NODE_IPS}" VLLM_HOST_IP="${HEAD_IP}" \
    ID185_HEAD_IP="${HEAD_IP}" ID185_EXPECTED_NNODES=1 \
    ID185_EXPECTED_GPUS_PER_NODE=8 ID185_EXPECTED_GPU_COUNTS=8 \
    ID185_HEAD_GPU_COUNT=8 ID185_CLUSTER_NODES="${ID185_CLUSTER_NODES}" \
