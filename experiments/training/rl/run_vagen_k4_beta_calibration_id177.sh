...
CONTROL=${ROOT}/outputs/experiments/training/rl/slurm/id177-k4-${SLURM_JOB_ID}
ENV_PORT=$((19400 + SLURM_JOB_ID % 400))
ENV_URL=http://127.0.0.1:${ENV_PORT}
RUNTIME_ROOT=/tmp/i177-${SLURM_JOB_ID}
...
[[ ! -e "${RUN_OUT}" ]] || { echo "ID177 output already exists" >&2; exit 2; }
[[ ! -e "${RUN_META}" ]] || { echo "ID177 metadata already exists" >&2; exit 2; }
[[ ! -e "${CONTROL}" ]] || { echo "ID177 control output already exists" >&2; exit 2; }
...
payload={"experiment_id":177,"exit_code":status,"status":"passed" if status==0 else "failed","finished_at":datetime.now(timezone.utc).isoformat()}
...
PY
  [[ "${RUNTIME_ROOT}" == /tmp/i177-* ]] && rm -rf -- "${RUNTIME_ROOT}"
  exit "${status}"
}
trap cleanup EXIT

cat >"${RUN_META}" <<EOF
# ID177 optimizer-free K4 beta calibration diagnostic retry
...
- provenance: ID174 proved the original ID74 action-token rows had median zero spread and could not support beta calibration. ID176 repaired only those eight rows and passed heldout NLL/BF16-spread plus exact frozen-component gates. ID177 keeps the approved behavior/search values and run seed, uses the immutable completed ID176 checkpoint, and persists all finite diagnostics before accept/review classification.
...
- initialization: completed ID176 Qwen action-row repair derived from corrected ID74 epoch_001, plus byte-identical ID74 SharedSlotProjector, history-1/horizon-4 wm_predictor, and 8-action ValueHead; planning source step remains 776.
...
- output: ${RUN_OUT}; a failed or interrupted ID177 is not resumable and must never be reused.
...
for split in ('base_train','common_sense_train','long_horizon_train'):
 command=[sys.executable,'-m','nimloth.environment.navigation.prewarm','--env-url',url,'--eval-set',split,'--seed','0','--timeout-seconds','300','--env-id',f'id177-prewarm-{split}-{job}']
...
payload={'status':'passed' if accepted else 'requires_human_review','experiment_id':177,'calibration_accepted':accepted,'review_reason':summary['review_reason'],'calibrated_beta_requires_human_approval':beta,'optimizer':None,'checkpoint':None,'canary_started':False}
