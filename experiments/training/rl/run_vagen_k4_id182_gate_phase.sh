...
RUNTIME_ROOT=/tmp/i182-${SLURM_JOB_ID}-${PHASE_TAG}
...
[[ "${RUN_NAME}" == 182_gate_k4schemeb_jointupdate_dp8_tp8_train3x8_t20_s100_c1_a1_b85p78297006578457_t1_cot07p095 ]]
...
if [[ "${PHASE}" == update_1 ]]; then
  [[ ! -e "${RUN_OUT}" ]] || { echo "ID182 output already exists" >&2; exit 2; }
...
export WANDB_RUN_ID=nimloth-id182-k4-single-update-restore-gate
...
PY
  [[ "${RUNTIME_ROOT}" == /tmp/i182-* ]] && rm -rf -- "${RUNTIME_ROOT}"
  exit "${status}"
}
trap cleanup EXIT
...
for name in ('train','val'):
 src=vagen/'examples/train/navigation'/f'{name}_navigation_joint_id182.yaml'
 text=src.read_text()
 ...
 dst=out/f'{name}_navigation_joint_id182.yaml'
...
(out/'source_hashes.json').write_text(json.dumps({
 'train_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/train_navigation_joint_id182.yaml').read_bytes()).hexdigest(),
 'val_config_sha256':hashlib.sha256((vagen/'examples/train/navigation/val_navigation_joint_id182.yaml').read_bytes()).hexdigest(),
 'assets':assets,
},indent=2)+'\n')
PY
export ID182_TRAIN_CONFIG=${PHASE_OUT}/train_navigation_joint_id182.yaml
export ID182_VAL_CONFIG=${PHASE_OUT}/val_navigation_joint_id182.yaml
export ID182_ACTOR_MODEL=${ACTOR_MODEL}
export ID182_PLANNING_CHECKPOINT=${PLANNING_MODEL}
export ID182_AGENT_CONFIG=${VAGEN}/vagen/configs/agent_no_concat.yaml
export ID182_RUN_NAME=${RUN_NAME}
export ID182_RUN_OUT=${RUN_OUT}

"${PY}" - "${ID182_TRAIN_CONFIG}" "${PHASE_OUT}" <<'PY'
import json, sys
...
assert len(rows)==24
assert counts=={
 'navigation_base_train_id182':8,
 'navigation_common_sense_train_id182':8,
 'navigation_long_horizon_train_id182':8,
}
...
# ID182 corrected single-update K4 Scheme-B integration gate

- retry provenance: ID181 reached actor update before the rank-local empty failure; it produced no complete update or checkpoint. ID182 keeps the same approved contract and uses the independently reviewed rank-local empty K4 shard fix.
...
if [[ "${PHASE}" == update_1 ]]; then
  ...
  for split in base_train common_sense_train long_horizon_train; do
    timeout --signal=TERM --kill-after=10s 300s "${PY}" -m nimloth.environment.navigation.prewarm --env-url "${ENV_URL}" --eval-set "${split}" --seed 0 --timeout-seconds 300 --env-id "id182-prewarm-${split}-${SLURM_JOB_ID}" | tee "${PHASE_OUT}/prewarm_${split}.json"
  done
fi
...
if phase=='restore_only':
 ...
 assert 'ID182_K4_FRESH_RESTORE_ONLY_ALL_OK global_step=1' in log
summary={
