...
HOLD_JOB=${1:?usage: launch_vagen_k4_id182_gate_on_hold.sh HOLD_JOB}
...
RUN_DATE=2026-08-16
RUNNER=${REPO}/experiments/training/rl/run_vagen_k4_id182_gate_phase.sh
...
exec srun --jobid="${HOLD_JOB}" --overlap --nodes=1 --ntasks=1 \
  ...
  bash -lc '
    ...
    PHASE=update_1 "${REPO}/experiments/training/rl/run_vagen_k4_id182_gate_phase.sh"
    PHASE=restore_only "${REPO}/experiments/training/rl/run_vagen_k4_id182_gate_phase.sh"
  '
