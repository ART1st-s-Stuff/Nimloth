...
def aggregate(
    ...
    for eval_set in eval_sets:
        ...
        if summary.get("status") != "ALL_OK":
            raise ValueError(f"{eval_set} rollout did not report ALL_OK")
        count = int(summary.get("num_trajectories", -1))
        if count != episodes_per_eval_set:
            raise ValueError(
                f"{eval_set} trajectory count mismatch: "
                f"{count} != {episodes_per_eval_set}"
            )
        if contract.get("eval_sets") != [eval_set]:
            raise ValueError(f"{eval_set} evaluation contract has wrong eval_sets")
        comparable = {
            key: value
        ...
        if common_contract is None:
            ...
            raise ValueError(f"{eval_set} evaluation contract does not match peers")

        metrics = summary.get("metrics", {}).get("overall")
        if not isinstance(metrics, dict):
            raise ValueError(f"{eval_set} summary has no overall metrics")
        normalized = {key: float(metrics[key]) for key in weighted}
        if not all(math.isfinite(value) for value in normalized.values()):
            raise ValueError(f"{eval_set} metrics are non-finite: {normalized}")
        by_eval_set[eval_set] = normalized
    ...
    return {
        "status": "ALL_OK",
        "evaluation": "sft2_pre_rl_mcts_parallel_v1",
