#!/usr/bin/env python3
"""Validate six logical 2-GPU agents on a 4+4+2+2 allocation."""
...
def validate_allocation(run_root: Path, job_id: str) -> dict[str, object]:
    ...
    assert sorted(host_task_counts.values()) == [1, 1, 2, 2]
    assert sorted(len(uuids) for uuids in host_uuids.values()) == [2, 2, 4, 4]
    assert len(all_uuids) == 12
    return {
        "logical_agents": 6,
        "physical_nodes": 4,
        "gpu_uuids": 12,
