...
test "${SLURM_NTASKS}" -eq 3
test "${AGENT_RANK}" -ge 0
test "${AGENT_RANK}" -lt 3
...
case "${AGENT_RANK}" in
  0) EXPECTED_LOCAL_WORLD_SIZE=8; GLOBAL_FIRST=0 ;;
  1) EXPECTED_LOCAL_WORLD_SIZE=4; GLOBAL_FIRST=8 ;;
  2) EXPECTED_LOCAL_WORLD_SIZE=4; GLOBAL_FIRST=12 ;;
esac
test "${GPU_COUNT}" -eq "${EXPECTED_LOCAL_WORLD_SIZE}"
test "$(printf '%s\n' "${GPU_NAMES}" | grep -c 'H800')" -eq "${EXPECTED_LOCAL_WORLD_SIZE}"
GLOBAL_LAST=$((GLOBAL_FIRST + EXPECTED_LOCAL_WORLD_SIZE - 1))
{
  ...
  echo "local_ranks=0-$((EXPECTED_LOCAL_WORLD_SIZE - 1)) global_ranks=${GLOBAL_FIRST}-${GLOBAL_LAST}"
...
export NPROC_PER_NODE=${EXPECTED_LOCAL_WORLD_SIZE}
export NNODES=3
export EXPECTED_WORLD_SIZE=16
