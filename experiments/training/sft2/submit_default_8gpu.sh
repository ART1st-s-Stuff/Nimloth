...
echo "latent_token_count: ${LATENT_TOKEN_COUNT}"
echo "latent_query_mode override: ${LATENT_QUERY_MODE:-<config/default>}"
echo "output: ${TRAIN_OUT}"
echo "node: ${SFT2_NODE:-<scheduler>}"

SBATCH_ARGS=(--account=peilab --job-name="sft2-default-8g")
if [ -n "${SFT2_DEPENDENCY:-}" ]; then
  SBATCH_ARGS+=(--dependency="${SFT2_DEPENDENCY}")
fi
if [ -n "${SFT2_NODE}" ]; then
...
J=$($SLURM "${SBATCH_ARGS[@]}" \
  --export=ALL,SFT2_LLM_TUNE="${SFT2_LLM_TUNE}",SFT2_VISION_TUNE="${SFT2_VISION_TUNE}",NGPUS="${NGPUS}",SFT1_EPOCH="${SFT1_EPOCH}",SKIP_SFT1_DONE=1,TRAIN_OUT_OVERRIDE="${TRAIN_OUT}",EXPERIMENT_NAME="${EXPERIMENT_NAME}",RUN_DATE="${RUN_DATE}",LATENT_TOKEN_COUNT="${LATENT_TOKEN_COUNT}",LATENT_QUERY_MODE="${LATENT_QUERY_MODE}",REQUIRE_PREBUILT_CACHE="${REQUIRE_PREBUILT_CACHE:-0}" \
