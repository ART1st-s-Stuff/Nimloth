...
J=$($SLURM "${SBATCH_ARGS[@]}" \
  --export=ALL,SFT2_LLM_TUNE="${SFT2_LLM_TUNE}",SFT2_VISION_TUNE="${SFT2_VISION_TUNE}",NGPUS="${NGPUS}",SFT1_EPOCH="${SFT1_EPOCH}",SKIP_SFT1_DONE=1,TRAIN_OUT_OVERRIDE="${TRAIN_OUT}",EXPERIMENT_NAME="${EXPERIMENT_NAME}",RUN_DATE="${RUN_DATE}",LATENT_TOKEN_COUNT="${LATENT_TOKEN_COUNT}",MASK_LATENT_QUERY_LABELS="${MASK_LATENT_QUERY_LABELS}" \
