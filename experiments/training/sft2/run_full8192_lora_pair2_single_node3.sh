...
OUT_ROOT=${OUT_ROOT:-${ROOT}/sft2/22_state8192_fullwm8192_llmlora_vislora_pair2_ws3_ga11_ep5_img8}
TRAIN_OUT=${TRAIN_OUT:-${OUT_ROOT}/train}
PY=${PY:-/project/peilab/atst/nimloth/.venv-vagen-main/bin/python3}
CONFIG=${CONFIG:-${REPO}/configs/training/sft2/latent_wm_value_k8_state8192_ep5_img8.yaml}
...
RUN_NAME=${RUN_NAME:-22_state8192_fullwm8192_llmlora64a128_vislora64a128_pair2_ws3_ga11_ep5_img8}
...
export MASTER_ADDR=dgx-27 WORLD_SIZE=3 NIMLOTH_DDP_GPU_STRIDE=2
...
SRUN_ARGS=(--het-group=0 --nodes=1 --ntasks=1 --kill-on-bad-exit=1)
if [[ -n "${HOLD_JOB:-}" ]]; then
...
srun "${SRUN_ARGS[@]}" bash -lc '
  ...
  for ((local=0; local<LOCAL_WORLD; local++)); do
    ...
    RANK=$rank LOCAL_RANK=$local WORLD_SIZE=3 MASTER_ADDR=dgx-27 MASTER_PORT="$MASTER_PORT" \
      "$PY" experiments/training/sft2/train.py \
        ...
        --epochs 5 --batch-size 2 --grad-accum 11 --checkpoint-interval-minutes 20 \
