...
test "${GPU_COUNT}" -eq 2
test "$(printf '%s\n' "${GPU_NAMES}" | grep -c 'H800')" -eq 2
GLOBAL_FIRST=$((AGENT_RANK * 2))
GLOBAL_LAST=$((GLOBAL_FIRST + 1))
{
  ...
  echo "local_ranks=0-1 global_ranks=${GLOBAL_FIRST}-${GLOBAL_LAST}"
