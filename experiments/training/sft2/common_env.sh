...
# Leave mode empty when unset so the selected YAML config can supply it.
LATENT_QUERY_MODE="${LATENT_QUERY_MODE:-}"
PYTHON_ENV="${PYTHON_ENV:-${REPO}/.venv}"
export LATENT_TOKEN_COUNT LATENT_QUERY_MODE PYTHON_ENV
export NIMLOTH_LATENT_TOKEN_COUNT="${LATENT_TOKEN_COUNT}"
WANDB_PROJECT="${WANDB_PROJECT:-nimloth-sft2}"
