#!/usr/bin/env python3
"""Validate six logical 4-GPU agents on a 2x8 + 2x4 allocation."""
...
def validate_allocation(run_root: Path, job_id: str) -> dict[str, object]:
    paths = sorted(run_root.glob(f"allocation_{job_id}_node*.log"))
    assert len(paths) == 6, f"expected 6 logical agents, got {len(paths)}"
    ranks: set[int] = set()
    ...
    assert sorted(Counter(path_host(path) for path in paths).values()) == [1, 1, 2, 2]
    assert sorted(len(uuids) for uuids in host_uuids.values()) == [4, 4, 8, 8]
    assert len(all_uuids) == 24
    return {
        "logical_agents": 6,
        "physical_nodes": 4,
        "gpu_uuids": 24,
        "tasks_per_host": sorted(Counter(path_host(path) for path in paths).values()),
    }
