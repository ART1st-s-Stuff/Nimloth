...
test "${SLURM_NTASKS}" -eq "${EXPECTED_NNODES}"
test "${NODE_RANK}" -ge 0
test "${NODE_RANK}" -lt "${EXPECTED_NNODES}"
...
test "${GPU_COUNT}" -eq "${NPROC_PER_NODE_EXPECTED}"
test "$(printf '%s\n' "${GPU_NAMES}" | grep -c 'H800')" -eq "${NPROC_PER_NODE_EXPECTED}"
GLOBAL_FIRST=$((NODE_RANK * NPROC_PER_NODE_EXPECTED))
GLOBAL_LAST=$((GLOBAL_FIRST + NPROC_PER_NODE_EXPECTED - 1))
LOCAL_LAST=$((NPROC_PER_NODE_EXPECTED - 1))
{
  ...
  echo "local_ranks=0-${LOCAL_LAST} global_ranks=${GLOBAL_FIRST}-${GLOBAL_LAST}"
...
export NPROC_PER_NODE="${NPROC_PER_NODE_EXPECTED}"
export NNODES="${EXPECTED_NNODES}"
export NODE_RANK
export BATCH_SIZE=1
export GRAD_ACCUM="${GRAD_ACCUM_EXPECTED}"
export RESUME=0
