#!/usr/bin/env python3
"""Compare pre-latent suffix token representations/cache between one-chunk and two-chunk KV paths."""
...
def _prepare(model, enc, device):
    ...
    return {
        ...
        "position_ids": _compute_position_ids(model, batch, embeds),
    }
...
def _layer0_cache_tensors(cache_obj):
    if hasattr(cache_obj, "layers"):
        layer0 = cache_obj.layers[0]
        k = layer0.keys.detach().cpu()
        v = layer0.values.detach().cpu()
        return k, v
    return cache_obj.key_cache[0].detach().cpu(), cache_obj.value_cache[0].detach().cpu()
...
def run_case(model, processor, token_id_map, device, steps, max_length, case):
    ...
    out_one = _forward(model, prep1, len0, len1, out0.past_key_values)

    reset_model_rope_state(model)
    out0b = _forward(model, prep0, 0, len0, None)
    out_mid = _forward(model, prep1, len0, latent1, out0b.past_key_values)

    # layer0 output for the token right before latent
    one_layer0 = out_one.hidden_states[1][0, probe_rel_one].detach().cpu()
    mid_layer0 = out_mid.hidden_states[1][0, -1].detach().cpu()

    # compare cached K/V for that same token after layer0 cache write
    k_one, v_one = _layer0_cache_tensors(out_one.past_key_values)
    k_mid, v_mid = _layer0_cache_tensors(out_mid.past_key_values)
    k_token_one = k_one[0, :, probe_abs, :]
    v_token_one = v_one[0, :, probe_abs, :]
    k_token_mid = k_mid[0, :, probe_abs, :]
    v_token_mid = v_mid[0, :, probe_abs, :]
    ...
    return {
        ...
        "layer0_output_probe_diff": _maxdiff(one_layer0, mid_layer0),
        "layer0_cache_k_probe_diff": _maxdiff(k_token_one, k_token_mid),
        "layer0_cache_v_probe_diff": _maxdiff(v_token_one, v_token_mid),
        "one_chunk_hidden_final_probe_diff_vs_mid": _maxdiff(out_one.last_hidden_state[0, probe_rel_one], out_mid.last_hidden_state[0, -1]),
        "cache_lens": {"one": int(out_one.past_key_values.get_seq_length()), "mid": int(out_mid.past_key_values.get_seq_length())},
    }
