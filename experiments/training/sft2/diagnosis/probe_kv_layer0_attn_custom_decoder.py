#!/usr/bin/env python3
"""Capture layer0 attention internals for KV one-chunk vs two-chunk Qwen decoder."""
...
def _compute_position_ids(model, batch, inputs_embeds):
...
def _patch_layer0_capture(model, target_idx: int):
    attn = model.model.language_model.layers[0].self_attn
    original = attn.forward
    capture: dict[str, Any] = {}

    def patched(self, hidden_states, attention_mask=None, position_ids=None, past_key_values=None, output_attentions=False, use_cache=False, position_embeddings=None, **kwargs):
        ...
        capture["q_len"] = int(q_len)
        capture["attention_mask_shape"] = list(attention_mask.shape) if attention_mask is not None and hasattr(attention_mask, "shape") else None
        capture["position_ids_shape"] = list(position_ids.shape) if position_ids is not None and hasattr(position_ids, "shape") else None
        pkv = past_key_values if past_key_values is not None else kwargs.pop("past_key_value", None)
        capture["past_len_before"] = int(pkv.get_seq_length()) if pkv is not None and hasattr(pkv, "get_seq_length") else 0
        try:
            out = original(hidden_states, attention_mask=attention_mask, position_ids=position_ids, past_key_values=pkv, output_attentions=output_attentions, use_cache=use_cache, position_embeddings=position_embeddings, **kwargs)
        except TypeError as exc:
            if "past_key_values" not in str(exc):
                raise
            out = original(hidden_states, attention_mask=attention_mask, position_ids=position_ids, past_key_value=pkv, output_attentions=output_attentions, use_cache=use_cache, position_embeddings=position_embeddings, **kwargs)
...
def _forward_capture(model, prep, start, end, past, target_idx: int):
    attn, original, capture = _patch_layer0_capture(model, target_idx)
    try:
...
def run_case(model, processor, token_id_map, device, steps, max_length, case):
    ...
    reset_model_rope_state(model)
    out0, _ = _forward_capture(model, prep0, 0, len0, None, find_last_latent_state_index(enc0["input_ids"], token_id_map))
    out_one, cap_one = _forward_capture(model, prep1, len0, len1, out0.past_key_values, latent1 - len0)

    reset_model_rope_state(model)
    out0b, _ = _forward_capture(model, prep0, 0, len0, None, find_last_latent_state_index(enc0["input_ids"], token_id_map))
    out_mid, _ = _forward_capture(model, prep1, len0, split, out0b.past_key_values, max(0, split - len0 - 1))
    out_two, cap_two = _forward_capture(model, prep1, split, len1, out_mid.past_key_values, 0)
    ...
    return {
        ...
        "len1": len1,
        "split": split,
        "q_len_one": cap_one.get("q_len"),
        "q_len_two": cap_two.get("q_len"),
        "past_len_before_one": cap_one.get("past_len_before"),
        "past_len_before_two": cap_two.get("past_len_before"),
        "attention_mask_shape_one": cap_one.get("attention_mask_shape"),
        "attention_mask_shape_two": cap_two.get("attention_mask_shape"),
        "hidden_target_diff": _maxdiff(cap_one["hidden_target"], cap_two["hidden_target"]),
        "q_proj_target_diff": _maxdiff(cap_one["q_proj_target"], cap_two["q_proj_target"]),
        "k_proj_target_diff": _maxdiff(cap_one["k_proj_target"], cap_two["k_proj_target"]),
        "v_proj_target_diff": _maxdiff(cap_one["v_proj_target"], cap_two["v_proj_target"]),
        "attn_output_target_diff": _maxdiff(cap_one["attn_output_target"], cap_two["attn_output_target"]),
        "final_hidden_diff": _maxdiff(out_one.last_hidden_state[0, latent1 - len0], out_two.last_hidden_state[0, 0]),
