#!/usr/bin/env python3
"""Probe layer0/layer1 attention internals for the first suffix token after chunk boundary."""
...
def _patch_attn(model, layer_idx: int, target_idx: int):
    ...
    def patched(self, hidden_states, attention_mask=None, position_ids=None, output_attentions=False, use_cache=False, position_embeddings=None, **kwargs):
        ...
        capture["position_ids_target"] = position_ids[:, 0, target_idx].detach().cpu().tolist() if position_ids is not None else None
...
def run_case(model, processor, device, steps, max_length, case):
    ...
    reset_model_rope_state(model)
    out0 = model.model.language_model(
        input_ids=None,
    ...
    # first suffix token only
    out_one_l0, cap_one_l0 = _forward_capture(model, prep1, len0, len0 + 1, out0.past_key_values, 0, 0)
    reset_model_rope_state(model)
    out0b = model.model.language_model(
        input_ids=None,
        inputs_embeds=prep0["inputs_embeds"][:, :len0],
        attention_mask=prep0["attention_mask"][:, :len0],
        position_ids=prep0["position_ids"][..., :len0],
    ...
    out_two_l0, cap_two_l0 = _forward_capture(model, prep1, len0, len0 + 1, out0b.past_key_values, 0, 0)

    # same token but capture layer1 on an isolated rerun for each path
    reset_model_rope_state(model)
    out0c = model.model.language_model(
        input_ids=None,
        inputs_embeds=prep0["inputs_embeds"][:, :len0],
        attention_mask=prep0["attention_mask"][:, :len0],
        position_ids=prep0["position_ids"][..., :len0],
    ...
    out_one_l1, cap_one_l1 = _forward_capture(model, prep1, len0, len0 + 1, out0c.past_key_values, 1, 0)
    reset_model_rope_state(model)
    out0d = model.model.language_model(
        input_ids=None,
        inputs_embeds=prep0["inputs_embeds"][:, :len0],
        attention_mask=prep0["attention_mask"][:, :len0],
        position_ids=prep0["position_ids"][..., :len0],
    ...
    out_two_l1, cap_two_l1 = _forward_capture(model, prep1, len0, len0 + 1, out0d.past_key_values, 1, 0)
    ...
    def compare(a, b):
        return {
            ...
            "q_len_one": a["q_len"],
            "q_len_two": b["q_len"],
            "past_len_before_one": a["past_len_before"],
            "past_len_before_two": b["past_len_before"],
            "attention_mask_shape_one": a["attention_mask_shape"],
            "attention_mask_shape_two": b["attention_mask_shape"],
            "position_ids_shape_one": a["position_ids_shape"],
            "position_ids_shape_two": b["position_ids_shape"],
            "position_ids_target_one": a["position_ids_target"],
            "position_ids_target_two": b["position_ids_target"],
            "cache_position_one": a["cache_position"],
            "cache_position_two": b["cache_position"],
        }

    return {
        "case": case,
        "len0": len0,
        "layer0": compare(cap_one_l0, cap_two_l0),
        "layer1": compare(cap_one_l1, cap_two_l1),
        "final_hidden_diff_layer0_run": _maxdiff(out_one_l0.last_hidden_state[0, 0], out_two_l0.last_hidden_state[0, 0]),
        "final_hidden_diff_layer1_run": _maxdiff(out_one_l1.last_hidden_state[0, 0], out_two_l1.last_hidden_state[0, 0]),
    }
...
def main():
    ...
    special_token_ids(processor.tokenizer)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        reports.append(run_case(model, processor, device, expand_record_transitions(_make_record(Path(tmp)))[:2], args.max_length, "synthetic"))
    if args.train_jsonl:
        rec = load_jsonl_records(args.train_jsonl, max_records=args.record_index + 1)[args.record_index]
        reports.append(run_case(model, processor, device, expand_record_transitions(rec)[:2], args.max_length, f"real_{rec.get('id')}"))
