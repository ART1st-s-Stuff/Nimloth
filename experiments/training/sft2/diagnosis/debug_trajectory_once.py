...
from experiments.training.sft.diagnosis.packed_trajectory import _reset_model_rope_state
...
def _position_ids_prefix_equal(model, prefix_enc: dict[str, torch.Tensor], full_enc: dict[str, torch.Tensor]) -> bool | None:
    if not hasattr(model, "model") or not hasattr(model.model, "get_rope_index"):
        return None
    try:
        prefix_batch = batch_single_encoding(prefix_enc)
        full_batch = batch_single_encoding(full_enc)
...
def analyze_case(
    ...
    *,
    ...
    for sample in steps:
        ...
        _reset_model_rope_state(model)
        prefix_latent, _ = extract_qwen_latents(model, batch_single_encoding(prefix_enc), token_id_map, device)
