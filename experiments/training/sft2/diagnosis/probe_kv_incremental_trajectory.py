...
from experiments.training.sft.diagnosis.trajectory_once import find_step_latent_indices
...
def _to_device_batch(enc: dict[str, torch.Tensor], device: torch.device) -> dict[str, torch.Tensor]:
    return {k: v.to(device) for k, v in batch_single_encoding(enc).items()}
