...
HF checkpoint 开始，在六卡上依次运行统一后的 SFT1 和 SFT2，各训练一轮，显式
...
SFT1的一轮遍历613条完整轨迹，约13个 optimizer step；SFT2以回答前缀为样本，
完整遍历7309个训练回答，约153个 optimizer step。SFT2 batch1 是一个回答及其
之前的全部真实历史，不截断或抽样回答。
每卡 batch1 和 GA8 保持不变；world6 的有效 batch 是48，低于此前配置的64。
...
六卡 rank 映射、合并 checkpoint 阶段元数据和最终120 episode 完整性都会检查。
