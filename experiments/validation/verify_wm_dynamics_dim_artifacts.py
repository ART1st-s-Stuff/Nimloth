...
def _evaluation_gate(root: Path) -> dict[str, Any]:
    ...
    if dynamics.get("one_step_mode") != "direct_predict_next" or dynamics.get("one_step_control_mode") != "direct_predict_next":
        raise ValueError("direct one-step metric/control mode missing")
    ...
    if dynamics["horizons"]["1"].get("shuffled_mode") != "autoregressive_rollout":
        raise ValueError("autoregressive shuffled-control mode missing")
