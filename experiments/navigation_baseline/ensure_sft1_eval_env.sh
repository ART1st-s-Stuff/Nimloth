...
ROLLOUT_ENV_RUN=${SCRIPTDIR}/runs/sft1_rollouts_vagen79_greedy_parallel
ENV_CONTROL=${ROLLOUT_ENV_RUN}/external_env_4gpu
ENV_URL_FILE=${ENV_CONTROL}/env_urls.txt
ENV_NODE=${ENV_NODE:-dgx-35}
...
submit_env() {
  $SLURM --account=peilab --nodelist="${ENV_NODE}" \
    --export=ALL,PORT_BASE="${PORT_BASE}" \
    "${SCRIPTDIR}/sft1_env_vagen79_4gpu.slurm" | awk '{print $NF}'
}
