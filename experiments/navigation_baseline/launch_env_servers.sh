...
# Launch two 4-GPU VAGEN navigation env servers.
# Defaults: each server has max_envs=64/thread_pool=64, total capacity=128.
# Overrides:
#   PARTITION=normal bash launch_env_servers.sh
#   MAX_ENVS=48 THREAD_POOL=48 bash launch_env_servers.sh
#   SBATCH_EXTRA="--nodelist=dgx-21" bash launch_env_servers.sh  # advanced/manual
...
HOSTFILE=${SCRIPTDIR}/env_server_host.txt
JOBFILE=${SCRIPTDIR}/env_server_jobs.txt
