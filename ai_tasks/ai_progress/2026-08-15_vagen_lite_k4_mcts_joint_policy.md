...
- K4 actor replay已接入：current actor使用persisted MCTS root means和批准beta重建Scheme-B PPO ratio/entropy，planner scores强制detach，actor梯度只进入current Qwen logits；main_ppo完整保留K4 search config到custom actor，training contract ID绑定K4 policy。VAGEN现为`c48b144`；由于superpod SSH当前返回`Connection closed by UNKNOWN port 65535`，torch/server回归仍待执行，不能视为验证通过。
