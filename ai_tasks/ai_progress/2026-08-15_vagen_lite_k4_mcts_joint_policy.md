...
- K4 full planning transaction已接线：rank-consistent projector/predictor/ValueHead及统一optimizer fingerprint；rank0原子导出full planner transport并stage/CAS发布；initial K4 bootstrap、atomic sidecar、active transport、三模块与optimizer state、fresh-runtime restore identity均已实现，K4 WM配置进入training contract hash。VAGEN=`ea8905d`。通用production gate仍关闭；superpod torch回归待执行，不能据此宣称production ready。
