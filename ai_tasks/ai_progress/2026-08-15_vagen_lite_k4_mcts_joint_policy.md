...
- ID182现为`needs_restore_only_retry`且可从完整`global_step_1`恢复；严禁重跑update。server README/interim_status/progress已记录。修复必须让两个phase都显式进入VAGEN cwd，并为新fresh restore attempt使用不覆盖旧失败证据的新phase目录；再次占用8 GPU前仍需人类批准。
