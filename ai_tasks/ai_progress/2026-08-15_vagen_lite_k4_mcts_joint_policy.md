...
- corrected parent`17c92ce7`把bootstrap W&B local目录移至`${RAY_LOG_ROOT}/wandb`，正式`RUN_OUT`仍只允许phase runner首次创建；静态回归同时拒绝旧路径。下一retry必须fresh committed worktree；phase2继续等待phase1完整step5/validator/W&B。
