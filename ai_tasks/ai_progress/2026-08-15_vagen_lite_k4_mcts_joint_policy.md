...
- Job`521851`在`dgx-[09,18,30,37]`通过exact4×2/Ray/import/source hash/SIGReg后，head `dgx-09`的150秒FloorPlan1 direct-render零输出exit124；终态`FAILED 124:0`、elapsed`00:04:41`。无env server/TaskRunner/validation/rollout/optimizer/W&B/checkpoint；四节点pre/post manifests齐全、cleanup为空。失败root README已记录且不可覆盖，新增`E0127`。2×4 test-only预计22:42，排除09后的4×2预计18:43，故4+4可行但更晚；retry保留4×2，排除09/13/32/51，改用`_retry1` output和W&B identity。
