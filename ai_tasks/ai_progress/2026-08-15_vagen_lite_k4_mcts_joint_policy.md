...
- retry2分离worker与Navigation-head eligibility：23/37仍可作模型worker，head额外排除13/23/32/37/51并在node/IP identity中强制排首；无合格head即fail closed。common/runner cleanup仅按继承的runtime-root environment认领进程，避免cmdline误杀协调`srun`。parent=`3a90e9db`、服务器launcher`11 passed`、独立Opus review`APPROVE`无P0/P1。尚未提交retry2。
