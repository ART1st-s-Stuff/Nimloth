...
- corrected parent`17c92ce7`把bootstrap W&B local目录移至`${RAY_LOG_ROOT}/wandb`，正式`RUN_OUT`仍只允许phase runner首次创建；静态回归同时拒绝旧路径。fresh五层clean retry3 worktree已建，phase1 Job`520711`于16:52提交。当前允许节点仅dgx-52有≥4空卡，dgx-18仅2卡，任务为`PENDING(Resources)`，scheduler估计19:28；未占GPU、未创建正式output/W&B。phase2继续等待phase1完整step5/validator/W&B。
