...
- corrected parent`386ca4ed`把`/cm/shared/apps/slurm/current/bin`及固定`SLURM_CONF`传播到raylet/driver，direct server probe为`slurm 23.02.6`，launcher`10 passed`；cleanup改为重试双节点owned-process audit，并在已有主体错误时保留primary status。fresh retry runtime worktree已建立且五层clean；phase2继续等待phase1完整step5/validator/W&B。
