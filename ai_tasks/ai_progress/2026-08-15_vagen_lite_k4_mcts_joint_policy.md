...
- 新增tiny完整`actor PPO + WM + DINO + SIGReg + ValueHead`单update与`export -> persist -> fresh actor restore -> fingerprint`测试；发现并修复transport字段名误写（`E0116`），SIGReg按每个global micro-group有效样本数归一，避免gradient accumulation改变正式`0.1`权重。服务器最终`169 passed, 120 subtests`；parent=`e6dd1d28`、VAGEN=`6cd96ef`、VERL=`494f2644`，服务器三层clean。通用production gate仍关闭，未启动GPU optimizer、canary或长训练。
