...
- Compressed vision-token train launcher 验证：本地 `bash -n` 和 `py_compile train.py` 通过；服务器 worktree 同步到 `6974f46` 后 `bash -n experiments/training/representation_ablation/train_compressed_vision_predictor.slurm` 与 `py_compile train.py` 通过。Job `466245` 暴露默认 `.venv`/transformers 4.49 不兼容 adapter-backed checkpoint；修复为默认 `.venv-vagen-main` 后，服务器 `bash -n` 与 `.venv-vagen-main` import/config smoke 通过。
