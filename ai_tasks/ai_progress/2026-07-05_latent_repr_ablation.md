...
- B 线下一步：checkpoint 权限已解决，vLLM allocator blocker 已修复；当前 blocker 是 current VAGEN 缺少/不接受训练 eval sets `base_train` / `common_sense_train` / `long_horizon_train`。需要人类确认真实 train dataset source 或兼容 VAGEN checkout；不得把 heldout `base` / `common_sense` / `long_horizon` 当成 SFT train 数据。若需要修改 VAGEN submodule 或补 train dataset files，必须先停止并请求人类确认。
