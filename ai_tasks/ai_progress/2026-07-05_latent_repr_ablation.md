...
- B 线下一步：先让 step300 HF model shards 对当前提交用户可读（或提供可读副本），再用新输出目录重试 Plan B；重点确认是否发生 env reset、是否写出真实 `validation/*/shard_*/300.jsonl`。若后续发现必须修改 submodule，必须先停止并请求人类确认。
