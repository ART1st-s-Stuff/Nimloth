...
- Plan B retry15 smoke 已启动：复用 retry14 env job `466322`（`dgx-05`, ports `19530-19533`），新 run dir `/project/peilab/atst/nimloth/experiments/navigation_baseline/runs/sft1_rollouts_highsuccess_step300_greedy_parallel_retry15`，manual `srun --overlap` array task 3 PID `1284150`。截至最近检查：进程仍运行，config 中 `limit_mm_per_prompt: 20`，vLLM 日志显示 encoder cache profiled with `2 image items`，已越过 retry14 的多图报错并持续生成；尚未写出 `validation/**/300.jsonl`。
...
- B 线下一步：retry15 smoke 正在运行；继续监控直到 `validation/**/300.jsonl` 出现或新失败。若 array task 3 smoke 成功，再决定是否启动 train shards 0-2；不得把 heldout `base` / `common_sense` / `long_horizon` 当成 SFT train 数据。
