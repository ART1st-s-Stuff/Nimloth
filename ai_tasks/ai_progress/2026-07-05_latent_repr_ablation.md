...
- B 线下一步：`/project/peilab/atst/vagen_ckpt/actor/huggingface` 的两个 safetensors shard 仍不可读；需要先让 step300 HF model shards 对当前提交用户可读（或提供真正可读的 HF 副本），再用新输出目录重试 Plan B。另一路是明确确认后把可读的 actor FSDP shards 合并成新 HF 目录。重试重点确认是否发生 env reset、是否写出真实 `validation/*/shard_*/300.jsonl`。若后续发现必须修改 submodule，必须先停止并请求人类确认。
