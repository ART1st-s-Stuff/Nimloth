...
- The 12/20/22/24-GPU 16-rollout configs and both rollout launch paths propagate
  the same cap.
...
- Focused pytest has not yet run locally because neither available local Python
  environment contains pytest. Remote vLLM 0.11/API and focused tests remain
  to be run after commit synchronization.
