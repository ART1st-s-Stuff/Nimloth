...
- retry2实际质量：106/960 success=11.04%；base_train17.50%，common10.31%，long5.31%。人类随后纠正：要求的是源训练过程中evaluation的`actor_rollout_ref.rollout.val_kwargs`，不是optimization rollout sampling；正确参数是`do_sample=false, temperature=0, top_p=1.0, top_k=-1, n=1`，每轮512 tokens/20 turns/1 action。retry2实际使用greedy0且runtime kwargs为top_p1/top_k-1，参数本身符合该要求；agent把“train数据”擅自解释成应改0.7/0.95是第二次理解错误。按人类明确命令，已永久删除active `rollout/validation`的19,336 files/8.0G（8 JSONL、19,328 PNG），有效count=0。wrapper现显式写出top_p/top_k，所有新train/val/test样本统一使用源train-time eval参数。
