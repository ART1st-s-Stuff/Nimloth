...
- 人类要求结构禁止T mismatch并以T1重评：`9978971`/server42 tests强制input T==config、T1 API只接受one-slot architecture、删除fake padding；SFT2新增`--wm-history-size`且当前trainer仅接受1/checkpoint必须匹配；legacy bad metadata仅显式迁移slice trained pos0。corrected recursive-T1 eval479918完成，W&B`x8fqdses`：pred L1.320138/state MSE.003416/cos.70357，horizon无单调漂移，图仍beige collapse；与旧T4数值/图几乎相同。故T mismatch是结构bug但非本checkpoint视觉差的主因；T1 h1已off-manifold。Projected CFM ratio1.0028限制结论。
