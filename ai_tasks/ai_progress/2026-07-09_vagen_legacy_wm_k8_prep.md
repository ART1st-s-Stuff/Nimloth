...
- 原实现没有保存W&B内部run ID，抢占恢复会创建同名新run。新增`wandb_run_id.txt`持久化和`wandb.init(id=...,resume="allow")`并登记E0024；恢复前会写回run ID `5zm5pxqx`，继续同一project ID2 run。preempt节点当前无健康8GPU空闲（显示空闲的节点均DOWN/NOT_RESPONDING），暂不能健康恢复。
