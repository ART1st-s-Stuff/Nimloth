...
- predicted State可视化`3882d31`/server11 tests：WM输出projected8192，不能喂direct Query CFM；因此exact projector离线生成aligned projected cache，训练Query/Projected→Qwen16×512 adapters并由同一frozen Qwen CFM解码。actual projected step0+recorded actions自回归h1..5；五列`GT|Qwen|Query actual|Projected actual|WM predicted`，报state/token/image/horizon metrics。smoke479779 RUNNING，afterok adapter479780/eval479781。
