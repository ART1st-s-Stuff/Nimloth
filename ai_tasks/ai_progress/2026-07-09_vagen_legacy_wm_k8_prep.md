...
- 人类指出RL可加载后同步最近RL canonical handoff：stable snapshot→merge/verify826 adapters→full HF。`6fdeab6`同步prep scripts并让recon拒绝raw PEFT，使用现有epoch2 RL_INIT_READY merged。single/2-rank gate各77/12 states bitwise exact/finite（FP b7229f8cdebe4ac9/5eb8cebc723ee099）；job479394仅post-check heredoc SyntaxError，登录节点同validator通过。按授权删除raw/invalid/failed/cancelled17/17b/18/19/20 outputs并clean重建。retry cache479397健康RUNNING（peft=false/handoff_ready），afterok train479398/eval479399已提交。
