...
- fair8-token CFM smoke479923因sample mode bug失败；`685ef62`按representation修复/server18 tests。retry479931训练完又因project quota满在final atomic save失败。failed ckpts删除；completed ID19/25 intermediate ckpts清理约5GB，仅保留best/final；dead deps取消。storage-safe smoke479937 PENDING→formal479938→eval479939。
