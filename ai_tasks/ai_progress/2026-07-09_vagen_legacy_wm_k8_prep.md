...
- 按人类要求启动精确same-scene对比`3_compare_k8_vs_vit_oldsamescenes_r8_t5_euler50_cfg2`：`afb3773`固化旧combined原8 records/order/first5 actions，用当前epoch2 Qwen+StateProjector重新编码这些旧prefix的k8 states，再以完全相同GT/actions/noise、Euler50/CFG2对比`GT | lowLR ViT GT/Pred | current k8 GT/Pred`。combined job474920在normal/dgx-27运行，W&B `nimloth-recon` ID3。实验README未quote heredoc导致Markdown反引号被执行，已修正文档并记录重复错误`E0028`。
