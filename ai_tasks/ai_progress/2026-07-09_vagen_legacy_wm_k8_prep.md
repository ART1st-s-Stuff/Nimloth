...
- single-node8 job476365 ETA51h后按人类要求改normal fragments。nonuniform launch先后暴露one-task/GPU invalid ordinal与首barrier device猜测hang；`5e2b454`显式ProcessGroup device/server26，登记E0033。476457因stale状态被误取消18s，补充E0029。当前476479使用dgx32:2+dgx54:2、world4/GA8精确有效batch32，W&B`z3c0w63v`，至少88 finite steps/median9.43s/micro/GPU63GiB。IB benchmark33.5GiB/s；world2 IB诊断476486健康但更慢后停止。真实46,569 base batches→world4 11,643 micro/epoch；CSV9.43s interval实际为GA8 optimizer step，latest step128对应micro1024，故正确估算约3.8h/epoch/7.6h两epoch+checkpoint/val（先前误乘micro count得到61h，已纠正）。latest完整且world4/GA8/8192/dynamics2048 invariants通过。homogeneous world4 IB resume476507以`afterany:476479`提交，若当前8h allocation未完成则从20min latest/同W&B续跑，且禁止两个writer重叠。
