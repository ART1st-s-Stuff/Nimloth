...
- WM-pred evaluator`b1776d4`+`dba2a03`/server11 tests：diverse40六列`GT|QwenGT|QwenWMpred|queryGT|projectedGT|currentWMpred`；两WM均从step0按5 actions autoregressive，current1024 pred经best projected→Qwen adapter渲染，projectedGT控制adapter distortion。无8-query WM pred，因为当前WM不输出8×2048，禁止虚构。job475996 pending priority；两次gpt5.5监控子agent被harness等待abort，job未失败。
