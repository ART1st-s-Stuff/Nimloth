...
- 对照实现branch`exp/k8-preprojection-recon` commit`5d92e7e`：保存Qwen final-norm`[8,3584]` query hidden；paired token patch decoder保持shape-compatible body同初始化和完全相同sample/target，对比projected`[1,1024]`/preprojection`[8,3584]`，独立best并报告correct/wrong/PSNR/output delta/contact；server18 tests passed。
- smoke475045 `COMPLETED00:01:07`：cache77/16、alignment、paired step10/checkpoint/contact/W&B`90im9e9r`均通过；resume475048 `COMPLETED00:00:32`，cache hit、step10→12、同W&B ID。原normal8 formal475049/475050因预计2026-07-16启动而执行前取消；normal2 cache475052在dgx-29运行00:05:10并仅写2×512-row partial shards后，按人类提示dgx-22有4GPU而取消，train475053执行前取消。有效replacement为preempt/dgx-22 cache475078（4GPU/nproc4/64CPU/128G/4h，原子重建）→ normal train475079（b16/18,560≈5ep、W&B ID10）。提交时节点alloc4/8，调度前变成alloc8/8，当前475078 `PENDING(Priority)`；已有hold需job ID才能attach。formal README准备时重复E0028 unquoted-heredoc错误，仅损坏文本未影响命令，已用Python完整重写并读取核验。
