...
- 该错误对应upstream static_graph+no_sync regression：no_sync跳过prepare_for_backward，但static-graph sink仍finalize。修复策略保留多forward/checkpointing依赖的static graph，并在当前runtime每个micro-batch同步DDP梯度；all-reduce线性，因此与累积后同步数学等价，但通信变多。retry必须使用新W&B ID2和新输出，comment=`ddpsyncfix`；cache继续只读复用。
