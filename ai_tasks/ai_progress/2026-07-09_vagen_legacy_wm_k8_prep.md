...
- retry2实际质量：106/960 success=11.04%；base_train17.50%，common10.31%，long5.31%。随后人类指出前文已明确source train sampling：源training rollout是`do_sample=true, temperature=0.7, top_p=0.95`，只有validation/test是greedy temperature0。本次错误地把validation parity kwargs用于production train collection；根因是未把train rollout与val/test调用场景分开核对。960 records虽有0 bad JSON/empty/missing image，但对当前训练目的已判无效，禁止conversion/SFT，等待人类决定删除或隔离。另有6 records共30 assistant turns格式无效。正确重启前须恢复源train sampling并单独保留val/test greedy，先做train sampling质量gate。
