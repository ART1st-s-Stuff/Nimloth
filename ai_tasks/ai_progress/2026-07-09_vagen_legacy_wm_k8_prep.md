...
- 人类要求结构禁止T mismatch并以T1重评：`9978971`/server42 tests；eval479918 W&B`x8fqdses`仍collapse，pred L1.320138/state MSE.003416/cos.70357，证明T bug非主因，T1 h1已off-manifold。
- WM validator479922完成 W&B`gs21abz9`/5699pairs：predictor MSE.003004差于identity.002663和trainmean.002403；但wrong action.003525，correct best41.57%/rank3.04。整体next-State未训好但turn/back有action signal；dominant forward明显差于identity。
- fair8-token CFM前两次smoke分别遇sample mode bug和quota atomic-save failure；修复/清理后storage-safe chain479937→479938→479939。
- 新SFT2 T4-tokenized-residual：`1541c50/cbd879f/5bb5a4e/4728f0a`，server latest49 tests；State8192→8×1024、T4=32tokens、masked left PAD/block-causal、zero-init residual、同record teacher-forced/nonpacked contexts与recursive warmup；125,962,312 params；identity-relative metrics和strict invariants。
- smoke479978完成2:27/W&B`ndcz52xp`：val WM.00463178优于identity.00469884，T4/token8/residual path通过。formal479979两components仍PENDING Resources，scheduler最早约10:49 HKT且不保证；ID29 strict3217/val355/world4 pair/GA8/5ep/20min resume。
- fair8token CFM完成：full ratio1.002517未过gate；旧single-token bottleneck非主因。
- T4 residual SFT2 formal479979/W&B`nvy3monl`：epoch1 step1655完成，val WM.00240206 vs identity.00369671/gain+.00129465，cos.900947>.851121，写epoch1/best。16:33 step~1696 epoch2，recent100 gain+.001428/89%正，latest16:30，无error。20:24 allocation cap预计epoch2后段，需续提。
