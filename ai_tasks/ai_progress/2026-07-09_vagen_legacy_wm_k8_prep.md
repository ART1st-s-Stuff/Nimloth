...
- direct8-query CFM实现`f76ddc6`：query manifest恢复8×2048 cross-attention、proven CFM body shape-compatible init、dropout0.15、lr3e-5→1e-5、CFG2、matched positive/query-wrong sheet；local compile/diff通过。服务器测试/提交未发生：SSH连续`Connection closed by UNKNOWN port 65535`，按SERVER规则判断VPN断开并停止，待人类重连。
