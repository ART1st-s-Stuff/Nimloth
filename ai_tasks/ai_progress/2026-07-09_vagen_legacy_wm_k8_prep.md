...
- resume job474104已无节点绑定提交preempt 8GPU，当前`PENDING(Priority)`。提交时没有健康8GPU节点（显示8卡空闲者均DOWN/NOT_RESPONDING）。checkpoint剩余12,423 optimizer steps；按step2137前含val/checkpoint的实测throughput估算，分配资源后约需14小时计算，排队和后续抢占延迟未知。
