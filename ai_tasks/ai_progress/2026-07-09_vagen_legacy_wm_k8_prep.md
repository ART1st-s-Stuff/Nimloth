...
- 71.67% parity与11.04% train不是同任务分布，不能直接比较。parity使用上述heldout60 geometry×2 wording，直线起终点距离均值2.74、最大3.00；retry2使用base_train/common_train/long_train前320，均值3.60/3.40/3.50、最大8.26/8.31/9.99。前320明显比全train1-1080更远，且源train config只训练base/common、没有long-horizon。重启前必须在实际目标split按类别/seed段做质量gate。
