...
- action-sequence evaluator`66b1e27`/server15 tests：严格复用5ulzuwun八序列，并加4条current heldout同时含turn-right4/turn-left5序列；每个post-action actual query以Euler50/CFG2/matched noise生成`GT|8-query CFM|same-horizon wrong`。这是trajectory reconstruction，非WM prediction（WM仅输出projected1024）。combined old-query cache+60-frame job475924已在dgx-09 RUNNING，W&B ID15。
