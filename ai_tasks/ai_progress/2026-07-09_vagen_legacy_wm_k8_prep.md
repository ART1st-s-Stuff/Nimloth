...
- W&B epoch1 val payload因running code使用transport step=epoch1、低于train step1456而被忽略；CSV/checkpoints保留完整结果，训练不受影响。本地修复`log_val_epoch(..., global_step=...)`并登记E0023；active process仍用afe2bf9旧代码，run结束后需从CSV回填val曲线。
