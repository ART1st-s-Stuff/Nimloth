...
- Commit`aef9f2d`实现独立fail-closed resume gate：成功result与world8 checkpoint必须匹配，加载后actor/critic fingerprint须等于ID39 source，source文件不删除，下一次critic state必须到step2并保存新global_step2。尚待新GPU identity direct验证。
