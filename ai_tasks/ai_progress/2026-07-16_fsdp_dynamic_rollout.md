...
- 人类纠正full-response mask不代表VAGEN token credit，并选择独立Qwen critic；登记E0049。实现可选turn_mc/masked_gae；pilot固定masked-GAE gamma1/lam1/final reward/schema4，保存逐token critic values及独立full-Qwen critic/8 rank optimizer。Actor显存修复为train-mode checkpoint+dropout0+selected logits+microbatch1；定向82 passed。ID25/u0bn3m72/job479415因rank runner漏PYTHONPATH在模型加载前28s失败，登记E0050。ID26/eow1bi6d/job479441动态分配dgx51+09+14+18；真实20-turn/4548-token actor forward启动，但root-only FSDP与train-mode non-reentrant checkpoint backward产生tensor metadata mismatch，8rank1:41失败、0 optimizer/critic/result，terminal并登记E0051。commit4724649加入actor/critic Qwen decoder+vision-block transformer auto-wrap、nested-unit gate和resume protocol；server tests83 passed。按新Slurm规则获批normal/8GPU后，ID27/dln2t4fd只提交一个bash hold：attempt1 job479491全PENDING/0时取消以预发布stage；attempt2 job479500于16:39实际1@dgx27+14+29+51+54+26及2@dgx18，55秒后释放。Nested FSDP gate及4548-token actor forward通过，但8rank首个actor backward仍CheckpointError，0 actor optimizer/critic/result，terminal/non-resumable。已修正E0051：Transformers4.55 Qwen block内部checkpoint绕过wrapper-level重放，仅layer-wise FSDP auto-wrap不充分；下一实现须禁用HF内部GC并使用同block边界的PyTorch external checkpoint wrapper。world8双模型显存仍待验证，quality pilot另被thought collapse阻塞。
- 进一步核对VAGEN源码：navigation脚本明确为actor和critic开启non-reentrant gradient checkpointing；VERL固定Transformers4.49，其Qwen外层model loop checkpoint `decoder_layer.__call__`，重算会重新进入已替换的FSDP wrapper。当前Transformers4.55.4将checkpoint移入`GradientCheckpointingLayer`并调用`partial(super().__call__)`，这是同名配置在ID27失效的关键版本差异；已补充E0051。commit96afef6禁用HF内部GC并采用external block CheckpointWrapper；server最终86 passed/2 warnings。ID28/layssyjb先将全PENDING/elapsed0的normal hold479574按人类指示替换为preempt dgx38 hold479602。Tiny全参数Qwen多模态distributed gate在8rank均完成2 checkpoint wrappers/3 FSDP units/backward/Adam；真实冻结base+LoRA r64 actor仍在首个4548-token backward出现同一CheckpointError，错位包含LoRA矩阵形状，0真实actor optimizer/critic/memory结果，terminal/non-resumable。最终CPU exact-LoRA最小复现确认真正差异是LoRA dropout context：forward临时p=0后在backward前恢复0.05，checkpoint重算新增`[1,4548,2048] bool` mask并使saved/recomputed列表错位；backward留在context内通过。commit d09e8b0把RL LoRA dropout默认/runner/probe固定0，非零fail-fast且写protocol；GPU尚未复验。按人类选择“仅失败或作废实验”，删除retry1、ID3、ID9、ID11、ID12、ID24的大checkpoint/model/optimizer/images，共96959711478 bytes apparent，保留metadata/log及ID10/ID22成功mechanics产物。新增`2026-07-18_verl_rl_assessment.md`；人类随后明确决定正式RL采用VERL+全量训练：actor语言/视觉全参数、独立critic全参数、immutable ref，停止将LoRA和自写PPO/FSDP trainer作为正式路径。首个DataProto exact replay里程碑已完成：严格prompt/response padding、mRoPE/multimodal、thought/action mask1、latent/action scaffold mask0及action-token reward placement，并直接通过pinned VAGEN masked-GAE；server完整suite91 passed/3 warnings。下一步接VAGEN多轮latent-query rollout和VERL StateProjector/WM auxiliary/checkpoint；在线/full-worker direct gate未完成且新teacher/SFT init前禁止quality pilot。
