...
- 当前normal资源不再满足历史4+2+2 fixed-node入口。新增scheduler-selected normal `3+2+2+1` world8 trainer + independent env GPU atomic launcher；active SFT2 dgx-[18,32,52,54]硬排除，并在allocation内先跑bounded env semantic preflight。commit e84933f；server `test_dynamic_rollout.py` 31 passed、bash syntax和Slurm配置检查通过。
- 新ID19/W&B`rvz46qrl`/job478923已于2026-07-17 23:15提交。配置为ID12 immutable full Epoch2 init、`base_train` seeds30002/30003、1 iteration、2 episodes×max2 steps、fresh LLM LoRA+WM+Value train、vision/StateProjector freeze、schema-v3 thought+action PPO、actor old/reference replay、low-var KL0.001和FA2。五个het groups当前均PENDING/elapsed0/Resources；未分配节点、未跑preflight、未产trajectory/update/final，不能声称端到端成功。fixed20 baseline继续被本smoke阻塞。
