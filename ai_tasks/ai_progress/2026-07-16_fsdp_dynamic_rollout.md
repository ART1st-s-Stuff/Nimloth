...
- 修复`465e8bd`：PEFT后按module path遵守显式Vision tune mode；本次freeze gate要求language B nonzero、visual B全zero；StateProjector按source BF16构造并严格dtype/value不变。server26 tests passed。人类随后纠正：Vision tune是实验可配置参数，不能把该次freeze表述为永久LoRA=0要求；登记E0030。launcher改为`VISION_TUNE=freeze|lora|full`及mode-aware gate。
...
- Pilot新exclusive output=`.../2026-07-17/11_pilot_fsdpdynamic_k8full8192_sft2step3059_frag2x1_iter20_b8_heldout20_visfreeze`，W&B ID11/run`u67p32ee`已实际创建并持久化step0。初始MODEL/SNAPSHOT引用成功ID10的immutable prep step3059（不使用ID10 RL checkpoint）：20iter×8 `base_train`×max20、batch2；固定20 heldout `base` baseline+iter5/10/15/20、temperature0；iter5累计0 train success early-stop；本次Vision freeze，StateProjector freeze，WM/Value训练。初次无resume；中断后只允许同world2显式完整iter checkpoint resume。硬exclude live SFT14/26/51/54，env dgx37。
