...
- ID14/15分别用shared/dedicated dgx37 env，都在首条episode后全trainer GPU0%且无trajectory。ID16 W&B=`epzlgdud`/478689通过SIGUSR1 stack证明尚未进入policy forward：rank0阻塞VAGEN `create_environments_batch`，其他ranks正确等待Gloo；600s后`ReadTimeout`。先前FSDP deadlock归因撤销；登记E0037（HTTP health/Unity process不证明create+reset）。ID16 11:36取消/0 steps/terminal。下一smoke用ID13已到真实policy turn的dgx51专用env，trainer改4+2+2 normal fragments。
