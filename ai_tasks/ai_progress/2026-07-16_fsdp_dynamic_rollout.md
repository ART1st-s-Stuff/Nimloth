...
- ID14 W&B=`cfm6lpiz`/job478603通过world8/free-port/env/multimodal gates，但共享rank7 env GPU使CloudRendering持续99% SM并污染诊断；11:18取消，0 step。ID15 W&B=`wnu71oeo`/job478647用8 dedicated trainers+独立dgx37 env仍复现首个真实multimodal forward全trainer GPU0%、process poll/futex sleep，故sharing无效但不是已证root cause；7:36取消，0 step。两者均FAILED/NON-RESUMABLE。下一run增加SIGUSR1 thread dump与per-rank forward enter/model-exit/token-selected/input metadata，禁止无证据归因。
