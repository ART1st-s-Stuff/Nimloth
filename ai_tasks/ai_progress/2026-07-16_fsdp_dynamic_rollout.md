...
- 进一步核对VAGEN源码：navigation脚本明确为actor和critic开启non-reentrant gradient checkpointing；VERL固定Transformers4.49，其Qwen外层model loop checkpoint `decoder_layer.__call__`，重算会重新进入已替换的FSDP wrapper。当前Transformers4.55.4将checkpoint移入`GradientCheckpointingLayer`并调用`partial(super().__call__)`，这是同名配置在ID27失效的关键版本差异；已补充E0051。commit96afef6禁用HF内部GC，按raw decoder/vision block先应用PyTorch external non-reentrant CheckpointWrapper、再FSDP auto-wrap内部block；actor/critic均加入wrapper-count gate和严格resume protocol。server84 passed/2 warnings且tiny Qwen external-checkpoint backward通过；distributed FSDP backward与真实20-turn显存尚未验证。
