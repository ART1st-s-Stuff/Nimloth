...
- 修复`465e8bd`：PEFT后按module path冻结非显式vision LoRA；gate要求language B nonzero、visual B全zero；StateProjector按source BF16构造并严格dtype/value不变。server26 tests passed。下次需全新W&B ID/output并从届时job477349 latest重新稳定snapshot；依规则等待人类批准再提交。
