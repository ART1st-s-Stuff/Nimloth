...
- 人类纠正full-response mask不代表VAGEN token credit，并选择独立Qwen critic；登记E0047。实现可选turn_mc/masked_gae；pilot固定masked-GAE gamma1/lam1/final reward/schema4，保存逐token critic values及独立full-Qwen critic/8 rank optimizer。Actor显存修复为train-mode checkpoint+dropout0+selected logits+microbatch1；双模型steady-state20-turn probe已编码但未运行。定向82 passed，真实world8仍待验证。
