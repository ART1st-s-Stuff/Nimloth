...
- 新增纯behavior replay TDD里程碑：VAGEN`1e70366`的`replay_guided_behavior_log_probs`逐条重验record并强制expected contract/snapshot，同batch必须同action table/config/snapshot；只从record读取rollout-time all-action frozen Q，current prior logits为唯一梯度入口，返回guided action对应的current与behavior log-prob且没有current-Q输入。真实CPU Torch/ledger定向`57 passed,7 skipped,7 subtests`，review批准。actor FSDP、Q owner、环境动作、optimizer/checkpoint均未接通，trainer继续在worker前fail closed。父gitlink已发布`b1f892fc`，VERL仍为`084f042b`。
