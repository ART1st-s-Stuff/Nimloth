...
因此，下一步不应继续修补旧世界模型或扩大旧适配器。更稳妥的路线是先用现有缓存扩大只读统计检查，再把当前图片表示和任务指令表示融合成一个新的统一状态；只有该状态通过视觉、目标和动作结果检查后，才从头训练新的世界模型、价值模型和搜索策略。
\end{abstract}
...
ID192已经保存14,261个状态的冻结特征缓存。建议先进行按原始图片内容分组的五折交叉验证：每次使用四组训练小型分类器，在剩余一组产生从未参与该分类器训练的预测，最后合并所有预测。

该检查的目的只是缩小“语言模型后图片表示与DINO相差多少”的统计范围。它不能冒充新的外部验证；现有归档外部验证结果仍应作为主要外部结果。该步骤不需要重新运行主模型，也不更新任何部署参数。

\subsection{第二步：训练新的统一状态小规模验证}

如果第一步支持语言模型后图片表示，可以测试：
...
\begin{equation}
  ...
  G\!\left(
    z_{\mathrm{anchor}},
    A_v(v_{\mathrm{image}}),
    A_g(h_{\mathrm{instruction}})
  \right),
\end{equation}

其中$v_{\mathrm{image}}$是同一次模型计算中、经过语言模型处理的当前图片表示，$h_{\mathrm{instruction}}$是精确instruction表示。$A_v$和$A_g$只负责把尺寸转成可融合形式，$G$最终仍输出一个$16\times1024$状态。
...
\begin{itemize}
  \item 视觉结构与DINO的比较；
  \item 目标分类；
  \item 向前、向左、向右动作结果的AUC、精确率--召回率和概率校准；
  \item 对真实回答措辞变化的稳定性；
  \item 训练与外部验证之间的稳定性。
\end{itemize}

如果语言模型后图片表示在扩大检查中确实不足，再考虑用DINO作教师进行蒸馏，或修正视觉编码器。DINO优先作为训练和评估的教师，不默认成为部署时的第二个视觉模型。

\subsection{第三步：从头训练能表示动作失败的一步世界模型}

新世界模型应预测动作成功概率，并分别建模“成功移动后的状态”和“动作被挡住后的状态”。真实动作结果只可用于监督和评估，部署时不能作为已知输入。

一步模型必须按每个主要动作，以及成功和被挡住子集分别优于复制基准。通过后才可扩展到第2步和第4步。

\subsection{第四步：重新训练价值模型并恢复搜索}

新状态定义确定后，应从头训练并校准ValueHead和动作价值。先检查真实状态和一步预测状态，再检查第2--4层预测。只有预测深度增加时价值排序和回报校准没有持续恶化，才恢复K4搜索和新的联合强化学习。
...
在这些证据下，继续增加旧世界模型训练轮数、扩大ID191适配器，或直接恢复K4强化学习，都不能解决基础输入问题。合理顺序是：扩大冻结统计检查，建立新的统一状态，通过视觉、目标和动作结果门槛，再从头训练一步世界模型、长预测、价值模型和搜索策略。
...
\begin{itemize}
  \item \path{ai_tasks/wm_visual_goal_state_alignment_plan.md}
  \item \path{ai_tasks/ai_progress/2026-08-23_frozen_sft1_goal_probe_residual_t1_canary.md}
  \item \path{ai_tasks/id191_state_interface_direction_canary_contract.md}
  \item \path{ai_tasks/id192_multimodal_feature_location_audit_contract.md}
  \item \path{/workspace/remote2/nimloth-artifacts/id61_id75_action_outcome_audit_retry1/result.json}
  \item \path{/workspace/remote2/nimloth-artifacts/id71_frozen_state_action_outcome_probe/result.json}
  \item \path{/workspace/remote2/nimloth-artifacts/id191_state_interface_direction_canary_retry1/result.json}
  \item \path{/workspace/remote2/nimloth-artifacts/id192_multimodal_feature_location_audit_retry2/result.json}
\end{itemize}
