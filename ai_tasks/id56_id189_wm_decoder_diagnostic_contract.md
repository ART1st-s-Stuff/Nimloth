...
- WM retains nontrivial signal: executed action is closest among eight depth-1 states on `46.67%` (chance `12.5%`, mean rank `1.83`). The original report said predicted beats copy against the frozen real-next DINO target on `100%`, RMSE `0.83801` versus `0.98837`; later code audit found that ID56 first resized the real image to the CFM decoder's `128×128` resolution. These two numbers are therefore a legacy decoder-resolution DINO sensitivity result, not the exact original-observation WM teacher target. See `E0144`; ID57 is the corrected read-only comparison.
