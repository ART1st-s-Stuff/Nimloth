...
- W&B: project `nimloth-sft2`, ID16, run `16_k1inject_all3217_qadapter_vfull_wmtrain_ep10_b2_ga16_ws2_px100352_img12_bestwm`, internal run ID `az8nqwv9`. IDs3/4 were initially selected while queued but concurrent SFT2 runs advanced the sequence through ID15 before this control received any allocation. ID16 is now an actual persisted W&B reservation rather than a documentation-only name.
- Output: `/project/peilab/atst/nimloth/outputs/experiments/vagen_legacy_wm_k8_full/2026-07-10/full_2e66e97/control_k1/sft2/16_k1inject_all3217_qadapter_vfull_wmtrain_ep10_b2_ga16_ws2_px100352_img12_bestwm`.
...
- Job `476029` targeted the two free dgx-27 GPUs but remained elapsed0 and the node dropped to one free GPU. During the continued wait, concurrent `nimloth-sft2` runs advanced the numeric sequence through ID15. Pending ID4 job `476338` was cancelled elapsed0 with no training output.
- To prevent further ID churn, the control atomically moved to ID16 and created/persisted actual W&B run `az8nqwv9` with only `queue/reserved=1` at W&B step0. It is displayed finished while queued; training will reopen the same internal run via `train/wandb_run_id.txt`, and its first train global step is1.
- Final job `476585` started on dgx-52 and successfully resumed reserved W&B run `az8nqwv9`. Log/checkpoint metadata verify k1/inject/query-adapter, Qwen freeze, vision full+EMA, WM/value trainable, required compact BF16 cache, world2/batch2/GA16, and sync-each-microbatch DDP.
- Epoch1 completed at step1456: val WM MSE=`0.0046331959`, SIGReg=`0.4078793445`, value total=`0.1223068553`, fixed rollout success fraction=`0.3098591549`; epoch1/best/latest complete. At elapsed06:45 epoch2 reached logged step2085 (43.2% of epoch2, 14.3% overall). Latest periodic checkpoint metadata=`epoch2/step2032/micro9216`, so current recovery rewind<=53 observed steps.
- GPU memory=~62.5–63.0GiB/81.6GiB and utilization=97–100%; no traceback/OOM/NaN/Inf. W&B train/val payloads are visible.
- Epoch2 completed at step2912: val WM MSE=`0.0023899326`, SIGReg=`0.4059752358`, value=`0.1252817440`, fixed rollout success=`0.3098591549`. WM MSE improved48.4% from epoch1 and is 6.5% above prior k8 epoch2 (`0.00224416`); best updated to epoch2 and epoch2/best/latest are complete.
- Human directed pausing epoch3 for RL feasibility work. Job `476585` was cancelled without training error at elapsed11:02:38 after logged epoch3 step3213. Latest periodic metadata=`epoch3/step3125/micro3408`; complete epoch2 step2912 remains best and available.
- Full CSV is archived as `train_step_log_paused_epoch3_job476585.csv`; active CSV was atomically trimmed to checkpoint step3125. Latest resume rewinds88 logged steps; epoch-boundary resume may explicitly select `epoch_002`. W&B `az8nqwv9` was cleanly finished and remains resumable by persisted run ID. GPUs were released.
